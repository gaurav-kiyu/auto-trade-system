...
class TestEnterpriseDashboard:
    ...
    def dashboard(self, tmp_path):
        ...
        Regression: this used to construct EnterpriseDashboard() with no path
        overrides at all, so AuthHandler.__init__ (which runs schema
        migrations, and creates a default "admin" user with a known default
        password if the table is empty) ran against the REAL db/auth.db -
        see tests/test_all_ui_screens_and_navigation.py's dashboard_client
        fixture fix for the config.json equivalent of this same bug.
        """
        pytest.importorskip("fastapi")
    ...
    def test_dashboard_has_auth_routes(self, dashboard):
        routes = _route_paths(dashboard.app.routes)
...
class TestConfigManagement:
    ...
    def test_validate_config_ok(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard
        db = EnterpriseDashboard(config={
            "trader_state_path": str(tmp_path / "trader_state.json"),
            "auth_db_path": str(tmp_path / "auth.db"),
    ...
    def test_validate_config_env_ref(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard
        db = EnterpriseDashboard(config={
            "trader_state_path": str(tmp_path / "trader_state.json"),
            "auth_db_path": str(tmp_path / "auth.db"),
