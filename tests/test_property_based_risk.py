...
def _clear_hard_halt():
    """Clear the process-wide hard halt around every test in this file.

    Risk-limit invariants here exercise ``check_daily_loss()`` with extreme
    losses (e.g. daily_pnl=-100_000 vs max_daily_loss=-5000), which calls
    ``trip_hard_halt()`` on the global ``_HARD_HALT`` event. If left set,
    every later test that executes orders (execution service, paper trading)
    is silently REJECTED with "Hard halt active" — an order-dependence that
    only surfaces when this file runs before those tests. Clearing directly
    (bypassing the 60s clear cooldown) keeps the suite order-independent.
    """
    import core.safety_state as _ss

    _ss._HARD_HALT.clear()
    _ss._hard_halt_reason = ""
    yield
    _ss._HARD_HALT.clear()
    _ss._hard_halt_reason = ""
...
# Some tests construct a full RiskService per example (~seconds each under
# load), so 100 examples would take many minutes. Cap examples while keeping
# the deadline disabled — this is what previously tripped
# DeadlineExceeded (e.g. test_volatility_multiplier_monotonic_decreasing).
_SLOW_OK_SETTINGS = settings(deadline=None, max_examples=15)
