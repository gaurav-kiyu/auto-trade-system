...
is raised when the library is absent. The adapter gates construction on a
module-level ``KITE_AVAILABLE`` flag, so the constructor tests force that flag
False via monkeypatch - this is deterministic on every runner (kiteconnect is a
hard dependency in pyproject.toml and IS installed on CI), while the stateless
helper functions are tested unconditionally.
"""
...
class TestKiteBrokerAdapterConstructor:
    ...
    def force_kite_unavailable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Simulate kiteconnect being absent regardless of the environment.

        kiteconnect is a declared dependency, so CI runners install it and
        ``KITE_AVAILABLE`` would be True there (the old tests only passed on
        machines where it happened not to be installed).
        """
        monkeypatch.setattr(
            "infrastructure.adapters.brokers.kite.adapter.KITE_AVAILABLE", False
        )
...
class TestCreateKiteAdapterFromContext:
    ...
    @pytest.fixture(autouse=True)
    def force_kite_unavailable(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Same as the constructor class: kiteconnect is installed on CI, so
        force the module flag False to make the ImportError expectations
...
class MockKiteConnect:
    ...
    def instruments(self, exchange: str) -> list[dict]:
        if exchange == "NFO":
            return [
                {"tradingsymbol": "NIFTY24DEC23500CE", "exchange": "NFO", "instrument_token": 55555},
            ]
        return [
        ...
        return [
            {
                ...
                "average_price": 125.5, "filled_quantity": 75,
            },
            {
...
class TestKiteBrokerAdapterMocked:
    ...
    # ── Get fill price / filled quantity ─────────────────────────────────
    # Regression coverage: BrokerAdapter (the legacy wrapper this class is
    # always constructed inside of) only delegates get_fill_price/
    # get_filled_quantity to the wrapped port if the port implements them -
    # without these, a live Kite order would always report 0.0/0 through
    # the wrapper, causing every exit to record zero P&L.

    def test_get_fill_price_returns_real_average_price(self):
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter.get_fill_price("ORDER123") == 125.5

    def test_get_fill_price_none_when_zero_or_unfilled(self):
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter.get_fill_price("ORDER456") is None
        assert adapter.get_fill_price("UNKNOWN_ID") is None

    def test_get_fill_price_none_when_not_connected(self):
        adapter = self._make_adapter()
        assert adapter.get_fill_price("ORDER123") is None

    def test_get_filled_quantity_returns_real_quantity(self):
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter.get_filled_quantity("ORDER123") == 75

    def test_get_filled_quantity_zero_when_unfilled(self):
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter.get_filled_quantity("ORDER456") == 0

    def test_get_filled_quantity_none_when_unknown_order(self):
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter.get_filled_quantity("UNKNOWN_ID") is None
    ...
    def test_get_quote_defaults_to_zero_oi_for_non_option(self):
        adapter = self._make_adapter()
        adapter.connect()
        quote = adapter.get_quote("NIFTY")
        assert quote.oi == 0

    def test_get_quote_nfo_option_returns_real_oi(self):
        """config-relevant regression: an NFO option lookup must resolve via
        the NFO instrument dump, not silently fail because an earlier NSE
        lookup already populated a (formerly single, now per-exchange)
        instrument cache."""
        adapter = self._make_adapter()
        adapter.connect()
        adapter.get_quote("NIFTY")  # populate the NSE cache first
        quote = adapter.get_quote("NIFTY24DEC23500CE", exchange="NFO")
        assert quote.symbol == "NIFTY24DEC23500CE"
        assert quote.bid == 150.5
        assert quote.ask == 151.0
        assert quote.oi == 450000

    def test_get_quote_nfo_unresolvable_symbol_raises(self):
        adapter = self._make_adapter()
        adapter.connect()
        with pytest.raises(RuntimeError, match="Cannot resolve instrument token"):
            adapter.get_quote("NIFTY24DEC99999CE", exchange="NFO")

    def test_instruments_cache_is_per_exchange(self):
        """Regression: a single flat instruments cache built from one
        exchange's dump used to make lookups on a DIFFERENT exchange return
        None within the cache TTL, even though the symbol genuinely exists
        on that other exchange."""
        adapter = self._make_adapter()
        adapter.connect()
        assert adapter._get_instrument_token("NIFTY", exchange="NSE") == 12345
        # Immediately after, an NFO lookup (well within the NSE cache's TTL)
        # must still resolve correctly.
        assert adapter._get_instrument_token("NIFTY24DEC23500CE", exchange="NFO") == 55555
