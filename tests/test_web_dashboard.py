...
class TestMaybeStartDashboard:
    ...
    def test_webhook_enabled_returns_queued(self, tmp_path):
        ...
        if result is not None:
            ...
            assert resp.status_code == 200
            data = resp.json()
            # Endpoint actively routes via the multi-asset dispatcher when available
            # (status="routed"), otherwise falls back to advisory queuing ("queued").
            assert data.get("status") in ("queued", "routed")
            assert "ts" in data
