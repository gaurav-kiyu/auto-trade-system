...
def _make_valid_ohlcv(rows: int = 40):
    """Minimal valid OHLCV fixture for evaluator-path unit tests."""
    import pandas as pd
...
class TestSignalEvaluator:
    ...
    def test_evaluate_with_mocked_success(self, evaluator: SignalEvaluator):
        ...
        with patch("core.iv_rank.get_iv_rank", return_value=0.5), \
            ...
            result, reason = evaluator.evaluate(
                name="NIFTY",
                frames={
                    "df1m": _make_valid_ohlcv(40),
                    "df5m": _make_valid_ohlcv(40),
                    "df15m": _make_valid_ohlcv(40),
                },
                vix=15.0,
            )
    ...
    def test_evaluate_calls_get_iv_rank_with_vix_not_name(self, evaluator: SignalEvaluator):
        """Regression: get_iv_rank() must receive the numeric VIX level, not
        the index symbol string. Passing the symbol always raised inside
        get_iv_rank's own try/except and silently returned -1.0, permanently
        disabling the iv_spike hard block.
        """
        mock_signal = AdaptiveSignal(
            tier="MODERATE", score=72, raw_score=75,
            confidence=0.7, direction="PUT",
        ...
        with patch("core.iv_rank.get_iv_rank", return_value=0.5) as mock_iv_rank, \
             patch("core.oi_snapshot_store.get_pcr_at", return_value=1.0), \
             patch("core.oi_snapshot_store.get_oi_at", return_value=0.0), \
             patch("index_app.domains.signal.evaluator._eval_v2", return_value=(mock_signal, "")):
        ...
        assert mock_iv_rank.call_args[0][0] == 42.5
        assert mock_iv_rank.call_args[0][0] != "NIFTY"
