...
def _check_skip(result: subprocess.CompletedProcess) -> bool:
    """Check subprocess stdout for SKIP prefix and call pytest.skip if found.
    Returns True if the test was skipped, False otherwise."""
    if "SKIP:" in result.stdout:
        pytest.skip(result.stdout.split("SKIP:")[1].strip())
        return True
    return False
...
class TestSignalStaleness:
    ...
    def test_stale_signal_blocked_in_enter_trade(self):
        ...
        code = _preamble() + """
# Override execution mode to MANUAL so PositionService returns early (no broker needed)
mod.MANUAL_SIGNALS_ONLY = True
mod.EXECUTION_MODE = "MANUAL"
...
# Bypass auction session gate - PositionService.enter_trade imports this inside the method
import core.datetime_ist
core.datetime_ist.is_in_auction_session = lambda now=None: False
...
# Disable expiry controls so the tests aren't blocked by time-of-day gates
mod._expiry_controller._enable_controls = False
# Bypass auction session gate - PositionService.enter_trade imports inside the method
import core.datetime_ist
core.datetime_ist.is_in_auction_session = lambda now=None: False
...
"""
    ...
    def test_stale_signal_age_matches_config_threshold(self):
        """SIGNAL_MAX_AGE loaded from config.json is what the staleness gate uses."""
        # Expected value is read live from config.json so this test never drifts
        # from the actual shipped threshold (historically 65, now 90).
        import json
        _expected_age = int(json.loads(Path(str(CONFIG)).read_text(encoding="utf-8")).get("SIGNAL_MAX_AGE", 90))
        code = _preamble() + """
# Override execution mode to MANUAL so PositionService returns early (no broker needed)
mod.MANUAL_SIGNALS_ONLY = True
mod.EXECUTION_MODE = "MANUAL"
...
# Verify SIGNAL_MAX_AGE loaded from config.json matches the shipped threshold
...
"""
    ...
    def test_stale_signal_does_not_proceed_to_order_placement(self):
        ...
        code = _preamble() + """
if not mod.MANUAL_SIGNALS_ONLY:
    print("SKIP: Test requires MANUAL mode")
    exit(0)
...
class TestZombiePnLConfirmation:
    ...
    def test_check_pending_reconciliation_emits_warning(self):
        ...
        code = _preamble() + """
if mod.PAPER_MODE:
    print("SKIP: Test requires non-paper mode")
    exit(0)
...
if mod.BROKER_API_ENABLED is not False or mod.MANUAL_SIGNALS_ONLY is not True:
    print("SKIP: Test requires MANUAL mode (BROKER_API_ENABLED=False, MANUAL_SIGNALS_ONLY=True)")
    exit(0)
