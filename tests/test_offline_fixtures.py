...
def _isolated_market_data_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    """Isolate DataEngine tests from the process-global market-data cache.

    ``get_market_data_cache()`` returns a module-level singleton shared with the
    running bot and every other test. ``DataEngine.fetch_all_frames`` checks this
    cache BEFORE calling the injected fetch function, so real frames cached by
    another test (or the live bot) short-circuited the fixture lambda on CI
    (observed: ``assert {'NIFTY': [1, 2, 3]} == {'NIFTY': {'5m': ...}}``).
    Give each test its own in-memory cache instance instead; the within-test
    coupling (first engine's ``put`` -> second engine's ``get``) is preserved.
    """
    from infrastructure.market_data.market_data_cache import MarketDataCache

    fresh_cache = MarketDataCache()
    monkeypatch.setattr(
        "core.data_engine.get_market_data_cache", lambda: fresh_cache
    )


@pytest.fixture(autouse=True)
def _restore_opbuying_index_config_env():
    """Regression guard: _load_index_module() below sets OPBUYING_INDEX_CONFIG
    to the REAL project's json/config.json via a raw os.environ assignment
    (not monkeypatch), with no cleanup of its own. Left unrestored, this env
    var leaks for the rest of the pytest process - any LATER test across ANY
    file that constructs EnterpriseDashboard(index_config_path=<isolated tmp
    path>) would have that override silently ignored (env var wins in
    _resolve_config_path()), writing config-apply test data straight into the
    real json/config.json (observed twice in one session: real
    CATEGORY_SCORE_THRESHOLDS/INDEX_MIN_SCORE/MIN_SCORE_THRESHOLD entries got
    clobbered by leaked KEY_1..KEY_4 test fixture keys).
    """
    prev = os.environ.get("OPBUYING_INDEX_CONFIG")
    yield
    if prev is None:
        os.environ.pop("OPBUYING_INDEX_CONFIG", None)
    else:
        os.environ["OPBUYING_INDEX_CONFIG"] = prev


def _load_index_module(tag: str):
    os.environ["OPBUYING_INDEX_CONFIG"] = str(ROOT / "json" / "config.json")
    argv_prev = sys.argv[:]
