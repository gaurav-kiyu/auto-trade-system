...
class TestConfigVersion:
    ...
    def test_config_version_is_non_empty_string(self):
        """CONFIG_VERSION follows the config schema (string, e.g. '2.53.0')."""
        cfg_path = ROOT / "json" / "config.json"
        with cfg_path.open(encoding="utf-8") as f:
            cfg = json.load(f)
        ver = cfg["CONFIG_VERSION"]
        assert isinstance(ver, str) and ver.strip(), (
            f"CONFIG_VERSION must be a non-empty string, got {ver!r}"
        )
