...
class TestConfigEdgeCases:
    ...
    def test_apply_env_overrides_no_match(self):
        ...
        Isolated against any OPBUYING_* var some other test in the same
        full-suite run left behind without cleanup - this test's own
        assertion only cares about "KEY", so any leaked OPBUYING_KEY is
        removed for the duration of this test rather than chased down.
        """
        import os
        ...
        leaked = os.environ.pop("OPBUYING_KEY", None)
        try:
            ...
            if leaked is not None:
                os.environ["OPBUYING_KEY"] = leaked
