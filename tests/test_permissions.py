...
class TestPermissionMatrix:
    ...
    def test_admin_has_all_permissions(self):
        for perm in Permission:
            if perm is Permission.MANAGE_PERMISSIONS:
                assert not role_has_permission(Role.ADMIN, perm)
            else:
...
class TestGetRolePermissions:
    def test_admin_has_10_operational_permissions(self):
        perms = get_role_permissions(Role.ADMIN)
        assert len(perms) == 10
        assert Permission.MANAGE_USERS in perms
        assert Permission.MANAGE_PERMISSIONS not in perms
    ...
    def test_modifying_returned_set_does_not_affect_matrix(self):
        ...
        assert len(get_role_permissions(Role.ADMIN)) == 10
