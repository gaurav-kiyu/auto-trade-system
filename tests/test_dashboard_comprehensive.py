...
class TestRiskConcentration:
    ...
    def test_risk_concentration_critical(self, dashboard, client):
        with patch.object(
            ...
            "_load_recent_trades",
            return_value=[
                {"status": "open", "exit_time": None, "index": "NIFTY", "pnl": 0, "entry_price": 600, "quantity": 500},
            ],
        ):
            with patch.object(
                dashboard,
    ...
    def test_risk_concentration_high(self, dashboard, client):
        with patch.object(
            ...
            "_load_recent_trades",
            return_value=[
                {"status": "open", "exit_time": None, "index": "NIFTY", "pnl": 0, "entry_price": 200, "quantity": 10},
            ],
        ):
            with patch.object(
                dashboard,
    ...
    def test_risk_concentration_moderate(self, dashboard, client):
        with patch.object(
            ...
            "_load_recent_trades",
            return_value=[
                {"status": "open", "exit_time": None, "index": "NIFTY", "pnl": 0, "entry_price": 100, "quantity": 10},
            ],
        ):
            with patch.object(
                dashboard,
...
class TestWebhookSignalInjection:
    ...
    def test_webhook_enabled_queues(self, base_cfg, trades_db):
        ...
        c = TestClient(db.app)
        # A symbol is required: without one the dispatcher router replies
        # {"status": "skipped", "reason": "no_symbol"} (added in v2.54 routing).
        resp = c.post("/signals/inject", json={"symbol": "NIFTY", "direction": "CALL", "score": 85})
        assert resp.status_code == 200
        data = resp.json()
        # Since v2.54 the multi-asset dispatcher may actively route the signal
        # ("routed") or fall back to advisory queuing ("queued") when the DI
        # container has no dispatcher.
        assert data["status"] in ("queued", "routed")
    ...
    def test_webhook_rate_limited(self, base_cfg, trades_db):
        ...
        c = TestClient(db.app)
        resp = c.post("/signals/inject", json={"symbol": "NIFTY", "direction": "CALL"})
        assert resp.status_code == 200
    ...
    def test_webhook_json_decode_error(self, base_cfg, trades_db):
        ...
        db = EnterpriseDashboard(config=cfg, db_path=trades_db)
        c = TestClient(db.app)
        resp = c.post("/signals/inject", content=b"not json", headers={"Content-Type": "application/json"})
        assert resp.status_code == 200
    ...
    def test_webhook_rate_limiter_error(self, base_cfg, trades_db):
        ...
        resp = c.post("/signals/inject", json={"symbol": "NIFTY", "direction": "CALL"})
        assert resp.status_code == 200
        assert resp.json()["status"] in ("queued", "routed")
...
class TestStaticFileServing:
    ...
    def test_static_dir_creation_failure(self, monkeypatch, tmp_path):
        ...
        def bad_static_mkdir(self, *a, **kw):
            # Fail ONLY the dashboard static dir; other mkdir calls must succeed.
            if str(self).replace("\\", "/").endswith("/static"):
                raise OSError("Cannot create dir")
            return real_mkdir(self, *a, **kw)

        def static_is_dir(self, *a, **kw):
            # Hide any pre-existing repo static/ dir so the create-failure
            # branch is actually exercised.
            if str(self).replace("\\", "/").endswith("/static"):
                return False
            return real_is_dir(self, *a, **kw)

        monkeypatch.setattr(pathlib.Path, "mkdir", bad_static_mkdir)
        monkeypatch.setattr(pathlib.Path, "is_dir", static_is_dir)
...
class TestEdgeCases:
    ...
    def test_read_state_missing(self, tmp_path):
        ...
        assert state.get("hard_halt") is False
