...
class TestMarketStatus:
    ...
    @patch("core.datetime_ist.now_ist")
    def test_get_market_status_exact_open_boundary(self, mock_now):
        """At exactly 09:15, market is OPEN (not PRE_MARKET) - kills Lt->LtE mutant."""
        mock_now.return_value = datetime.datetime(2026, 1, 5, 9, 15, 0)  # exactly at NSE_OPEN
        status = get_market_status({})
        # Original: current_time < _NSE_OPEN -> False at 09:15 -> OPEN
        # Mutant: current_time <= _NSE_OPEN -> True at 09:15 -> PRE_MARKET
        assert status == MarketStatus.OPEN
...
class TestCorporateAction:
    ...
    def test_is_corp_action_day_and_survivor_killed(self):
        """Kills the and->or mutant: different symbol but matching date should be False.

        Original: action.symbol == sym_upper and action.date == check_date
                 -> False and True -> False -> not found (correct)
        Mutant:   action.symbol == sym_upper or action.date == check_date
                 -> False or True -> True -> found (wrong)
        """
        cfg = {
            "corp_action_calendar_enabled": True,
            "corp_action_data": [
                {"symbol": "ICICIBANK", "date": "2026-05-15", "type": "DIVIDEND", "factor": 10.0},
            ],
        }
        # Query for HDFCBANK on same date as ICICIBANK action - should NOT match
        found, desc = is_corp_action_day("HDFCBANK", datetime.date(2026, 5, 15), cfg)
        assert found is False
        assert desc == ""
