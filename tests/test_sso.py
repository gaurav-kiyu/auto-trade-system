...
class TestSSOAvailability:
    ...
    def test_is_available_true_when_authlib_installed(self):
        """is_available should be True when authlib is installed."""
        sso = SSOAuthenticator()
        assert sso.is_available is True

    def test_get_authorization_url_returns_url_when_authlib_installed(self):
        """get_authorization_url should return a URL when authlib is installed."""
        ...
        assert "client_id=test-id" in result
...
class TestSSOReady:
    ...
    def test_is_ready_google_configured(self):
        """is_ready with full Google config and authlib installed should be ready."""
        cfg = SSOConfig(
            provider="google",
            client_id="id",
        ...
        sso = SSOAuthenticator(config=cfg)
        ready, issues = sso.is_ready()
        assert ready is True  # authlib is installed + full config
