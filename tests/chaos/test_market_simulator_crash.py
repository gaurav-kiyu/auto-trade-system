...
class TestMarketSimulatorCrash:
    ...
    def test_simulator_handles_corrupted_config(self):
        ...
        # inject_failures=False keeps this deterministic: even with
        # rejection_probability=0 the unseeded exchange-failure roll (~3.5%
        # combined circuit-breaker/gap/failure) could otherwise randomly return
        # TIMEOUT/FAILED, flaking this chaos test on some CI runs.
        result = sim.simulate_order(
            order_id="CLEAN", quantity=1, price=100.0, inject_failures=False
        )
        assert result.status == "ACCEPTED"
