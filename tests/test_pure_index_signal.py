...
class TestValidateFrameAlignment:
    ...
    def test_one_bar_period_lag_passes_with_widened_tolerances(self):
        """Live yfinance 5m/15m bars are completed bars that lag the 1m frame
        by up to one full bar period (300s/900s). The defaults must accept
        this realistic lag; the old 120s/300s tolerances rejected it."""
        import datetime as _dt

        base = _dt.datetime(2026, 8, 7, 11, 43, tzinfo=_dt.timezone.utc)
        idx1 = [base - _dt.timedelta(minutes=i) for i in range(30)][::-1]
        df1 = make_df([100.0] * 30)
        df1.index = idx1

        # 5m last bar 3 minutes behind 1m; 15m last bar 13 minutes behind
        df5 = make_df([100.0] * 10)
        df5.index = [base - _dt.timedelta(minutes=3 + 5 * i) for i in range(10)][::-1]
        df15 = make_df([100.0] * 5)
        df15.index = [base - _dt.timedelta(minutes=13 + 15 * i) for i in range(5)][::-1]

        # Default one-bar-period tolerances (360s / 960s) accept the lag
        assert _validate_frame_alignment(df1, df5, df15, 360, 960) is True
        # The old hardcoded tolerances reject it (this was the live bug)
        assert _validate_frame_alignment(df1, df5, df15, 120, 300) is False

    def test_top_level_override_wins_over_nested_block(self):
        """The simulation engine disables the check via top-level
        ``FRAME_ALIGN_1M_5M = 99999``; that explicit override must take
        precedence over the nested ``indicator`` block values."""
        import datetime as _dt
        ...
        params = PureIndexSignalParams(
            name="NIFTY",
            signal_cfg={
                "indicator": {"frame_align_1m_5m_secs": 360, "frame_align_1m_15m_secs": 960},
        ...
        # With the 99999 override the alignment check is effectively disabled,
        # so evaluation proceeds past the frame_align gate (may still block on
        # other structural gates, but never on frame_align).
        payload, reason = evaluate_index_signal_partial(
            params=params,
            df1=df1, df5=df5, df15=df15,
            vix=12.0, iv=0.0, oi_sup=0.0, oi_res=0.0,
            pcr=1.0, smart="NEUTRAL",
        )
        assert reason != "frame_align"
