...
def test_readiness_ml_accuracy_uses_actual_rate_not_missing_calibrated_key(monkeypatch):
    """Regression: compute_calibration()'s bin dicts never carried a "calibrated"
    key, so the old `b.get("calibrated", False)` check made ML accuracy always 0%
    whenever any predictions existed. It should instead compare actual_rate to
    predicted_mean within ml_calibration_tolerance."""
    import core.ml_performance_tracker as ml_tracker

    def fake_compute_calibration(*, n_bins=5, db_path="db/ml_tracker.db"):
        return [
            {"bin_low": 0.5, "bin_mid": 0.55, "predicted_mean": 0.55, "actual_rate": 0.60, "count": 40},
            {"bin_low": 0.0, "bin_mid": 0.05, "predicted_mean": 0.05, "actual_rate": 0.90, "count": 10},
        ]

    monkeypatch.setattr(ml_tracker, "compute_calibration", fake_compute_calibration)

    db = make_db(n_paper=60, win_frac=0.65, n_days=15)
    try:
        report = check_live_readiness(db, CFG_OK)
        ml_crit = next(c for c in report.warning_criteria if c.name == "ML accuracy")
        # Only the first bin (|0.60-0.55|=0.05 <= tolerance 0.15) counts as
        # calibrated; the second (|0.90-0.05|=0.85) does not.
        assert ml_crit.actual == pytest.approx(round(40 / 50 * 100, 1))
    finally:
        os.unlink(db)
