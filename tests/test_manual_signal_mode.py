...
def _get_manual_config_path() -> str:
    """Return a config file whose EXECUTION_MODE is MANUAL (deterministic).

    Copies the repo config.json (falling back to config.template.json when
    config.json is absent, e.g. CI) and forces EXECUTION_MODE=MANUAL. The
    MANUAL-gate subprocess tests therefore behave identically on every
    machine regardless of the local runtime config.
    """
    global _TEMP_CONFIG_PATH
    if _TEMP_CONFIG_PATH:
        return _TEMP_CONFIG_PATH
    src = CONFIG if CONFIG.is_file() else ROOT / "json" / "config.template.json"
    cfg = json.loads(src.read_text(encoding="utf-8"))
    cfg["EXECUTION_MODE"] = "MANUAL"
    cfg["MANUAL_SIGNALS_ONLY"] = True
    # Write INSIDE the project root: the ConfigLoader path-safety guard rejects
    # config files that resolve outside the root (falls back to safe defaults),
    # which would silently make these subprocess tests exercise DEFAULT_SAFE_CONFIG
    # instead of the intended MANUAL config.
    _cfg_dir = ROOT / ".tmp"
    _cfg_dir.mkdir(parents=True, exist_ok=True)
    fd, path = tempfile.mkstemp(
        suffix=".json", prefix="opb_manual_cfg_", dir=str(_cfg_dir),
    )
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        json.dump(cfg, fh, indent=2)
    _TEMP_CONFIG_PATH = path
    return path
...
def _load_mod_code() -> str:
    ...
    return (
        ...
        f"os.environ['OPBUYING_INDEX_CONFIG'] = r'{_get_manual_config_path()}'\n"
...
class TestDashboardBrokerModeManual:
    ...
    def test_auto_mode_label_names_backend(self):
        label = self._pe().dashboard_broker_mode(execution_mode="AUTO", broker_backend="KITE", broker_api_enabled=True)
...
# PositionService.enter_trade imports core.datetime_ist.is_in_auction_session
# LOCALLY inside the method, so patch the module function (not mod.*) to make
# the golden path immune to NSE auction windows (09:00-09:15 / 15:30-15:45 IST).
import core.datetime_ist as _cdt_ist
_cdt_ist.is_in_auction_session = lambda: False
