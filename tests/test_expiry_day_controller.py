...
class TestExpiryDayController:
    ...
    def test_custom_block_after_overrides_default_cutoff(self):
        """EXPIRY_CUTOFF_HOUR wiring: a custom block_after must actually move
        the cutoff, not just default to the hardcoded 13:00."""
        thursday_1215 = datetime(2026, 6, 11, 12, 15)  # Thursday, before default 13:00 cutoff
        default_ctrl = ExpiryDayController()
        assert default_ctrl.can_enter_position(now=thursday_1215, index_name="NIFTY").allowed is True

        strict_ctrl = ExpiryDayController(block_after=time(12, 0))  # e.g. EXPIRY_CUTOFF_HOUR=12
        result = strict_ctrl.can_enter_position(now=thursday_1215, index_name="NIFTY")
