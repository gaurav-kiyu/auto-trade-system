...
class TestCanonicalModulesImportable:
    ...
    def test_module_exists_on_disk_true_for_real_modules(self) -> None:
        """Dotted paths that resolve to files under ROOT must be found
        without importing anything."""
        from scripts.check_architecture_compliance import (
            _module_exists_on_disk,
        )
        assert _module_exists_on_disk("core.services.risk_service") is True
        assert _module_exists_on_disk("core.datetime_ist") is True
        assert _module_exists_on_disk("core.oi_snapshot_store") is True

    def test_module_exists_on_disk_false_for_missing(self) -> None:
        from scripts.check_architecture_compliance import (
            _module_exists_on_disk,
        )
        assert _module_exists_on_disk("core.does_not_exist_zzz") is False

    def test_module_exists_on_disk_package_init(self, temp_project: Path) -> None:
        """Package-style modules (dir/__init__.py) must also resolve."""
        from unittest.mock import patch

        from scripts.check_architecture_compliance import (
            _module_exists_on_disk,
        )
        pkg = temp_project / "core" / "somepkg"
        pkg.mkdir(parents=True)
        (pkg / "__init__.py").write_text("")
        with patch("scripts.check_architecture_compliance.ROOT", temp_project):
            assert _module_exists_on_disk("core.somepkg") is True
