...
def pg_db() -> Generator[DatabasePort, None, None]:
    ...
    try:
        ...
        adapter.commit()
    except Exception:  # Test teardown - catch all to ensure cleanup never fails
        pass
    adapter.disconnect()
...
class TestDsnParsing:
    ...
    def test_full_dsn(self) -> None:
        ...
        params = parse_pg_dsn("postgresql://user:pass@host1:5432/mydb")
    ...
    def test_dsn_no_password(self) -> None:
        from core.adapters.database.dsn_utils import parse_pg_dsn
        params = parse_pg_dsn("postgresql://user@host/mydb")
    ...
    def test_dsn_traditional_format(self) -> None:
        from core.adapters.database.dsn_utils import parse_pg_dsn
        params = parse_pg_dsn("localhost:5432:mydb:user:pass")
        assert params["host"] == "localhost"
        assert params["port"] == 5432
        assert params["dbname"] == "mydb"
        assert params["user"] == "user"
    ...
    def test_dsn_url_no_prefix(self) -> None:
        from core.adapters.database.dsn_utils import parse_pg_dsn
        params = parse_pg_dsn("user:pass@host:5432/mydb")
        assert params["host"] == "host"
        assert params["user"] == "user"
        assert params["password"] == "pass"
        assert params["dbname"] == "mydb"
