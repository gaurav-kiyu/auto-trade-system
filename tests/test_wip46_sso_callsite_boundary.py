"""WIP46 SSO call-site boundary tests."""
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]


def test_sso_has_canonical_url_support():
    text = (ROOT / "core/auth/sso.py").read_text(encoding="utf-8")
    assert ("build_action_url" in text) or ("get_public_base_url" in text)


def test_routes_use_canonical_callback():
...
def test_resolver_has_deployment_and_admin_layers():
