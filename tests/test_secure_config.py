...
class TestSecureConfigInit:
    def test_empty_init(self):
        ...
        # Isolate from real environment secrets that leak into SecureConfig
        # via _credential_storage (which takes a snapshot of os.environ at import time)
        from infrastructure.config.secure_config import _credential_storage
        with patch.object(_credential_storage, "get_credential", return_value=None):
            sc = SecureConfig()
