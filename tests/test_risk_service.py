...
class TestEvaluateTrade:
    ...
    def test_all_checks_pass(self):
        ...
        # recommended_position_size is advisory only here (the real live entry
        # path sizes via mandate_service/mandate_enforcer, not this field —
        # see core/position_service.py:309). With price=23500/stop_loss=23000
        # (a 500-point diff) and lot_size=50, one lot's SL-loss (25000) exceeds
        # the 5% max_risk_per_trade cap on 100000 capital, so 0 is the correct,
        # capital-safe recommendation — not a reason to deny the trade itself.
...
class TestCalculatePositionSize:
    ...
    def test_basic_sizing(self):
        ...
        # this to 0: max_risk_amount = 100000*0.05=5000, one lot's SL-loss is
        # 500*50=25000 (25% of capital) >> the 5% cap. A prior formula bug
        # divided the cap's denominator by risk_per_trade again, inflating it
        # ~50x and silently disabling this backstop for scenarios like this.
        assert result == 0

    def test_zero_volatility_uses_low_multiplier(self):
        """Low volatility → higher multiplier (1.2), still capped by capital (see test_basic_sizing)."""
        service = RiskService()
...
class TestHelperMethods:
    ...
    def test_get_lot_size_prefers_index_map_over_hardcoded_fallback(self):
        """Regression: lot size must come from config (INDEX_MAP/instruments),
        not the hardcoded dict, whenever the caller supplies an index_map -
        otherwise an NSE lot-size revision would silently never take effect."""
        service = RiskService(index_map={"NIFTY": {"lot": 75}})
        assert service._get_lot_size("NIFTY") == 75
        assert service._get_lot_size("nifty") == 75  # case-insensitive lookup

    def test_get_lot_size_falls_back_when_symbol_missing_from_index_map(self):
        service = RiskService(index_map={"NIFTY": {"lot": 75}})
        assert service._get_lot_size("BANKNIFTY") == 15  # hardcoded fallback
