...
class TestGetIntradayMonitor:
    def setup_method(self):
        from core.intraday_performance_monitor import reset_intraday_monitor
    ...
    def test_returns_same_instance_across_calls(self):
        ...
        m1 = get_intraday_monitor({"intraday_monitor_enabled": True})
        m2 = get_intraday_monitor()
        assert m1 is m2

    def test_state_persists_across_calls(self):
        from core.intraday_performance_monitor import get_intraday_monitor
        get_intraday_monitor({"intraday_monitor_enabled": True}).record_trade_close(-50.0, was_winner=False)
        stats = get_intraday_monitor().get_stats()
        assert stats.trades_today == 1

    def test_new_cfg_updates_existing_instance(self):
        from core.intraday_performance_monitor import get_intraday_monitor
        m1 = get_intraday_monitor({"intraday_monitor_enabled": True})
        m2 = get_intraday_monitor({"intraday_monitor_enabled": False, "intraday_min_trades_to_adapt": 99})
        assert m1 is m2
        assert m1._cfg.get("intraday_min_trades_to_adapt") == 99

    def test_reset_intraday_monitor_drops_singleton(self):
        from core.intraday_performance_monitor import get_intraday_monitor, reset_intraday_monitor
        m1 = get_intraday_monitor({"intraday_monitor_enabled": True})
        m1.record_trade_close(-50.0, was_winner=False)
        reset_intraday_monitor()
        m2 = get_intraday_monitor({"intraday_monitor_enabled": True})
        assert m2 is not m1
        assert m2.get_stats().trades_today == 0


# =============================================================================
# Day-rollover self-reset Tests
# =============================================================================

class TestDayRollover:
    def test_new_calendar_day_resets_state(self, monitor: IntradayPerformanceMonitor, monkeypatch):
        ...
        monitor.record_trade_close(-50.0, was_winner=False)
        monitor.record_trade_close(-50.0, was_winner=False)
        assert monitor.get_stats().trades_today == 2

        tomorrow = datetime_ist.now_ist() + _dt.timedelta(days=1)
        monkeypatch.setattr(datetime_ist, "now_ist", lambda: tomorrow)
        stats = monitor.get_stats()
        assert stats.trades_today == 0
        assert stats.consecutive_losses == 0

    def test_same_day_does_not_reset(self, monitor: IntradayPerformanceMonitor):
        monitor.record_trade_close(-50.0, was_winner=False)
        monitor.get_current_params()
        monitor.get_stats()
        assert monitor.get_stats().trades_today == 1
