...
_PATCH_TARGET = "core.capital_manager.trip_hard_halt"
...
class TestScale:
    ...
    def test_drawdown_scale_disabled_keeps_full_size(self):
        cm = CapitalManager(
            initial_capital=100000.0,
            max_daily_loss=-2000.0,
            max_drawdown_pct=0.20,
            drawdown_scale_enabled=False,
        )
        cm.record_trade(-30000.0, is_winner=False)  # current = 70000, dd = 30% (would hard-block if enabled)
        result = cm.scale(base_lots=2, max_lots=2)
        assert result.drawdown_factor == 1.0
        assert result.drawdown_pct > 0  # still reported, just not applied to sizing

    def test_drawdown_scale_enabled_by_default(self):
        cm = CapitalManager(initial_capital=100000.0, max_daily_loss=-2000.0, max_drawdown_pct=0.20)
        cm.record_trade(-30000.0, is_winner=False)
        result = cm.scale(base_lots=2, max_lots=2)
        assert result.drawdown_factor < 1.0
