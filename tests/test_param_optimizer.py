...
def test_no_db_returns_none(tmp_path):
    # tmp_path, not a bare relative filename - sqlite3.connect() creates an
    # empty file for a "nonexistent" path as a side effect, and a relative
    # path here would leak that file into the repo root instead of pytest's
    # auto-cleaned tmp dir.
    result = optimize_param(
        "SL_PCT", [0.2, 0.3],
        db_path=str(tmp_path / "nonexistent_opt_xyz.db"),
