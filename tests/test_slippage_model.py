...
def test_calibrate_no_db_returns_none(tmp_path):
    # tmp_path, not a bare relative filename - see test_param_optimizer.py's
    # equivalent test for why (sqlite3.connect() creates the file as a side
    # effect even for a "nonexistent" path).
    result = calibrate_model(
        db_path=str(tmp_path / "nonexistent_slip_xyz.db"),
