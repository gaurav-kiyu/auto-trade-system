...
class TestFindProjectRoot:
    ...
    def test_has_pyproject_toml(self):
        root = find_project_root()
        assert (root / "pyproject.toml").exists() or (root / "json/index_config.defaults.json").exists()
...
class TestDiscoverDbFiles:
    ...
    def test_finds_db_files(self, tmp_path):
        """Directory with .db files returns them (under the db/ folder)."""
        db_dir = tmp_path / "db"
        db_dir.mkdir()
        (db_dir / "trades.db").touch()
        (db_dir / "trade_journal.db").touch()
...
class TestRunBackup:
    ...
    def test_backup_cycle(self, tmp_path):
        ...
        # Create test DB files (under the db/ folder)
        db_dir = tmp_path / "db"
        db_dir.mkdir()
        (db_dir / "trades.db").write_text("trades data")
        (db_dir / "journal.db").write_text("journal data")
    ...
    def test_dry_run(self, tmp_path):
        ...
        db_dir = tmp_path / "db"
        db_dir.mkdir()
        (db_dir / "trades.db").write_text("data")
