...
# VaR tests create a fresh SQLite DB per generated example (disk IO). Even a
# 10s per-example deadline flakes under full-suite CPU contention (the trading
# bot, CI runners) — the first call can take 100s+ while retries take ~10ms,
# which Hypothesis reports as DeadlineExceeded/FlakyFailure. Disable the
# deadline entirely for these IO-heavy tests (the property assertions, not the
# timing, are what's under test).
_VAR_DEADLINE_MS = None
...
@settings(deadline=_VAR_DEADLINE_MS, max_examples=50)
