...
def mock_vix_history():
    ...
    # Isolate from the on-disk cache: _get_history checks the file cache BEFORE
    # calling _fetch_vix_history, so a fresh json/iv_history_cache.json written by
    # another test (or the running bot) would bypass this mock. Patch the file
    # cache to miss and never persist, keeping tests deterministic on any runner.
    with patch("core.iv_rank._fetch_vix_history") as mock_fetch, patch(
        "core.iv_rank._load_file_cache", return_value={}
    ), patch("core.iv_rank._save_file_cache"):
        import random
        random.seed(42)
