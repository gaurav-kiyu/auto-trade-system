...
class TestCheckAllGuards:
    def test_all_pass(self, guards: ExecutionGuards) -> None:
        # Quote timestamps must be IST (now_ist): the stale-data guard compares
        # against now_ist(), and datetime.now() on a non-IST (UTC) CI runner is
        # ~5.5h behind, failing the 2s max-age check.
        ok, reason, details = guards.check_all_guards(
            "NIFTY", "CALL", 100.0, 100.5, now_ist(),
        )
        assert ok
        assert reason == ""
