...
class TestCheckDocumentationSync:
    ...
    def test_suffixed_test_names_not_flagged(self) -> None:
        """Modules covered by descriptive test names are not flagged.

        test_metrics_trend_routes.py covers metrics_trend.py and
        test_self_service_provisioning.py covers provisioning.py, so the
        checker must not report them as missing tests.
        """
        issues = sync_artifacts.check_documentation_sync()
        flagged = "\n".join(issues)
        assert "metrics_trend.py" not in flagged
        assert "provisioning.py" not in flagged


class TestTestCoversModule:
    """Tests for the _test_covers_module token-sequence matcher."""

    def test_exact_match(self) -> None:
        assert sync_artifacts._test_covers_module("test_risk_service", "risk_service")

    def test_suffix_match(self) -> None:
        assert sync_artifacts._test_covers_module("test_metrics_trend_routes", "metrics_trend")

    def test_prefix_match(self) -> None:
        assert sync_artifacts._test_covers_module("test_self_service_provisioning", "provisioning")

    def test_middle_match(self) -> None:
        assert sync_artifacts._test_covers_module("test_wal_journal_recovery", "journal")

    def test_unrelated_name_no_match(self) -> None:
        assert not sync_artifacts._test_covers_module("test_telegram_queue", "position_sizer")

    def test_empty_inputs_no_match(self) -> None:
        assert not sync_artifacts._test_covers_module("", "foo")
        assert not sync_artifacts._test_covers_module("test_foo", "")
