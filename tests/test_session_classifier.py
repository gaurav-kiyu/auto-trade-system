...
class TestClassifySession:
    ...
    def test_opening_915_to_1015(self):
        ...
        # At exactly 10:15: t < t_trending is False -> TRENDING (kills <-><= mutant)
        assert classify_session(self._t(10, 14), cfg) == SessionType.OPENING
        assert classify_session(self._t(10, 15), cfg) == SessionType.TRENDING
...
class TestGetExpirySession:
    ...
    def test_expiry_exact_nse_open_boundary(self):
        """Kill <= -> < mutant on first operator of nse_open <= current_time.
        At exactly 9:15: 9:15 <= 9:15 is True, 9:15 < 9:15 is False."""
        thursday = datetime.date(2026, 6, 25)
        result = get_expiry_session("NIFTY", self._t(9, 15), check_date=thursday)
        # At 9:15, first check nse_open <= 9:15 < morning_end -> True -> MORNING
        assert result is not None
        assert result.name == ExpirySessionName.EXPIRY_MORNING

    def test_expiry_exact_morning_end_boundary(self):
        """Kill <= < mutant at morning_end=11:00: at exactly 11:00, transitions to MIDDAY."""
        thursday = datetime.date(2026, 6, 25)
        # At exactly 11:00: nse_open <= 11:00 is True, 11:00 < morning_end(11:00) is False -> MIDDAY
        result = get_expiry_session("NIFTY", self._t(11, 0), check_date=thursday)
        assert result is not None
        assert result.name == ExpirySessionName.EXPIRY_MIDDAY

    def test_expiry_exact_caution_boundary(self):
        """Kill <= < mutant at caution_st=12:30."""
        thursday = datetime.date(2026, 6, 25)
        # At exactly 12:30: morning_end(11:00) <= 12:30 is True, 12:30 < caution(12:30) is False -> CAUTION
        result = get_expiry_session("NIFTY", self._t(12, 30), check_date=thursday)
        assert result is not None
        assert result.name == ExpirySessionName.EXPIRY_CAUTION

    def test_expiry_exact_block_boundary(self):
        """Kill >= > mutant at block_st=13:30: at exactly 13:30, transitions to BLOCKED."""
        thursday = datetime.date(2026, 6, 25)
        # At exactly 13:30: current_time >= block_st is True -> BLOCKED
        result = get_expiry_session("NIFTY", self._t(13, 30), check_date=thursday)
        assert result is not None
        assert result.name == ExpirySessionName.EXPIRY_BLOCKED
