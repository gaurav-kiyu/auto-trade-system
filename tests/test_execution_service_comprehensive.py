...
def _isolate_state_machine():
    """Reset the ExecutionStateMachineManager singleton around every test.

    ExecutionService wires itself to the module-level singleton manager, so
    machines created by one test would otherwise leak into the next test's
    ``run_ack_watchdog``/``_execute_with_retries`` assertions (e.g. leftover
    SUBMITTED machines making ``checked`` > 0). Resetting both before and
    after each test makes this file pass in any suite order.
    """
...
def mock_broker() -> MagicMock:
    ...
    broker.get_fill_price.return_value = 23500.0
    return broker
...
# ═══════════════════════════════════════════════════════════════════════
#  Paper-fill P&L reporting (real PaperBrokerAdapter, production DI shape)
#
#  In production, PositionService.enter_trade/exit_position call
#  execute_order(order_request) with NO execution_context, so
#  execution_mode defaults to AUTOMATIC — not PAPER. The broker_port is a
#  BrokerAdapter wrapping a PaperBrokerAdapter (BrokerFactory.create_broker()
#  always returns BrokerAdapter(PaperBrokerAdapter()) in paper mode), so
#  __class__.__name__ is "BrokerAdapter", not "PaperBrokerAdapter" either.
#  Both paper-mode-detection clauses in _attempt_order_execution therefore
#  miss, and execution falls through to the generic broker path — which is
#  the path this test exercises, using the real classes rather than a
#  MagicMock (a MagicMock would auto-vivify get_average_price and hide the
#  bug, which is exactly what happened before this fix).
# ═══════════════════════════════════════════════════════════════════════


class TestPaperFillPnLReporting:
    def _make_service(self, mid_price: float) -> ExecutionService:
        paper_adapter = PaperBrokerAdapter(price_getter=lambda name, direction, strike: mid_price)
        broker_port = BrokerAdapter(paper_adapter)
        config = ExecutionServiceConfig(
            enable_duplicate_prevention=True,
            idempotency_cache_size=100,
            idempotency_expiry_hours=24,
        ...
        svc.set_operating_mode_manager(None)
        return svc

    def test_fill_price_is_real_not_zero(self, order_request: OrderRequest):
        """Regression for the zero-P&L bug: average_price must reflect the
        real simulated fill price, not the old hardcoded-stub 0.0.
        """
        service = self._make_service(mid_price=23500.0)
        result = service.execute_order(order_request)  # no execution_context -> AUTOMATIC, as in production

        assert result.status == OrderStatus.FILLED
        assert result.average_price is not None
        assert result.average_price > 0.0
        # Entry fill = mid * (1 + slippage%); default paper_slippage_pct is 0.5
        assert result.average_price == pytest.approx(23500.0 * 1.005, rel=1e-3)

    def test_filled_quantity_delegates_to_paper_adapter(self, order_request: OrderRequest):
        service = self._make_service(mid_price=23500.0)
        result = service.execute_order(order_request)

        assert result.filled_quantity == order_request.lot_size

    def test_no_price_getter_still_zero_but_not_crashing(self, order_request: OrderRequest):
        """No price feed wired -> fill_price is 0.0 (unknown, not fabricated);
        regression guard that this still resolves to a real (not None-typed
        crash) result rather than raising.
        """
        service = self._make_service(mid_price=0.0)
        result = service.execute_order(order_request)

        assert result.average_price in (0.0, None)
...
class TestExecutePaperOrderDetail:
    def test_paper_market_order_with_slippage(self, service: ExecutionService):
        ...
        service._paper_trader._paper_price_cache["NIFTY"] = 23500.0
...
class TestGetCurrentPriceForSymbol:
    def test_known_symbol(self, service: ExecutionService):
        ...
        # Matches PaperTrader._SYMBOL_META default for NIFTY
    ...
    def test_cache_used(self, service: ExecutionService):
        service._paper_trader._paper_price_cache["NIFTY"] = 99999.0
        price = service._get_current_price_for_symbol("NIFTY")
    ...
    def test_cache_cleanup(self, service: ExecutionService):
        ...
        for i in range(60):
            service._paper_trader._paper_price_cache[f"SYM_{i}"] = float(i)
        # Access a symbol to trigger cleanup
        service._get_current_price_for_symbol("NIFTY")
        # Cache should now have < 60 entries
        assert len(service._paper_trader._paper_price_cache) < 60
