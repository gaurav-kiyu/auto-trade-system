...
class TestCertificationGateResult:
    def test_default_values(self):
        ...
        assert r.total_certifiers == 7
        assert r.passed_certifiers == 0
        assert r.timestamp is not None
        # New certifier fields exist
        assert hasattr(r, "slo_compliance")
        assert hasattr(r, "security_compliance")
    ...
    def test_summary_passed(self):
        r = CertificationGateResult(
            ...
            passed_certifiers=7,
            total_certifiers=7,
            slo_compliance={"status": "PASSED"},
            security_compliance={"status": "PASSED"},
...
class TestCertificationGate:
    ...
    def test_run_all_has_certifier_results(self):
        ...
        assert result.slo_compliance is not None
    ...
    def test_run_all_with_skip_config(self):
        gate = CertificationGate({
            ...
            "cert_gate_skip_slo": True,
            "cert_gate_skip_security": True,
            "cert_gate_skip_architecture": True,
            "cert_gate_skip_hygiene": True,
        })
        result = gate.run_all()
        # All should be skipped
        assert result.skipped_certifiers == 7
...
# ── SLO Compliance Runner Tests ────────────────────────────────────────────

class TestSloComplianceRunner:
    def test_skip_when_disabled(self):
        result = _run_slo_compliance({"cert_gate_skip_slo": True})
        assert result["status"] == "SKIPPED"

    def test_returns_dict(self):
        result = _run_slo_compliance({})
        assert isinstance(result, dict)
        assert "status" in result
        assert "has_data" in result

    def test_ci_mode_no_data(self):
        """With no SLO metrics recorded, should pass with warnings (CI mode)."""
        result = _run_slo_compliance({})
...
# ── Security Compliance Runner Tests ──────────────────────────────────────────

class TestSecurityComplianceRunner:
    def test_skip_when_disabled(self):
        result = _run_security_compliance({"cert_gate_skip_security": True})
        assert result["status"] == "SKIPPED"

    def test_returns_dict(self):
        result = _run_security_compliance({})
        assert isinstance(result, dict)
        assert "status" in result
    ...
    def test_reports_check_count(self):
        ...
        assert total <= result["total_checks"]
