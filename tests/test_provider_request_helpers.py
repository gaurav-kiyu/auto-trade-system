...
class TestRecordProviderRequest:
    ...
    def _mod(self):
        ...
        The reloaded module inherits Python's module-global dict, so the
        in-memory ``_PROVIDER_REQUESTS`` list is NOT reset by ``reload()`` —
        entries recorded by earlier tests in the suite would otherwise skew
        length-based assertions (and old entries get pruned mid-test). Clear
        the list after reload for hermetic per-test state.
        """
        import importlib
