...
def test_config_drift_logs_missing_defaults_key(caplog):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        defaults_data = {"NEW_FEATURE_A": False, "NEW_FEATURE_B": 42}
        with open(defaults_file, "w") as f:
            json.dump(defaults_data, f)

        merged = {"EXISTING_KEY": "value"}  # Missing both NEW_FEATURE_A and NEW_FEATURE_B
        mock_cfg = _make_mock_secure_config(defaults_data, merged, defaults_path=defaults_file)
