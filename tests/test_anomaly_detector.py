...
class TestAnomalyDetector:
    ...
    def test_update_and_check_insufficient_data(self):
        ...
        try:
            ...
            # With values [10, 10, 20]: mean=13.33, std=4.71, z for 20 = 1.42
            assert is_anomaly is False  # Below threshold of 3.0
            assert abs(z_score - 1.42) < 0.01
        finally:
            os.unlink(history_file)
    ...
    def test_update_and_check_detects_anomaly(self):
        ...
        try:
            ...
            # With history [10,10,10,10,10] and new value 30:
            # temp_history = [10,10,10,10,10,30] (includes current value)
            # mean = (5*10 + 30)/6 = 80/6 = 13.33
            # std = sqrt(((10-13.33)^2*5 + (30-13.33)^2)/6) = sqrt(83.33) = 9.13
            # z = |30-13.33|/9.13 = 16.67/9.13 = 2.24
            # z > 2.0 threshold, so is_anomaly = True
            assert is_anomaly is True
            # z_score is computed correctly (not 0.0 when std > 0)
            assert z_score == pytest.approx(2.236, rel=0.01)
        finally:
            os.unlink(history_file)
    ...
    def test_update_and_check_no_anomaly_similar_value(self):
        ...
        try:
            ...
            # Test value close to baseline - should not be anomaly
            is_anomaly, z_score = detector.update_and_check("test_metric", 10.5)
            # temp_history = [10,10,10,10,10,10.5]
            # mean = (5*10 + 10.5)/6 = 60.5/6 = 10.08
            # std = sqrt(((10-10.08)^2*5 + (10.5-10.08)^2)/6) = sqrt(0.035)
            # z = |10.5-10.08|/std = 0.42/std
            # If z > 2.0, is_anomaly = True; otherwise False
            # std ≈ 0.204, so z ≈ 2.06, which is just above 2.0 threshold
            # Due to rounding, this may be anomaly with small z_score
            if is_anomaly:
                assert z_score > 2.0
        finally:
            os.unlink(history_file)
    ...
    def test_detect_anomaly_convenience_function(self):
        ...
        try:
            ...
            # z = |10-6.67|/2.36 = 3.33/2.36 ≈ 1.41
            # z = 1.41 < 2.0, so NOT an anomaly
            assert is_anomaly is False
            assert z_score == pytest.approx(1.41, rel=0.05)
        finally:
            os.unlink(history_file)
    ...
    def test_different_metrics_isolated(self):
        ...
        try:
            ...
            assert is_anom_a is False  # z-score below 2.0 threshold
            ...
            assert is_anom_b is False  # z-score below 2.0 threshold
        finally:
            os.unlink(history_file)
