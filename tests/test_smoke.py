...
def test_adaptive_threshold_enabled_reads_from_config():
    """Regression: ADAPTIVE_THRESHOLD_ENABLED used to be hardcoded to True
    regardless of the ADAPTIVE_THRESHOLD_ENABLED config key, so admins could
    never actually turn it off. It must now read from _CFG (default True,
    matching the old hardcoded behavior)."""
    src = INDEX_IMPL.read_text(encoding="utf-8")
    assert "ADAPTIVE_THRESHOLD_ENABLED = True\n" not in src
    assert 'ADAPTIVE_THRESHOLD_ENABLED = bool(_CFG.get("ADAPTIVE_THRESHOLD_ENABLED", True))' in src


def test_expiry_controller_reads_cutoff_hour_from_config():
    """Regression: ExpiryDayController was always constructed with its
    hardcoded BLOCK_AFTER=13:00 default; EXPIRY_CUTOFF_HOUR (also default 13)
    was validated but never passed through. Must now be wired via block_after."""
    src = INDEX_IMPL.read_text(encoding="utf-8")
    assert 'block_after=_dt_time(int(_CFG.get("EXPIRY_CUTOFF_HOUR", 13)), 0)' in src


def test_scripts_exist():
    assert (ROOT / "index_app" / "index_trader.py").is_file()
    assert (ROOT / "index_app" / "gui" / "trader_desk.py").is_file()
    assert (ROOT / "core" / "strategy" / "orchestrator.py").is_file()
    assert (ROOT / "core" / "services" / "use_cases" / "trading_orchestrator.py").is_file()
...
def test_core_package_imports():
    code = """
from core import DataEngine, RiskConfig, StateManager, now_ist
...
assert DataEngine and ExecutionEngine and RiskConfig and StateManager and StrategyOrchestrator and TradingOrchestrator
...
os.environ["OPBUYING_INDEX_CONFIG"] = r"{ROOT / 'json' / 'config.json'}"
...
def test_index_selftest_exits_zero():
    env = {**os.environ, "OPBUYING_INDEX_CONFIG": str(ROOT / "json" / "config.json")}
    r = subprocess.run(
