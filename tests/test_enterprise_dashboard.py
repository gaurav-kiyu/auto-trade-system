...
class TestDashboardInit:
    def test_creation_defaults(self, tmp_path):
        ...
        assert db._state_path == "json/trader_state.json"
        assert db._db_path == "db/trades.db"
...
class TestConfigApply:
    ...
    def test_apply_creates_audit_trail(self, tmp_path):
        ...
        audit_file = Path("json/config_audit.jsonl")
        assert audit_file.is_file()
        assert "alice" in audit_file.read_text(encoding="utf-8")
...
class TestConfigApplySyncsUserPermissions:
    """Regression: _apply_config_change()'s EMAIL_TO/CHAT_ID -> admin user
    permission sync called get_user_permission()/upsert_user_permission(),
    neither of which ever existed on UserPermissionManager (only the plural
    get_user_permissions() and update_user_permissions() do) - this silently
    raised AttributeError, swallowed by a bare except, every single time."""

    def test_email_to_and_chat_id_sync_to_admin_permissions(self, tmp_path, monkeypatch):
        from core.auth.user_signal_permissions import UserPermissionManager
        from core.enterprise_dashboard import EnterpriseDashboard

        isolated_mgr = UserPermissionManager(store_path=tmp_path / "user_signal_permissions.json")
        monkeypatch.setattr(UserPermissionManager, "get_instance", classmethod(lambda cls: isolated_mgr))
        ...
        r = db._apply_config_change(
            {"EMAIL_TO": "a@x.com,b@x.com", "CHAT_ID": "111,222"}, "testuser",
        )
        assert r["success"]

        admin_perm = isolated_mgr.get_user_permissions("admin")
        assert admin_perm is not None
        assert admin_perm.email == "a@x.com,b@x.com"
        assert admin_perm.telegram_chat_id == "111,222"

    def test_sync_failure_does_not_block_config_apply(self, tmp_path, monkeypatch):
        """Even if the permission sync itself fails, the config write must
        still succeed - this sync is best-effort, not a hard dependency."""
        from core.auth.user_signal_permissions import UserPermissionManager
        from core.enterprise_dashboard import EnterpriseDashboard

        def _broken_get_instance(cls):
            raise RuntimeError("permission store unavailable")

        monkeypatch.setattr(UserPermissionManager, "get_instance", classmethod(_broken_get_instance))
        ...
        r = db._apply_config_change({"EMAIL_TO": "a@x.com"}, "testuser")
        assert r["success"]
...
# ── Config Audit Log (who changed what, when) ──────────────────────────────────


class TestConfigAuditLog:
    def test_apply_writes_to_isolated_path_not_real_project_file(self, tmp_path):
        """Regression: _log_config_audit() used a bare hardcoded
        Path("json/config_audit.jsonl"), so every test exercising
        _apply_config_change() wrote real entries into the live project's
        audit trail. config_audit_log_path must be overridable like the
        other *_path config keys."""
        from core.enterprise_dashboard import EnterpriseDashboard
        ...
        db = EnterpriseDashboard(config={
            "web_dashboard_host": "127.0.0.1",
            "index_config_path": config_path,
            "config_audit_log_path": str(audit_path),
        })
        db._apply_config_change({"K": "new"}, "testuser")
        assert audit_path.is_file()
        real_project_audit = Path("json/config_audit.jsonl")
        if real_project_audit.is_file():
            assert "testuser" not in real_project_audit.read_text(encoding="utf-8")

    def test_audit_log_returns_recorded_entries_newest_first(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard
        ...
        db._apply_config_change({"FIRST_KEY": 1}, "alice")
        db._apply_config_change({"SECOND_KEY": 2}, "bob")

        entries = db._get_config_audit_log()
        assert len(entries) == 2
        assert entries[0]["username"] == "bob"
        assert entries[0]["keys"] == ["SECOND_KEY"]
        assert entries[1]["username"] == "alice"

    def test_audit_log_includes_rollback_entries(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard

        config_path = tmp_path / "config.json"
        _make_config_file(str(config_path), {"k": "v"})
        backup = config_path.parent / f"config.json.backup.{int(time.time())}"
        backup.write_text('{"k": "v2"}', encoding="utf-8")
        ...
        db._rollback_config(backup.name, "carol")

        entries = db._get_config_audit_log()
        assert len(entries) == 1
        assert entries[0]["action"] == "config_rollback"
        assert entries[0]["username"] == "carol"

    def test_audit_log_empty_when_no_file(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard

        db = EnterpriseDashboard(config={
            "web_dashboard_host": "127.0.0.1",
            "config_audit_log_path": str(tmp_path / "no_such_file.jsonl"),
        })
        assert db._get_config_audit_log() == []

    def test_audit_log_respects_limit(self, tmp_path):
        from core.enterprise_dashboard import EnterpriseDashboard
        ...
        for i in range(5):
            db._apply_config_change({f"KEY_{i}": i}, "testuser")
        assert len(db._get_config_audit_log(limit=2)) == 2
...
class TestReadState:
    ...
    def test_read_state_missing_file(self, tmp_path):
        ...
        state = db._read_state()
        # Missing file -> safe documented defaults (dashboard must still render)
        assert state.get("capital") == 10000.0
    ...
    def test_read_state_corrupt_file(self, tmp_path):
        ...
        state = db._read_state()
        # Corrupt file -> safe documented defaults (dashboard must still render)
        assert state.get("capital") == 10000.0
        assert state.get("status") == "RUNNING"
...
class TestLoadRecentTrades:
    ...
    def test_load_trades_no_db(self, tmp_path):
        ...
        # No DB -> documented demo-trade fallback (dashboard showcase UX)
    ...
    def test_load_trades_exception_handler(self, monkeypatch):
        """When load_trades raises, _load_recent_trades catches and falls back to demo data."""
        ...
        db = EnterpriseDashboard(config={"web_dashboard_host": "127.0.0.1"})
        trades = db._load_recent_trades(days=30, n=100)
        # Exception -> documented demo-trade fallback (dashboard showcase UX)
        assert isinstance(trades, list) and len(trades) > 0
        assert "direction" in trades[0]
        monkeypatch.undo()
...
class TestGetSignals:
    def test_get_signals_no_signal_log(self):
        ...
        db = EnterpriseDashboard(config={"web_dashboard_host": "127.0.0.1"})
        # No signal log -> generated/demo fallback signals (dashboard UX)
    ...
    def test_get_signals_with_signal_log(self):
        ...
        sigs = db._get_signals()
        assert len(sigs) == 2
        assert sigs[0].get("reasoning") == "Autonomous signal validation passed"
        assert sigs[0].get("sentiment") == "BULLISH"
...
class TestHttpEndpoints:
    ...
    def test_security_page_requires_login(self, mock_templates, no_csrf, admin_auth):
        """GET /security with no session redirects to /login (unauthenticated)."""
        d, _token = admin_auth
        with TestClient(d.app, follow_redirects=False) as c:
            r = c.get("/security")
        assert r.status_code in (302, 303, 307)
        assert r.headers["location"] == "/login"

    def test_security_page_blocks_non_admin(self, mock_templates, no_csrf, admin_auth):
        """Regression: /security's own APIs (/api/auth/users, /api/auth/audit) already
        require admin, but the page itself previously only required login — a logged-in
        viewer would see raw 403 error rows instead of being kept out of the page."""
        d, _admin_token = admin_auth
        res = d._auth.create_user("secpageviewer", "Xk7$mQz9Lp2!", role="viewer")
        assert res["success"], res
        viewer = d._auth.get_user_by_id(res["user_id"])
        viewer_token = d._auth.create_session(viewer)
        with TestClient(d.app, follow_redirects=False) as c:
            c.cookies.set("opb_session", viewer_token.token)
            r = c.get("/security")
        assert r.status_code == 403

    def test_security_page_allows_admin(self, mock_templates, no_csrf, admin_auth):
        """GET /security with an admin session renders 200 OK."""
        d, token = admin_auth
        with TestClient(d.app) as c:
            c.cookies.set("opb_session", token)
            r = c.get("/security")
