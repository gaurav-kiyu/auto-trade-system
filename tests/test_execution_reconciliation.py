...
def temp_db():
    ...
    Lives under the OS temp dir, not the repo root - a lingering SQLite
    file handle on Windows can make unlink() fail at teardown (hence the
    swallowed OSError below), and when that happened here previously with
    a repo-root-relative path, the leftover test_recon_*.db files
    accumulated in the tracked working tree instead of somewhere ignored.
    """
    import tempfile
    import time
    # Use a unique name with timestamp to avoid Windows locking
    db_path = str(Path(tempfile.gettempdir()) / f"test_recon_{int(time.time()*1000)}.db")
    yield db_path
