...
# ── STRUCTURED_EVENTS_ENABLED wire-in (first log_structured_event() call site) ──


class TestStructuredEventsOnHardHalt:
    def test_no_cfg_no_structured_log(self) -> None:
        """Existing call sites (kill file watcher, intraday P&L check) don't
        pass cfg at all - behavior must be identical to before this wire-in."""
        with patch("core.log_helpers.log_structured_event") as mock_log:
            trip_hard_halt("no cfg", source="test")
        mock_log.assert_not_called()

    def test_cfg_without_flag_no_structured_log(self) -> None:
        with patch("core.log_helpers.log_structured_event") as mock_log:
            trip_hard_halt("flag absent", source="test", cfg={})
        mock_log.assert_not_called()

    def test_flag_false_no_structured_log(self) -> None:
        with patch("core.log_helpers.log_structured_event") as mock_log:
            trip_hard_halt("flag false", source="test", cfg={"STRUCTURED_EVENTS_ENABLED": False})
        mock_log.assert_not_called()

    def test_flag_true_fires_structured_log(self) -> None:
        with patch("core.log_helpers.log_structured_event") as mock_log:
            trip_hard_halt("flag true", source="test_source", cfg={"STRUCTURED_EVENTS_ENABLED": True})
        mock_log.assert_called_once()
        args, _ = mock_log.call_args
        assert args[1] == "hard_halt_tripped"
        assert args[2]["reason"] == "flag true"
        assert args[2]["source"] == "test_source"

    def test_flag_true_but_already_halted_does_not_double_log(self) -> None:
        """trip_hard_halt() no-ops (including the structured log) if already halted."""
        trip_hard_halt("first", source="test")
        with patch("core.log_helpers.log_structured_event") as mock_log:
            trip_hard_halt("second", source="test", cfg={"STRUCTURED_EVENTS_ENABLED": True})
        mock_log.assert_not_called()

    def test_structured_log_error_is_swallowed(self) -> None:
        """A bug in the structured-log wire-in must never prevent the halt
        itself from taking effect."""
        with patch("core.log_helpers.log_structured_event", side_effect=ValueError("boom")):
            trip_hard_halt("still halts", source="test", cfg={"STRUCTURED_EVENTS_ENABLED": True})
        assert is_hard_halted()
...
class TestKillFile:
    ...
    def test_kill_file_watcher_requests_graceful_shutdown(self) -> None:
        """STOP_TRADING must both hard-halt AND signal _shutdown so the main
        trading loop exits (kill file is the documented graceful-stop path)."""
        with (
            patch("core.safety_state.is_kill_file_present", return_value=True),
            patch("core.safety_state._kill_watcher_started", True),
            # Isolate: don't actually execute process-wide shutdown callbacks
            # from the test thread (order-dependent, side-effectful).
            patch("core.safety_state.request_shutdown") as mock_shutdown,
        ):
            core.safety_state._kill_file_watcher()
            assert is_hard_halted()
            assert "STOP_TRADING" in hard_halt_reason()
            mock_shutdown.assert_called_once()
            assert "STOP_TRADING" in mock_shutdown.call_args[0][0]
