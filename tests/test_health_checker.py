...
class TestCheckBrokerHealth:
    ...
    def test_broker_not_initialized_paper_mode(self, mock_import):
        """Paper/SIGNAL_ONLY mode with no broker adapter should be OK."""
        mock_mod = MagicMock()
        mock_mod._broker = None
        mock_import.return_value = mock_mod
        results = check_broker_health({})
        assert any("Broker availability" in r.name and r.status == "OK" for r in results)

    @patch("importlib.import_module")
    def test_broker_not_initialized_live_mode(self, mock_import):
        """Live mode with no broker adapter should WARN."""
        mock_mod = MagicMock()
        mock_mod._broker = None
        mock_import.return_value = mock_mod
        results = check_broker_health({"paper_mode": False, "execution_mode": "LIVE"})
    ...
    @patch("importlib.import_module")
    def test_broker_unhealthy_live_mode(self, mock_import):
        """Unhealthy broker in live mode should FAIL."""
        mock_mod = MagicMock()
        mock_broker = MagicMock()
    ...
    @patch("importlib.import_module")
    def test_broker_unhealthy_paper_mode(self, mock_import):
        """Unhealthy broker in paper mode should be OK (paper simulation)."""
        mock_mod = MagicMock()
        mock_broker = MagicMock()
        mock_broker.is_healthy.return_value = False
        mock_mod._broker = mock_broker
