...
# ── classify_tier: config-sourced overrides ─────────────────────────────────

def test_classify_tier_no_overrides_matches_hardcoded_defaults() -> None:
    """Regression guard: callers that don't pass overrides keep today's behavior."""
    for score in (0, 55, 60, 65, 69, 70, 75, 79, 80, 85, 100):
        assert classify_tier(score) == classify_tier(
            score, strong_min=TIER_STRONG_MIN, moderate_min=TIER_MODERATE_MIN, weak_min=TIER_WEAK_MIN,
        )


def test_classify_tier_respects_config_override() -> None:
    """A Super-Admin-configured TIER_STRONG_MIN must actually change classification."""
    assert classify_tier(82, strong_min=85, moderate_min=70, weak_min=60) == "MODERATE"
    assert classify_tier(85, strong_min=85, moderate_min=70, weak_min=60) == "STRONG"


def test_get_tier_rules_respects_config_override() -> None:
    rules = get_tier_rules(82, strong_min=85, moderate_min=70, weak_min=60)
    assert rules.tier == "MODERATE"


def test_tier_bounds_from_config_reads_keys() -> None:
    assert tier_bounds_from_config({"TIER_STRONG_MIN": 85, "TIER_MODERATE_MIN": 72, "TIER_WEAK_MIN": 55}) == (
        85, 72, 55,
    )


def test_tier_bounds_from_config_falls_back_to_hardcoded_defaults() -> None:
    assert tier_bounds_from_config({}) == (TIER_STRONG_MIN, TIER_MODERATE_MIN, TIER_WEAK_MIN)
    assert tier_bounds_from_config(None) == (TIER_STRONG_MIN, TIER_MODERATE_MIN, TIER_WEAK_MIN)
