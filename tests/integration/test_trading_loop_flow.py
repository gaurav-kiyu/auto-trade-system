...
class TestFullTradingLoopFlow:
    ...
    def test_config_loading_to_loop_entry(
        ...
        monkeypatch.setenv("OPBUYING_DISABLE_DOTENV", "1")
    ...
    def test_periodic_reconcile_calls_qty_mismatch_check_and_throttles(self):
        """Regression guard: _reconcile_positions_live() must actually be invoked
        from the live loop (it previously had no caller at all), throttled to
        RECONCILE_INTERVAL seconds so it doesn't run on every scan cycle."""
        import index_app.index_trader as it

        it._last_qty_mismatch_check_ts = 0.0
        with patch.object(it, "_execution_service") as mock_exec, \
             patch.object(it, "_reconcile_positions_live") as mock_qty_check:
            mock_exec.run_ack_watchdog.return_value = {"acknowledged": 0, "errors": 0}

            it._periodic_reconcile()
            assert mock_qty_check.call_count == 1

            # Immediately calling again must NOT re-run the check (throttled)
            it._periodic_reconcile()
            assert mock_qty_check.call_count == 1

            # Once the interval has elapsed, it must run again
            it._last_qty_mismatch_check_ts = 0.0
            it._periodic_reconcile()
            assert mock_qty_check.call_count == 2
    ...
    @patch("index_app.index_trader.now_ist")
    @patch("index_app.index_trader.market_status")
    def test_reentry_evaluator_blocks_when_durable_signal_missing(
        self,
        mock_market_status: MagicMock,
        mock_now_ist: MagicMock,
    ):
        """Verify fail-closed signal persistence boundary when durable signal_id is missing.

        When an opportunity does not supply a durable signal_id and SignalTracker
        cannot record or recover one, execution is strictly blocked with
        SIGNAL_PERSISTENCE_BLOCK.
        """
        import index_app.index_trader as it
        ...
        # Ad-hoc signal without durable signal_id
        sig = {
            "signal": "BUY", "direction": "CALL", "score": 85,
            "price": 150.0, "signal_ts": time.time(), "timestamp": time.time(),
            "breakout_ok": True,
                    ...
                    with patch.object(it, "_warmup_manager") as mock_warmup:
                        ...
                        with patch.object(it, "_news_sentinel") as mock_news:
                            ...
                            with patch("core.datetime_ist.is_in_auction_session", return_value=False):
                                with patch.object(it, "_execution_service") as mock_exec:
                                    with patch.object(it, "_margin_validator") as mock_margin:
                                        mock_margin.validate.return_value = MagicMock(allowed=True, error_message="", warning_message="")
                                        with patch.object(it, "_portfolio_service") as mock_portfolio:
                                            mock_portfolio.get_available_margin.return_value = 10000.0
                                            with patch("core.signals.signal_tracker.SignalTracker.get_instance") as mock_tracker_inst:
                                                mock_tracker = MagicMock()
                                                mock_tracker.record_generated_signal.return_value = ""
                                                mock_tracker.get_active_signal_id.return_value = None
                                                mock_tracker_inst.return_value = mock_tracker

                                                it.enter_trade("NIFTY", sig)

        # Decision log must show SIGNAL_PERSISTENCE_BLOCK
        assert "NIFTY" in it.decision_log
        log_msg = str(it.decision_log.get("NIFTY", {}).get("msg", ""))
        assert "SIGNAL_PERSISTENCE_BLOCK" in log_msg, f"Expected SIGNAL_PERSISTENCE_BLOCK, got: {log_msg}"
        mock_exec.execute_order.assert_not_called()
