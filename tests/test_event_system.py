...
class TestSingletons:
    ...
    def test_get_event_store_returns_store(self):
        ...
        Note: v2.57.0 redirects to the unified ``core.event_store.EventStore``.
        """
        store = get_event_store()
        # Accept both legacy EventStore and unified EventStore
        from core.event_store import EventStore as UnifiedEventStore
        assert isinstance(store, (EventStore, UnifiedEventStore))
    ...
    def test_get_event_store_from_bus(self):
        ...
        Note: v2.57.0 redirects to unified EventStore, so the bus's
        internal store may differ from the global singleton.
        """
        store = get_event_store()
