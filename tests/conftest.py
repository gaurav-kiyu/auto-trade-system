...
# Pre-import yfinance at module level to ensure numpy loads exactly once.
# Python 3.14+ numpy raises ImportError if loaded more than once per process.
try:
    import yfinance  # triggers numpy import once at module load
...
def _patch_network_calls(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    Uses object-based monkeypatching (not string-based) to avoid triggering
    repeated numpy/yfinance imports. Python 3.14+ numpy raises ImportError
    if loaded more than once per process.
