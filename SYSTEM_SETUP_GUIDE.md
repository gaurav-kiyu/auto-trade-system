# OPB Index Options Bot v2.58.0 - Complete Setup Guide

## Version: 2.58.0 (Production Ready - Limited Live Pilot)
## Date: July 30, 2026
...
| File | Use Case |
|------|-----------|
| json/config.json | Default (Rs.5,000, MANUAL mode) |
| config.lowcap.json | Low capital (Rs.2,000) |
| json/config.paper.json | Paper trading only |
| json/config.template.json | Template for customization |
...
**Telegram & Gmail Alerts Setup**

Per `SECRETS_MIGRATION_GUIDE.md`/`SECURITY.md`, these values must be supplied
via `OPBUYING_*` environment variables (or `.env`), never hardcoded in
config.json — example placeholders only, do not commit real values:
```json
...
**Full 2,553+ NSE Stock Universe Real-Time Scan**
```bash
python -m core.all_nse_scanner
```

**No signals generated**
```
Check: VIX must be 12-28, time must be 9:20-14:45 IST, score must meet STRONG (>=80) or MODERATE (>=68)
```
...
|---------|-----|-------------|
| v2.58.0 | July 2026 | CI/CD pipeline, dead code optimization, governance boost |
| v2.56.0 | May 2026 | TOCTOU fix, idempotency, failover alert |
...
- Performance: docs/FINAL_CERTIFICATION_REPORT_v2.56.0.md
- Quick Start: QUICK_START_GUIDE.md
- Presentation: OPB_Presentation_v2.56.0.pptx
- Current Release: RELEASE_NOTES.md (v2.58.0)
