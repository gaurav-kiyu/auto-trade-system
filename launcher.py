...
if not (_BASE_DIR / "index_app" / "index_trader.py").exists() and (_BASE_DIR.parent / "index_app" / "index_trader.py").exists():
    APP_DIR: Path = _BASE_DIR.parent
else:
    APP_DIR: Path = _BASE_DIR

# ── Launcher settings ─────────────────────────────────────────────────────────
_SETTINGS_FILE = APP_DIR / "json/launcher_settings.json"
_DEFAULTS: dict = {
    "default_mode":      "PAPER",
    "auto_launch":       False,
...
def _get_dashboard_port() -> int:
    """Read the real bot's web_dashboard_port (3-layer config merge:
    json/index_config.defaults.json -> json/config.json -> json/config.local.json).

    _S (launcher_settings.json) has no such key - it's a separate, launcher-only
    settings file - so _S.get("web_dashboard_port", 8000) always fell through to
    the hardcoded 8000 fallback, which is the dead legacy core/web_dashboard.py
    port, not the real enterprise dashboard the bot actually serves on 8765.
    """
    port = 8765
    for _relpath in ("json/index_config.defaults.json", "json/config.json", "json/config.local.json"):
        try:
            _f = APP_DIR / _relpath
            if _f.exists():
                _data = json.loads(_f.read_text(encoding="utf-8"))
                if "web_dashboard_port" in _data:
                    port = int(_data["web_dashboard_port"])
...
class LauncherApp:
    ...
    def _setup_window(self) -> None:
        self.root.title("GAURAV — OPBuying Index App v5.0.0")
    ...
    def _build_ui(self) -> None:
        ...
        tk.Label(hdr, text="INDEX OPTION BUYING — GAURAV v5.0.0",
    ...
    def _check_launched_process(self, mode: str) -> None:
        ...
        if ret is not None:
            ...
            # Process is still running - good, but the dashboard's own FastAPI
            # server/DI wiring can take longer than this 2s check to actually
            # bind its port, so poll for it rather than assuming it's ready.
            self._status(f"Launched [{mode}] - waiting for Web Dashboard to come up …")
            self._dashboard_port = _get_dashboard_port()
            self._wait_for_dashboard(attempt=0)

    def _wait_for_dashboard(self, attempt: int) -> None:
        """Poll the dashboard port before opening the browser (max ~30s)."""
        import socket
        ...
        if ready:
            self._log(f"\n  [INFO] Web Dashboard is up on port {port}.")
            self._status(f"Opening Web Dashboard (port {port}) in browser …")
            try:
                import webbrowser
        ...
        if attempt >= 30:
            self._log(
                f"\n  [WARN] Web Dashboard did not respond on port {port} after 30s."
            )
            self._log(
                "  The bot itself may still be starting - open "
                f"http://localhost:{port}/ manually once it's ready, or check "
                "the app's own console window for errors."
            )
            self._status(f"App running - dashboard not yet reachable on port {port}.")
            self.root.after(3000, self.root.quit)
            return

        self.root.after(1000, self._wait_for_dashboard, attempt + 1)
