...
import sqlite3
try:
    src = sqlite3.connect('db/trades.db')
...
if ok == 'ok':
    ...
    import shutil, os
    os.replace('db/trades.db', 'trades_corrupted_' + __import__('datetime').datetime.now().strftime('%Y%m%d_%H%M%S') + '.db')
    shutil.copy('trades_recovered.db', 'db/trades.db')
...
if backups:
    shutil.copy(backups[0], 'db/trades.db')
...
python -c "
import sqlite3, os
if os.path.exists('db/trades.db'):
    os.rename('db/trades.db', f'trades_unrecoverable_{__import__(\"datetime\").datetime.now().strftime(\"%Y%m%d_%H%M%S\")}.db')
conn = sqlite3.connect('db/trades.db')
