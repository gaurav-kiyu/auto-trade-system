...
| Configuration files backed up | `ls -la json/config.json.backup.*` |
...
1. **Take a pre-drill snapshot:**
   ...
   cp json/trader_state.json trader_state.pre_drill.json
   cp json/config.json config.pre_drill.json
   ```
...
1. **Simulate corruption:**
   ...
   dd if=/dev/urandom of=db/trades.db bs=1024 count=10 conv=notrunc
   ```

2. **Attempt recovery:**
   ```bash
   # SQLite's built-in recovery
   sqlite3 db/trades.db ".recover" | sqlite3 trades_recovered.db
...
2. **Restore pre-drill state:**
   ...
   cp trader_state.pre_drill.json json/trader_state.json
   
   # Restore config
   cp config.pre_drill.json json/config.json
   ```
