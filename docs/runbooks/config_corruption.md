...
## Resolution
1. Check which config file is corrupted (json/config.json, json/config.local.json)
2. Restore from backup: `copy json/config.json.bak json/config.json`
3. If no backup → regenerate from defaults: `copy json/index_config.defaults.json json/config.json`
4. Re-apply any overrides (env vars, json/config.local.json)
