...
# Stock config
cp json/stock_config.template.json json/stock_config.json

# Index config defaults (should already be present)
ls -la json/index_config.defaults.json
```
...
Edit `json/config.json` for your risk preferences:
...
python -c "
import sqlite3
for db in ['db/trades.db', 'db/trade_journal.db', 'db/ml_tracker.db', 'db/execution_state.db']:
...
|----------|---------|----------|
| `db/trades.db` | Trade log | Project root |
| `db/trade_journal.db` | Execution quality | Project root |
| `db/ml_tracker.db` | ML predictions | Project root |
| `db/oi_snapshots.db` | OI history | Project root |
| `db/event_store.db` | Event hash chain | Project root |
| `db/execution_state.db` | State machine persistence | Project root |
...
- [ ] Paper trades execute (check `db/trades.db`)
...
trades = load_trades(\"db/trades.db\")
...
# 0 10 * * 0 cd /path/to/opb_trading_bot && tar -czf backups/full_$(date +\\%Y\\%m\\%d).tar.gz json/config.json data/
```
...
find /opt/opb_trading_bot -type f -name "json/config.json" -exec chmod 600 {} \;
