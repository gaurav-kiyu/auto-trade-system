...
# v2.56.0 Item 12 — News Sentinel (RSS feed parsing)
feedparser>=6.0.0

# v2.56.0 Item 20 — A/B Strategy Tester (Mann-Whitney U significance test)
scipy>=1.11.0
...
cp json/index_config.defaults.json json/config.json
cp json/stock_config.defaults.json json/stock_config.json

# Edit json/config.json to set your preferences
...
1. **Defaults**: `json/index_config.defaults.json` (single source of truth for default values)
2. **Config Files**: `json/config.json` (user-overridden values)
3. **Local Config**: `json/config.local.json` (local overrides, not committed to git)
...
chmod 600 json/config.json
chmod 600 json/config.local.json
chmod 600 .env

# Restrict permissions on database files
chmod 600 data/*.db
chmod 600 db/trades.db
chmod 600 db/trade_journal.db
```
...
2. **Review change logs**: Check `CHANGELOG.md` and `docs/adr/` for architecture-affecting changes
3. **Install new dependencies**: `pip install -r requirements.txt`
4. **Migrate configuration**: Ensure `json/index_config.defaults.json` is present and valid
...
- `SYSTEM_SETUP_GUIDE.md`: User guide for setup and trading
- `docs/adr/`: Technical architecture decision records
...
*Last Updated: 2026-08-15 (per git history of this file — the previous `$(date)` was never actually expanded)*  
*Version: v2.59.0*