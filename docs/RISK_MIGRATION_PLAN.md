...
├── core/predictive_risk.py        — Removed (was 0 importers)
...
1. **Remove confirmed-dead risk engines**    - core/predictive_risk.py [Removed] — 0 importers confirmed
   - core/trading_risk.py [Removed] — 0 importers confirmed
...
1. **Mark old engines deprecated**
   - `core/services/risk_service.py` — Mark as deprecated, keep only `RiskConfig` / `RiskDecision` data classes
...
1. **Remove deprecated risk modules**
   - `core/services/risk_service.py` (keep RiskConfig/RiskDecision if used elsewhere)
