# Rollback Plan — OPB Institutional Platform v2.56.0
...
| 3 | Save current state: `copy json/trader_state.json json/trader_state.json.pre_rollback` | 1 min | Operator |
...
| 7 | Restore state: `copy json/trader_state.json.pre_rollback json/trader_state.json` | 1 min | Operator |
...
|------|--------|----------|
| 1 | Restore previous config: `git checkout HEAD~1 -- json/config.json` | 1 min |
...
- `json/config.json.bak` — Automatic backup on every config change
- `json/config_audit.jsonl` — Complete config change history
...
| Data corruption | Restore from backup: `copy db/trades.db.bak db/trades.db` | 1 min |
| WAL corruption | `python -c "from core.wal.journal import recover; recover()"` | 2 min |

**Database backup locations:**
- `db/trades.db.bak` — Last known-good backup
- `backups/trades_YYYYMMDD.db` — Daily backup
- `json/trader_state.json` — Session state (auto-saved)
...
# Database rollback
python scripts/rollback.py --db --backup-file db/trades.db.bak

# Config rollback
python scripts/rollback.py --config --restore json/config.json.bak
```
...
- [ ] Database backup exists (`db/trades.db.bak`)
- [ ] Config backup exists (`json/config.json.bak`)
