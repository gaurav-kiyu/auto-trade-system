...
- [ ] Backup of all databases: `db/trades.db`, `json/trader_state.json`
...
| New config keys missing | LOW | MEDIUM | Defaults in `json/index_config.defaults.json` |
...
| Trade count | ≥100 paper trades executed | `db/trades.db` |
...
```python
# 1. Add default to json/index_config.defaults.json
...
if "OLD_KEY" in cfg:
    log.warning("OLD_KEY is deprecated and will be removed in v2.56.0")
...
| | Databases backed up | `copy db/trades.db db/trades.db.bak` |
