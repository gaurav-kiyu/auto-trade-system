...
# Verdict captured by the most recent run_pre_release_checks() call for the
# register-pattern consistency gate; read by main() when writing the audit
# record so the recorded verdict matches what the gate actually decided.
_LAST_REGISTER_GATE_VERDICT: dict = {}
...
def run_pre_release_checks(
    ...
    Side effect: records the register-consistency gate verdict into the module
    global ``_LAST_REGISTER_GATE_VERDICT`` (read by main() when writing the
    release audit record) so the audit reflects what the gate actually decided.
    """
    failures: list[str] = []
    ...
    for d in required_docs:
        if (ROOT / d).exists():
            continue
        # Check docs/archive/ as fallback for archived docs
        archived = ROOT / "docs" / "archive" / d
        if archived.exists():
            log.info("  [OK] Required doc (archived): %s", d)
            continue
    ...
    # 10. SLO compliance gate (Phase 16 — critical SLOs block release)
    # Runs inside skip_certifications guard alongside replay/paper certifiers
    if not skip_certifications:
        _run_slo_gate(failures)

    # 11. Register-pattern consistency gate (trend tracker data integrity)
    global _LAST_REGISTER_GATE_VERDICT
    _LAST_REGISTER_GATE_VERDICT = _run_register_consistency_gate(failures) or {}
...
def _run_slo_gate(failures: list[str]) -> None:
    """Run SLO compliance gate.  Any critical SLO failure blocks release.

    Integrates with the SLOGovernance system (Phase 16) to check all
    critical SLOs before allowing a release to proceed.
    """
    try:
        from core.slo_governance import get_slo_governance
        slo = get_slo_governance()

        # Check if any SLO metrics have been recorded (CI/fresh-checkout safe)
        ...
        if not has_any_data:
            log.info("  [!] SLO compliance: No metrics recorded yet (expected in CI/fresh-checkout) — skipping")
            return

        releasable, message = slo.is_releasable()
        if not releasable:
            failures.append(f"SLO compliance gate: {message}")
            # Log all details
            blocking = [r for r in report.results if not r.passed and r.slo.critical]
            for b in blocking:
                log.warning("  [X] Critical SLO breach: %s (current=%.2f, target=%.2f %s)",
                           b.slo.name, b.current_value, b.slo.target, b.slo.unit)
            # Also log non-critical failures
            non_critical = [r for r in report.results if not r.passed and not r.slo.critical]
            for n in non_critical:
                log.info("  [!] Non-critical SLO: %s (current=%.2f, target=%.2f %s)",
                        n.slo.name, n.current_value, n.slo.target, n.slo.unit)
        else:
            log.info("  [OK] SLO compliance: %s", message)
    except ImportError:
        log.info("  [!] SLO governance module not available (core.slo_governance) — skipping")
    except Exception as exc:
        failures.append(f"SLO compliance gate error: {exc}")
...
def _run_certification_checks(failures: list[str]) -> None:
    ...
    trades_db = str(ROOT / "db/trades.db")
...
def capture_trend_snapshot(version: str) -> bool:
    """Capture a success-metrics trend snapshot for this release.

    Runs ``python -m core.success_metrics_trend --capture --release v{version}``,
    appending a release-labeled snapshot to ``json/success_metrics_trend.json``
    (tracked governance evidence). Non-blocking: a capture failure logs a
    warning but does not block the release.

    Returns True on success, False otherwise.
    """
    try:
        result = subprocess.run(
            [sys.executable, "-m", "core.success_metrics_trend",
             "--capture", "--release", f"v{version}"],
            capture_output=True, text=True, cwd=str(ROOT), timeout=60,
        )
        # NOTE: cwd=str(ROOT) is required — TREND_STATE_FILE is a relative path
        # ("json/success_metrics_trend.json"), so the snapshot must be written
        # against the repo root or the tracked history would split across CWDs.
        if result.returncode == 0:
            log.info("  [OK] Trend snapshot captured for v%s", version)
            return True
        log.warning("  [X] Trend snapshot capture failed: %s",
                    (result.stderr or result.stdout).strip()[-300:])
    except (subprocess.TimeoutExpired, FileNotFoundError) as e:
        log.warning("  [!] Trend snapshot capture error: %s", e)
    return False
...
def _run_register_consistency_gate(failures: list[str]) -> dict:
    """Run the register-pattern consistency gate.  Failure blocks release.

    Verifies the governance registers (dead-code / duplicate / config-drift /
    doc-drift) still use the ID prefixes the success-metrics trend tracker
    counts. A drift would silently corrupt the MET-07 technical-debt
    indicators, so it is a blocking pre-release gate.

    Returns a verdict dict consumed by write_audit_record():
        ``{"passed": bool, "status": "aligned"|"drift"|"skipped"|"unavailable",
        "drifted_registers": [...]}``
    """
    try:
        result = subprocess.run(
            [sys.executable, "-m", "core.success_metrics_trend", "--check-registers"],
            capture_output=True, text=True, cwd=str(ROOT), timeout=30,
        )
        if result.returncode != 0:
            failures.append(
                "Register consistency gate: register ID patterns have drifted "
                "from what the trend tracker counts (run `python -m "
                "core.success_metrics_trend --check-registers` for details)"
            )
            # The CLI prints the drift breakdown to stdout (per-register status
            # + "Result: DRIFT DETECTED"), so surface stdout — not stderr.
            detail = (result.stderr or result.stdout).strip()[-500:]
            log.warning("  [X] Register consistency output:\n%s", detail)
            return {"passed": False, "status": "drift", "drifted_registers": []}
        log.info("  [OK] Register consistency: all registers aligned")
        return {"passed": True, "status": "aligned", "drifted_registers": []}
    except FileNotFoundError:
        # Only reachable if the python interpreter itself is missing; a missing
        # core.success_metrics_trend module yields rc=1 via `python -m`, which
        # is treated as a blocking failure (fail-closed) below.
        log.info("  [!] success_metrics_trend module not available - skipping")
        # passed=None (not verified) rather than True — "skipped" must not be
        # mistaken for "registers verified aligned" in the audit record.
        return {"passed": None, "status": "skipped", "drifted_registers": []}
    except subprocess.TimeoutExpired as exc:
        failures.append(f"Register consistency check timed out: {exc}")
        return {"passed": False, "status": "unavailable", "drifted_registers": []}
...
def generate_release_notes(version: str, changes: list[str] | None = None) -> str:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Guard against junk/non-semver tags (e.g. a stray 'doesnotexist'
            # tag) and against describing the tag we are about to create.
            if candidate.startswith("v") and candidate != f"v{version}":
                last_tag = candidate
    except (subprocess.TimeoutExpired, FileNotFoundError):
        pass

    # Fallback: highest existing semver tag other than the release version
    if not last_tag:
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                for t in result.stdout.strip().splitlines():
                    if t.startswith("v") and t != f"v{version}":
                        last_tag = t.strip()
                        break
        except (subprocess.TimeoutExpired, FileNotFoundError):
            pass
...
def update_changelog(version: str, changes: list[str] | None = None) -> bool:
    ...
    try:
        ...
        if CHANGELOG_FILE.exists():
            ...
            # Idempotency guard: skip if a heading for this version already
            # exists (any date) - prevents duplicate entries on pipeline re-runs
            import re as _re
            if _re.search(rf"^## v{_re.escape(version)} \(", existing, _re.MULTILINE):
                log.info("  [OK] Changelog entry for v%s already present - skipped", version)
                return True
...
def write_audit_record(version: str, branch: str, changes: list[str] | None = None,
                       trend_snapshot_captured: bool = False,
                       register_gate_passed: bool | None = None,
                       register_gate_status: str = "unknown") -> bool:
    """Write release audit record.

    Args:
        version: Release version.
        branch: Release branch name.
        changes: Optional change descriptions.
        trend_snapshot_captured: Whether a success-metrics trend snapshot was captured.
        register_gate_passed: Whether the register-pattern consistency gate
            passed at pre-release time (``None`` = gate not run, e.g. a
            standalone --audit record).
        register_gate_status: Gate status: aligned / drift / skipped / unavailable.
    """
    try:
        ...
        record = {
            ...
            "verified": False,
            "reproducible": True,
            "trend_snapshot_captured": bool(trend_snapshot_captured),
            "register_gate_passed": register_gate_passed,
            "register_gate_status": register_gate_status,
        }
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--capture-trend", action="store_true",
                    help="Capture a success-metrics trend snapshot only")
    ...
    # ── Success-metrics trend snapshot only ──────────────────────────────
    if args.capture_trend:
        ok = capture_trend_snapshot(version)
        return 0 if ok else 1
    ...
    print("\n[1/7] Pre-release validation...")
    failures = run_pre_release_checks(skip_certifications=args.skip_cert)
    if failures:
        print(f"  [X] {len(failures)} failure(s) - run --check for details")
        return 1
    print("  [OK] Passed")

    # Step 2: Capture success-metrics trend snapshot
    print("\n[2/7] Capturing success-metrics trend snapshot...")
    trend_ok = capture_trend_snapshot(version)
    if trend_ok:
        print("  [OK] Trend snapshot captured (MET-07/MET-08 evidence)")
    else:
        print("  [!] Trend capture failed - continuing (non-blocking evidence step)")

    # Step 3: Branch creation
    if not args.skip_branch:
        print("\n[3/7] Creating release branch...")
        ...
        branch = "current"
        print("\n[3/7] Skipping branch creation")

    # Step 4: Release notes
    print("\n[4/7] Generating release notes...")
    ok = write_release_notes(version, args.changes)
    if not ok:
        return 1

    # Step 5: Changelog
    print("\n[5/7] Updating changelog...")
    ...
    gate_verdict = _LAST_REGISTER_GATE_VERDICT or {}
    ok = write_audit_record(version, branch, args.changes,
                            trend_snapshot_captured=trend_ok,
                            register_gate_passed=gate_verdict.get("passed"),
                            register_gate_status=gate_verdict.get("status", "unknown"))
    if not ok:
        return 1

    # Step 7: Tag
    print("\n[7/7] Creating release tag...")
    ...
    # Push (opt-in - requires --push flag)
    if args.push:
        print("\nPushing to origin...")
        ...
        print("\nPush skipped (use --push to push to origin)")
    ...
    print(f"  Trend snapshot: {'captured' if trend_ok else 'failed/skipped'}")
    print(f"  Register gate: {gate_verdict.get('status', 'unknown')}")
    print("=" * 70)
