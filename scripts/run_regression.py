...
def _check_core_imports() -> str:
    ...
    assert AuditEngine and ConfigValidator and DataEngine and RetentionEngine and RiskConfig and SafetyEngine and StateManager
...
def _check_backtest_fixture_regression() -> str:
    from core import BacktestConfig, BacktestEngine, CsvReplaySource, ReplayConfig
    ...
    class _StrategyWrapper:
        """Minimal wrapper for backtest regression, replaces deprecated StrategyEngine."""
        def generate_signal(self, name: str, frames: dict, vix: float = 0.0):
            return fixture_strategy(name, frames, vix)
        def snapshot(self, name: str, sig: dict | None):
            return _RegSnap(
                name=name or "",
                score=float(sig.get("score", 0)) if sig else 0.0,
                threshold=float(sig.get("threshold", 60)) if sig else 60.0,
                direction=str(sig.get("direction", "")) if sig else "",
...
def main() -> int:
    ...
    if args.include_selftest:
        checks.extend(
            [
                ("index selftest", lambda: _run_selftest(INDEX_IMPL, "OPBUYING_INDEX_CONFIG", "json/config.json", args.selftest_timeout_sec)),
            ]
        )
