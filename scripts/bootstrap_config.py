...
def main(argv: list[str] | None = None) -> int:
    ...
    n = 0
    if index:
        if _copy_if_missing(ROOT / "json/index_config.defaults.json", ROOT / "json/config.json", label="index"):
            n += 1
    if stock:
        if _copy_if_missing(ROOT / "json/stock_config.defaults.json", ROOT / "json/stock_config.json", label="stock"):
            n += 1
    if n == 0 and not (ROOT / "json/config.json").exists() and not (ROOT / "json/stock_config.json").exists():
