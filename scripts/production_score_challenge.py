...
class ChallengeReport:
    ...
    def summary(self) -> str:
        lines = [
            ...
            f"  Challenges: {self.total} | [PASS] {self.passed} passed | [FAIL] {self.failed} failed | "
            f"[CRIT] {self.critical_failures} critical",
        ...
        if self.results:
            ...
            for r in self.results:
                if not r.passed:
                    icon = "[CRIT]" if r.severity == "CRITICAL" else "[FAIL]"
...
def _challenge_replay_determinism() -> ChallengeResult:
    ...
    try:
        ...
        report = cert.certify(db_path="db/trades.db", max_trades=5)
...
if __name__ == "__main__":
    ...
    if any_fail:
        ...
        print("[PASS] ALL SCORES VALIDATED against adversarial challenges")
        raise SystemExit(0)
    else:
        print("[WARN] Some scores reduced but no critical failures")
        raise SystemExit(1)
