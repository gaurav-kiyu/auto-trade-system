...
DEFAULT_DATABASES: list[str] = [
    ...
    "db/event_store.db",
    "db/execution_state.db",
    "db/order_state.db",
    "db/formal_order_state.db",
    "db/replay_sessions.db",
    "db/shadow_mode.db",
...
def run_database_maintenance(db_names: list[str] | None = None) -> int:
    """Run WAL checkpoint (TRUNCATE) and VACUUM across all target databases."""
    target_dbs = db_names or DEFAULT_DATABASES
    maintained = 0
    for db_name in target_dbs:
        src = Path(db_name)
        if not src.is_file() or not db_name.endswith(".db"):
            continue
        try:
            import sqlite3
            ...
            try:
                ...
                conn.execute("VACUUM")
                maintained += 1
                _log.info("[MAINTENANCE] Checkpointed & Vacuumed: %s", db_name)
            finally:
                conn.close()
        except Exception as exc:
            _log.warning("[MAINTENANCE] Maintenance skipped for %s: %s", db_name, exc)
    return maintained
...
def backup_databases(
    ...
    Args:
        ...
        run_maint: If True, executes WAL checkpoint and VACUUM prior to snapshot.
    ...
    if run_maint:
        run_database_maintenance(db_names)
        ...
        try:
            ...
            if db_name.endswith(".db"):
                ...
                for ext in ["-wal", "-shm"]:
                    ...
                    if wal_src.is_file():
                        wal_dst = snapshot_dir / (db_name + ext)
                        wal_dst.parent.mkdir(parents=True, exist_ok=True)
...
def _restore_latest(db_name: str, backup_dir: str = DEFAULT_BACKUP_DIR, force: bool = False) -> bool:
    ...
    Args:
        db_name: Database file name to restore (e.g., "db/trades.db").
...
def _get_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--maintenance", action="store_true",
                    help="Run WAL checkpoint (TRUNCATE) and VACUUM across all databases")
