...
# (running as `python scripts/generate_architecture_pdf.py` puts scripts/ on path).
...
def _count_tests() -> int:
    """Count test functions across the tests/ tree (fast, no pytest run)."""
    total = 0
    for py in (ROOT / "tests").rglob("test_*.py"):
        total += sum(
            1
...
def build_pdf(output_path: Path) -> None:
    ...
    story.append(Paragraph(f"Version 2.57.1 - {now_ist().strftime('%B %d, %Y')}", styles["Normal"]))
    ...
    layers = [
        ...
        (f"<b>Tests</b> (<i>tests/</i>) - {test_count:,} unit/integration/stress tests covering all components."),
    ]
    for lay in layers:
    ...
    story.append(Paragraph(
        f"{test_count:,} tests covering stress, catastrophic scenarios, failure "
    ...
    story.append(Spacer(1, 4 * mm))
    story.append(Paragraph(
        "The OPBuying v2.57.1 system demonstrates production-grade architecture in its execution "
