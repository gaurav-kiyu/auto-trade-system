...
results.append(("WS14 Config", all([exists("json/index_config.defaults.json"), exists("schemas/index_config.schema.json")])))
...
# 5. SBOM generation
sbom_target = grep_file("sbom:", "Makefile") and grep_file("release:", "Makefile") and grep_file("sbom", "Makefile")
print(f"  {'PASS' if grep_file('sbom', 'Makefile') else 'FAIL'} | SBOM generation in Makefile")
...
all_pass = all([
    ...
    hr_endpoint and hr_handler, grep_file("sbom", "Makefile"), rb >= 11,
])
if all_pass:
