...
def check_env_example_sync() -> list[str]:
    ...
    defaults_file = ROOT / "json/index_config.defaults.json"
...
def _test_covers_module(test_stem: str, module_basename: str) -> bool:
    """True if a test filename plausibly covers a module basename.

    Matches when the module basename appears as a contiguous underscore-
    separated token sequence inside the test name. This recognizes real-world
    test file names that embed the module name with extra suffixes or
    prefixes, e.g.::

        metrics_trend   -> test_metrics_trend_routes.py   (suffix)
        provisioning    -> test_self_service_provisioning.py  (prefix)

    Args:
        test_stem: Test filename without extension (e.g. ``test_foo``).
        module_basename: Module filename without extension (e.g. ``foo``).

    Returns:
        True when the test name contains the module tokens in order.
    """
    if not module_basename or not test_stem:
        return False
    # Fast path: full basename present anywhere in the test name.
    if module_basename in test_stem:
        return True
    module_tokens = module_basename.split("_")
    test_tokens = test_stem.split("_")
    n = len(module_tokens)
    if n == 0 or len(test_tokens) < n:
        return False
    return any(
        test_tokens[i:i + n] == module_tokens
...
def check_documentation_sync() -> list[str]:
    ...
    A module is covered when a test file exists with the exact name
    ``test_<basename>.py`` OR any test whose name embeds the module basename
    as a contiguous token sequence (``test_metrics_trend_routes.py`` covers
    ``metrics_trend.py``). This avoids false-positive ``MISSING_TEST`` flags
    for modules whose tests use descriptive suffixes/prefixes.
    """
    issues: list[str] = []
    ...
    core_dir = ROOT / "core"
    if core_dir.is_dir():
        all_test_files = sorted((ROOT / "tests").rglob("test_*.py"))
        for py_file in core_dir.rglob("*.py"):
            ...
            module_stem = rel.with_suffix('').name
            ...
            if not test_path.exists():
                ...
                if not alt_test_paths:
                    # Fall back to token-sequence matching so descriptive
                    # test names (e.g. test_metrics_trend_routes.py) count.
                    covering = [
                        p for p in all_test_files
                        if _test_covers_module(p.stem, module_stem)
                    ]
                    if not covering:
                        issues.append(
...
def check_config_drift() -> list[str]:
    ...
    template_path = ROOT / "json/config.template.json"
