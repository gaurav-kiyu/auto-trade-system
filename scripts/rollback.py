...
Performs automated rollback for 5 scenarios as documented in docs/archive/ROLLBACK_PLAN.md:
...
CONFIG_FILE = ROOT / "json/config.json"
CONFIG_DEFAULTS = ROOT / "json/index_config.defaults.json"
TRADES_DB = ROOT / "db/trades.db"
TRADE_JOURNAL_DB = ROOT / "db/trade_journal.db"
...
def _verify_paper_mode() -> tuple[bool, str]:
    ...
    """
    try:
        cfg_path = ROOT / "json/index_config.defaults.json"
        if cfg_path.exists():
...
def check_status() -> int:
    ...
    checks: list[tuple[str, bool, str]] = [
        ...
        ("Trader state file", STATE_FILE.exists(), "json/trader_state.json"),
        ("Config file", CONFIG_FILE.exists(), "json/config.json"),
        ("Config defaults", CONFIG_DEFAULTS.exists(), "json/index_config.defaults.json"),
        ("Trades database", TRADES_DB.exists(), "db/trades.db"),
        ("Trade journal DB", TRADE_JOURNAL_DB.exists(), "db/trade_journal.db"),
