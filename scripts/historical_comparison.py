...
class HistoricalComparer:
    ...
    # Max comparisons to prevent timeout with 500+ Python files
    _MAX_MODULE_COMPARISONS = 200
    _MAX_TIME_BUDGET_SEC = 90  # overall time budget for the compare method

    def __init__(self, cfg: dict[str, Any] | None = None):
        self._cfg = cfg or {}
        self._exclude_dirs = self._cfg.get(
            "historical_comparison_exclude_dirs",
            ["node_modules", "__pycache__", ".git", ".egg-info", "dist", "build", "docs/archive"],
        )
    ...
    def compare(self, source_rev: str, target_rev: str = "HEAD") -> ComparisonReport:
        ...
        if not self._is_git_repo():
            report.summary = "Not a git repository — comparison skipped (informational, not a failure)"
            report.has_regressions = False
            return report
        ...
        # Run diffs (order matters: fastest first, then critical only)
        report.diff_stat = self._compute_diff_stat(source_rev, target_rev)

        # Only run module diff if we have time budget remaining
        if time.time() - self._start_time < self._MAX_TIME_BUDGET_SEC:
            report.module_diff = self._compute_module_diff(source_rev, target_rev)
        else:
            report.module_diff = ModuleDiff()
    ...
    def _compute_module_diff(self, src: str, tgt: str) -> ModuleDiff:
        ...
        Performance: only compares critical source modules (core/, index_app/)
        up to MAX_MODULE_COMPARISONS to avoid 1000+ git subprocess calls.
        """
        diff = ModuleDiff()
        ...
        # Only compare critical source modules (not test/, docs/, scripts/)
        # to limit git subprocess calls and prevent timeout
        common_modules = sorted(
            f for f in (src_set & tgt_set)
            if f.startswith(("core/", "index_app/", "infrastructure/"))
        )
        compared = 0
        for mod in common_modules:
            if compared >= self._MAX_MODULE_COMPARISONS:
                break
            if time.time() - self._start_time >= self._MAX_TIME_BUDGET_SEC:
                break
            compared += 1
    ...
    def _compute_test_diff(self, src: str, tgt: str) -> FileDiffReport:
        ...
        diff.test_files_removed = sorted(set(src_tests) - set(tgt_tests))        # Count tests (approximately) in current state
        try:
            result = subprocess.run(
    ...
    def _compute_config_diff(self, src: str, tgt: str) -> ConfigDiff:
        ...
        defaults_files = ["json/index_config.defaults.json", "json/stock_config.defaults.json"]
    ...
    def _list_docs(self, rev: str) -> list[str]:
        ...
        Excludes files in archive directories (e.g. docs/archive/) since
        those are historical snapshots expected to reference removed modules.
        """
        output = self._run_git(["ls-tree", "-r", "--name-only", rev])
        if not output:
            return []
