...
from core import BacktestConfig, BacktestEngine, CsvReplaySource, ReplayConfig, ReplayEngine
...
class StrategyEngine:
    """Inline wrapper replicating the deleted StrategyEngine for backtest CLI."""
    def __init__(self, generate_signal_fn=None, top_signals_fn=None, detect_regime_fn=None, detect_regime_and_adx_fn=None):
        self._generate_signal_fn = generate_signal_fn or (lambda n, f, v=0.0: None)
    ...
    def top_signals(self, signals, limit=5):
        if self._top_signals_fn:
            return self._top_signals_fn(signals, limit)
        return signals[:limit]

    def detect_regime(self, name, frames):
        if self._detect_regime_fn:
            return self._detect_regime_fn(name, frames)
        return "NEUTRAL"

    def detect_regime_and_adx(self, name, frames):
        if self._detect_regime_and_adx_fn:
            return self._detect_regime_and_adx_fn(name, frames)
        return "NEUTRAL", 0.0
...
def main() -> int:
    ...
    parser.add_argument("--config", default="json/config.json", help="Config JSON path.")
