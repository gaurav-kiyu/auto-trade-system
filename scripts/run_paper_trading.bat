...
REM ── Locate Python (try "py" launcher first, then "python") ──────────────
...
if not defined PYTHON (
    echo [%TIMESTAMP%] ERROR: Neither "py" nor "python" was found in PATH. >> "%LOG_FILE%"
    exit /b 1
)
...
REM Append run record - a real script, not an inline multi-line python -c
REM (batch can't execute a multi-line quoted -c string; each line past the
REM first was silently being run as its own separate, failing batch command)
%PYTHON% "%PROJECT_DIR%\scripts\_record_paper_run.py" "%TRACKING_FILE%" !BOT_EXIT! >> "%LOG_FILE%" 2>&1
