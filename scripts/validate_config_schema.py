...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.all:
        index_paths = [p for p in (ROOT / "json/index_config.defaults.json", ROOT / "json/config.template.json") if p.is_file()]
        stock_paths = [p for p in (ROOT / "json/stock_config.defaults.json", ROOT / "json/stock_config.template.json") if p.is_file()]
    ...
    if args.path:
        ...
        cand = (
            (ROOT / "json/index_config.defaults.json", ROOT / "json/config.template.json")
            if args.flavour == "index"
            else (ROOT / "json/stock_config.defaults.json", ROOT / "json/stock_config.template.json")
        )
