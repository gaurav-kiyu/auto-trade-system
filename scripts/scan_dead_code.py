...
Usage:
    ...
    python scripts/scan_dead_code.py --quick                 # Quick CI mode (skips tests/scripts/infra + orphan+duplicate scans)
...
# Quick mode - excludes test files, scripts, and infra modules, and skips
# orphan/duplicate scans for faster CI runs
...
def _scan_unused_imports_in_file(py_file: Path) -> list[DeadCodeFinding]:
    """Scan a single file for unused imports. Returns findings list.

    Extracted as a standalone function so it can be run in parallel via
    ThreadPoolExecutor (I/O-bound file reads + GIL-releasing AST parsing).
    """
    relative = py_file.relative_to(ROOT)
    file_findings: list[DeadCodeFinding] = []
        ...
        for line, name in imported_names.items():
            ...
            if base_name not in used_names:
                ...
                if f"'{base_name}'" not in content and f'"{base_name}"' not in content:
                    file_findings.append(DeadCodeFinding(
...
def scan_unused_imports() -> list[DeadCodeFinding]:
    ...
    Uses ThreadPoolExecutor for parallel I/O-bound AST parsing across
    all Python files in the project (excluding EXCLUDED_DIRS).
    """
    files = _walk_files()
    findings: list[DeadCodeFinding] = []

    # Parallelize file processing — file I/O and AST parsing are
    # I/O-bound (file reads) + CPU-bound (AST parsing in C, GIL-releasing).
    # 4–8 workers balances throughput on typical CI machines.
    num_workers = min(8, len(files) or 1)
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_map = {executor.submit(_scan_unused_imports_in_file, f): f for f in files}
        for future in as_completed(future_map):
            try:
                findings.extend(future.result())
            except Exception as exc:
                log.debug("Error scanning %s: %s", future_map[future].name, exc)
...
def scan_orphaned_symbols() -> list[DeadCodeFinding]:
    ...
    Uses a reverse import index (imported-name -> importing files) so the scan
    is O(total_imports + total_exports) instead of the previous
    O(modules x exports x imports) triple loop, which hung on large repos.
    """
    findings: list[DeadCodeFinding] = []
    ...
    # Reverse index: last import segment -> files importing a name matching it.
    # This captures both ``imp == name`` and ``imp.endswith(f".{name}")``
    # (an import whose dotted path ends in the exported name).
    importers_by_name: dict[str, set[Path]] = defaultdict(set)
    for import_file, import_names in find_all_imports().items():
        for imp in import_names:
            importers_by_name[imp.rsplit(".", 1)[-1]].add(import_file)
    ...
    for module_path, exports in module_exports.items():
        ...
        for name in sorted(exports):
            ...
            importers = importers_by_name.get(name, set())
            is_imported = any(f != module_path for f in importers)
...
def scan_duplicate_code() -> list[DuplicateFinding]:
    ...
    # Only flag symbols that appear in EXACTLY 2 files - more than 2 means it's
    # a common naming convention (e.g., _cli, to_dict, summary), not a duplicate.
    for name, locations in symbol_locations.items():
        if len(locations) == 2 and not name.startswith("_"):
            ...
            if name.startswith("test_"):
                continue
            file_a, line_a = locations[0]
            file_b, line_b = locations[1]
            # Skip if either file is in the tests/ directory - test files
            # naturally mirror production code and are not real duplicates
            rel_a = file_a.relative_to(ROOT)
            rel_b = file_b.relative_to(ROOT)
            if "tests" in rel_a.parts or "tests" in rel_b.parts:
                continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--quick", "-q", action="store_true",
                    help="Quick mode: exclude tests/, scripts/, infrastructure/ and skip orphan+duplicate scans")
    ...
    try:
        ...
        if args.check_orphans or (run_all and not args.quick):
