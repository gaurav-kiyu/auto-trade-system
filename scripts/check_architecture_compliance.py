...
CORE_NO_INFRA_MODULES = {
    ...
    "core.alert_router",
    # ics_telegram_bridge bridges Incident Commander to Telegram
...
# Strategy modules EXEMPT from the no-broker-direct-import rule.
# These modules may reference broker adapter names in docstrings or
# configuration descriptions without violating separation of concerns.
STRATEGY_BROKER_EXEMPT_MODULES = {
    # multi_asset_dispatcher references PaperBrokerAdapter in docstring examples
...
# Memoization caches.  Keyed on (ROOT, _SOURCE_DIRS) / file path so that tests
# that patch ROOT or _SOURCE_DIRS (temp-project fixtures) always see fresh
# results and never leak cached scans from the real tree.
_SOURCE_FILES_CACHE: dict[tuple[str, str], list[Path]] = {}
...
def _source_files() -> list[Path]:
    ...
    Memoized: the scan runs once per (ROOT, _SOURCE_DIRS) combination because
    the same tree is walked by three different checks on every invocation.
    """
    cache_key = (str(ROOT), "\0".join(str(p) for p in _SOURCE_DIRS))
    cached = _SOURCE_FILES_CACHE.get(cache_key)
    ...
    result = sorted(files)
    _SOURCE_FILES_CACHE[cache_key] = result
    return result
...
def _list_imports(filepath: Path) -> list[str]:
    ...
    Memoized per file so the same file is only read + parsed once per run.
    """
    key = str(filepath)
            ...
            if node.module:
                ...
                for alias in node.names:
                    if node.module:
                        ...
                        imports.append(full)
    _IMPORTS_CACHE[key] = imports
    return imports


def _module_exists_on_disk(mod_path: str) -> bool:
    """Resolve a dotted module path (e.g. ``core.services.risk_service``) to a
    file under ROOT and report whether it exists — without importing anything.

    Used by :func:`check_canonical_modules_importable` as a fast path.  Unlike
    ``importlib.util.find_spec`` it does not import parent packages, so it
    avoids the heavy ``core/__init__`` chain (pandas/lightgbm/shap) that made
    the canonical check take seconds instead of milliseconds.
    """
...
def check_strategy_no_broker_imports() -> list[str]:
    ...
    for pyfile in _source_files():
        mod = _module_name_from_file(pyfile)
        # Skip exempt modules
        if mod in STRATEGY_BROKER_EXEMPT_MODULES:
            continue
...
def check_canonical_modules_importable() -> list[str]:
    ...
    Fast path resolves the dotted path to a file under ROOT without importing
    anything (avoids heavy ``core/__init__`` side effects).  Falls back to
    ``importlib.util.find_spec`` only when the file is not found under ROOT,
    preserving behavior for modules resolved via sys.path elsewhere.
    """
    violations: list[str] = []
    for mod_path in REQUIRED_CANONICAL:
        if _module_exists_on_disk(mod_path):
            continue
