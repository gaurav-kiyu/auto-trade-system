...
- Environment variables take precedence over json/config.json values

## Backwards Compatibility

The system maintains backwards compatibility:
- If environment variables are not set, it will fall back to json/config.json values (which are now empty)
- Existing configurations that rely on json/config.json will need to be migrated to use environment variables
