...
#     See /VERSION for the current authoritative version — this header is
#     not auto-synced and has drifted before; don't trust it over /VERSION.
# ----------------------------------------------------------------
...
# Fix Windows charm codec for Unicode log characters (e.g., →, ◆, •)
if sys.platform == "win32" and isinstance(sys.stdout, io.TextIOWrapper):
    try:
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
    except (AttributeError, ValueError):
        pass
if sys.platform == "win32" and isinstance(sys.stderr, io.TextIOWrapper):
    try:
...
def send(message: str, critical: bool = False, **kwargs) -> None:
    ...
    impl = globals().get("_send_impl")
    if impl is None:
        # Not wired yet (early import / config-failure path): drop safely
        # rather than raising NameError during module import.
        return
    impl(message, critical=critical, **kwargs)
...
# CLI flag detection (documented in header, wired at import time so the
# module-level config load below respects it):
#   python -m index_app.index_trader --paper   ->  force PAPER mode
#   python -m index_app.index_trader --debug   ->  verbose DEBUG logging
_CLI_PAPER_MODE = "--paper" in sys.argv
if "--debug" in sys.argv:
    logging.getLogger("index_app").setLevel(logging.DEBUG)
...
def _apply_config_globals(cfg: dict[str, Any]) -> None:
    ...
    apply_execution_mode(cfg, cli_paper=_CLI_PAPER_MODE, infer_blank_from_broker=True)
...
def _set_config_fail_safe():
    ...
    With ``--paper`` the fail-safe still forces PAPER (never MANUAL), so
    the documented "--paper forces PAPER" contract holds even when the
...
RECONCILE_HALT_ON_QTY_MISMATCH = bool(_CFG.get("RECONCILE_HALT_ON_QTY_MISMATCH", True))
ADAPTIVE_THRESHOLD_ENABLED = bool(_CFG.get("ADAPTIVE_THRESHOLD_ENABLED", True))
MAX_POSITION_AGE = 9999

SIGNAL_CFG = {"SIGNAL_TS_MAX_AGE": int(_CFG.get("SIGNAL_TS_MAX_AGE", 300))}
...
# MandateEnforcer fallback is retired (v2.54 - RiskService handles all mandate checks)
# Legacy import removed in favor of RiskService via MandateService
...
# (config key EXPIRY_CUTOFF_HOUR, default 13 - matches the class's own
# hardcoded BLOCK_AFTER=13:00 exactly, so this is a no-behavior-change fix
# that makes the "configurable cutoff" the comment above already promised
# actually real).
...
_expiry_controller = ExpiryDayController(
    ...
    block_after=_dt_time(int(_CFG.get("EXPIRY_CUTOFF_HOUR", 13)), 0),
)
...
_audit_engine = AuditEngine(
    path=_CFG.get("AUDIT_LOG_PATH", "json/audit_trail.jsonl"),
...
def _trip_hard_halt(reason="Unknown"):
    # cfg=_CFG opts this call site into the STRUCTURED_EVENTS_ENABLED wire-in
    # inside trip_hard_halt() (core/safety_state.py); default False = no-op.
    trip_hard_halt(reason, source="index_trader", cfg=_CFG)
...
def _apply_execution_mode(cfg):
    return apply_execution_mode(cfg, cli_paper=_CLI_PAPER_MODE, infer_blank_from_broker=True)
...
def _broker_order_followup_enabled() -> bool:
    """Legacy stub: broker order followup is disabled in MANUAL mode."""
    return not MANUAL_SIGNALS_ONLY
...
_last_qty_mismatch_check_ts = 0.0

def _periodic_reconcile():
    """Periodic reconciliation. Delegates to trading.reconciliation (DEBT-008).

    Also runs the qty-mismatch hard-halt check (_reconcile_positions_live),
    throttled to RECONCILE_INTERVAL seconds - previously that function was
    never called from the live loop at all, so RECONCILE_HALT_ON_QTY_MISMATCH
    had nothing to trigger it despite being read from config correctly.
    """
    ...
    global _last_qty_mismatch_check_ts
    _recon_interval = float(_CFG.get("RECONCILE_INTERVAL", 90))
    _now_ts = time.time()
    if _now_ts - _last_qty_mismatch_check_ts >= _recon_interval:
        _last_qty_mismatch_check_ts = _now_ts
...
INDEX_MAP: dict = {
    # Indices (only actively traded ones — MIDCPNIFTY ^NSEMIDCAP delisted)
    "NIFTY": {"yf": "^NSEI"},
...
RISK_MODE = "FIXED"  # dead - unused anywhere; real RISK_MODE control is RiskServiceConfig.risk_mode (default "PERCENT", via index_app/domains/trading/container.py)
RISK_FIXED_AMOUNT = 500  # dead - unused anywhere; real RISK_FIXED_AMOUNT control is RiskServiceConfig.risk_fixed_amount
_raw_mdl = _CFG.get("MAX_DAILY_LOSS", -2000)
MAX_DAILY_LOSS = -abs(float(_raw_mdl))  # always negative (loss limit)
PORTFOLIO_MAX_SL_RISK_PCT = 0.75  # dead - unused anywhere; real PORTFOLIO_MAX_SL_RISK_PCT control is RiskServiceConfig.portfolio_max_sl_risk_pct
...
BROKERAGE_PER_TRADE = float(_CFG.get("BROKERAGE_PER_TRADE", 40))
PARTIAL_EXIT_ENABLED = False  # dead - unused anywhere; real PARTIAL_EXIT_ENABLED control is core/execution_policy.py's config.get("PARTIAL_EXIT_ENABLED", True) global override on TierRules' per-tier partial exits
...
def _run_trading_loop() -> None:
    ...
    if _CFG.get("web_dashboard_enabled", False):
        try:
            ...
            maybe_start_dashboard(_CFG, state_path="json/trader_state.json", db_path="db/trades.db")
        except Exception as _dash_exc:
            log.warning("[MAIN] Web dashboard startup error: %s", _dash_exc)
...
def main() -> None:
    ...
    # DB Schema Migration - ensure database version is current
    try:
        from core.db_migration import ensure_schema_version
        ensure_schema_version("db/trades.db")
        log.info("[DB_MIGRATION] Schema version validated")
    except (ImportError, ValueError, TypeError, KeyError, RuntimeError, OSError) as _mig_err:
        log.warning("[DB_MIGRATION] Schema migration skipped: %s", _mig_err)

    # Data Governance - start background cleanup scheduler
    try:
        from core.data_governance import CleanupScheduler, DataGovernor
        _data_gov_internal = DataGovernor(
            cfg=dict(config),
        )
        _cleanup_sched_internal = CleanupScheduler(
            governor=_data_gov_internal,
            interval_hours=24,
        )
        _cleanup_sched_internal.start()
        log.info("[DATA_GOV] Cleanup scheduler started (24h interval)")
    except (ImportError, ValueError, TypeError, KeyError, RuntimeError, OSError, AttributeError) as _gov_err:
        log.warning("[DATA_GOV] Cleanup scheduler skipped: %s", _gov_err)

    # Log Multi-Asset Strategy Dispatcher registered engines
    try:
        ...
        if dispatcher is not None:
            ...
            engines = status.get("registered_engines", {})
            active = [f"{ac}->{info.get('name','?')}" for ac, info in engines.items()]
            log.info("[DISPATCH] Multi-Asset Dispatcher ready: %s", ", ".join(active))
    except (ImportError, ValueError, TypeError, AttributeError) as _disp_err:
        log.debug("[DISPATCH] Dispatcher not available: %s", _disp_err)

    # Constitution System Startup — initialize all 13 modules and wire together
    try:
        from core.startup import startup_constitution_system
        log.info("[MAIN] Initializing Constitution v4.0 system...")
        startup_result = startup_constitution_system(cfg=dict(config))
        meta = startup_result.get("_meta", {})
        log.info(
            "[CONSTITUTION] %d/%d modules initialized, %d failed (%.2fs)",
            meta.get("modules_initialized", 0),
            len(startup_result) - 1,
            meta.get("modules_failed", 0),
            meta.get("duration_sec", 0.0),
        )
    except Exception as _cs_err:
        log.warning("[CONSTITUTION] System startup failed: %s", _cs_err)

    # Start Admin Control Plane after canonical DI initialization.
    # This resolves the already-created RiskService through RiskPort.
