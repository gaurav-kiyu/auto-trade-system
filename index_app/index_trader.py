...
def _run_trading_loop() -> None:
    ...
    while not _shutdown.is_set():
        ...
        try:
            ...
            except (ValueError, TypeError, KeyError) as _oi_err:
...
def main() -> None:
    ...
    try:
        from core.constitution_ai_gate import get_gate
        _ai_gate = get_gate(identity="index_trader")
        _gate_result = _ai_gate.validate(
        ...
        if _gate_result.passed:
            log.info("[AI_GOVERNANCE] Gate: %s", _gate_result.reason)
        else:
            log.warning("[AI_GOVERNANCE] Gate had %d warnings — %s",
                        len(_gate_result.failures), _gate_result.detail)
            for _f in _gate_result.failures:
                log.warning("[AI_GOVERNANCE]   - %s", _f)
        
            # Fail-closed enforcement: if governance enforcement is enabled
            # and gate has blocking failures, block startup with SystemExit
            if config.get_bool("ai_governance_enforce_fail_closed", False):
                log.critical("[AI_GOVERNANCE] FAIL-CLOSED: Gate validation failed - blocking startup")
                raise SystemExit(88)
        _audit_engine.record("ai_governance_gate",
                             passed=_gate_result.passed,
                             failures=len(_gate_result.failures))
    except SystemExit:
        raise
    except Exception as _gate_err:
        log.debug("[AI_GOVERNANCE] Gate check skipped: %s", _gate_err)
