...
def setup_di_container(
    ...
    idem_db_path = cfg.get("idempotency_db_path", "db/execution_state.db")
    ...
    execution_service = ExecutionService(
        ...
        config=ExecutionServiceConfig(
            idempotency_db_path=idem_db_path,
            slippage_model_calibration_enabled=bool(cfg.get("slippage_model_calibration_enabled", False)),
            trade_journal_db_path=str(cfg.get("ml_journal_path", "db/trade_journal.db")),
            # config key ORDER_PLACE_RETRIES (default 3, matching this
            # field's own dataclass default - no behavior change). Currently
            # has no observable effect either way: create_state_handler()'s
            # BrokerStateHandler has no live consumer in execute_order()'s
            # real order-placement path yet.
            max_retries=int(cfg.get("ORDER_PLACE_RETRIES", 3)),
            order_fill_timeout_sec=int(cfg.get("ORDER_FILL_TIMEOUT_SEC", 10)),
        ),
    ...
    # Wire Auto-Hedger to the same canonical ExecutionService used by
    # the trading application. This prevents the hedge path from creating
    # a separate execution stack or bypassing execution governance.
    try:
        from core.risk.auto_hedger import get_portfolio_auto_hedger
        get_portfolio_auto_hedger(execution_service)
        _log.info("Auto-Hedger wired to canonical ExecutionService")
    except (ImportError, AttributeError, TypeError) as exc:
        _log.warning("Auto-Hedger wiring skipped: %s", exc)
    ...
    _risk_config = RiskServiceConfig(
        ...
        max_drawdown_pct=float(cfg.get("MAX_DRAWDOWN", 0.20)),
        drawdown_size_scale_enabled=bool(cfg.get("DRAWDOWN_SIZE_SCALE", True)),
        vix_size_scale_enabled=bool(cfg.get("VIX_SIZE_SCALE", True)),
        portfolio_max_sl_risk_pct=float(cfg.get("PORTFOLIO_MAX_SL_RISK_PCT", 0.75)),
        max_single_trade_loss_pct=float(cfg.get("MAX_SINGLE_TRADE_LOSS_PCT", 0.08)),
        vix_halt_threshold=float(cfg.get("VIX_HALT_THRESHOLD", 30.0)),
        # config keys RISK_MODE/RISK_FIXED_AMOUNT/RISK_PER_TRADE/MAX_LOT_CAPITAL_PCT.
        # default_risk_per_trade default kept at 0.02 (not RISK_PER_TRADE's
        # own stored default of 0.03) to match today's real sizing exactly;
        # risk_mode defaults to "PERCENT" (not RISK_MODE's own stored
        # default of "FIXED") for the same reason - see json/index_config.defaults.json.
        default_risk_per_trade=float(cfg.get("RISK_PER_TRADE", 0.02)),
        risk_mode=str(cfg.get("RISK_MODE", "PERCENT")).upper(),
        risk_fixed_amount=float(cfg.get("RISK_FIXED_AMOUNT", 150.0)),
        max_lot_capital_pct=float(cfg.get("MAX_LOT_CAPITAL_PCT", 0.85)),
    )
    ...
    # cfg wired through so send() can consult the TG_QUIET_MODE/TG_TRADE_ONLY/
    # TG_TRADE_ALERTS_STRICT/TG_CACHE_TTL_SEC filters (core/notification_filters.py),
    # all inert unless notification_filters_enabled is explicitly turned on.
    ...
    # Canonical signal engine for the authoritative strategy path.
    # Do not wire the deprecated global SignalOrchestrator here.
    _signal_evaluator = SignalEvaluator(config=cfg)

    _strategy_orchestrator = StrategyOrchestrator(
        signal_orchestrator=_signal_evaluator,
        config=cfg,
    )
...
def _initialize_position_service(
    ...
    _log.info(
        ...
        "broker_api_enabled=%s -> %s",
        position_service._manual_signals_only, position_service._execution_mode,
        position_service._broker_api_enabled,
        "AUTO paper fills"
        if not (position_service._manual_signals_only
                or position_service._execution_mode.upper() in ("MANUAL", "MANUAL_ONLY", "SIGNAL_ONLY", "SIGNALS_ONLY"))
