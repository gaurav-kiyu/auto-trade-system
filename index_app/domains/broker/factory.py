    ...
    * Delegate to ``core.adapters.broker_adapters.create_broker_adapter_with_runtime_context``
    ...
    def create_broker(
        ...
        *,
        ...
        except (ImportError, ValueError, TypeError, OSError, ConnectionError) as exc:
            # ImportError included: a missing broker SDK (e.g. kiteconnect)
            # must degrade to paper mode, never crash the trading loop.
...
def build_broker_executor(
    cfg: dict[str, Any],
    *,
    paper_mode: bool = True,
    log_fn: Callable[[str], None] | None = None,
) -> tuple[Callable, Callable]:
    """Build entry/exit execution callbacks that route through a broker adapter.

    Creates a ``PaperBrokerAdapter`` (paper mode) or a real ``BrokerAdapter``
    and wraps ``place_order``/``exit_order`` as callbacks compatible with
    ``EquityTrader``, ``FuturesTrader``, ``CommodityTrader``, and
    ``CurrencyTrader``.

    The returned callbacks have signatures:
        execute_entry_fn(symbol, direction, qty, price) -> bool
        execute_exit_fn(symbol, qty, price) -> None

    Args:
        cfg: Configuration dict.
        paper_mode: If True (default), use PaperBrokerAdapter.
        log_fn: Optional logging function.

    Returns:
        Tuple of (execute_entry_fn, execute_exit_fn).
    """
    ...
    try:
        ...
        if paper_mode:
            adapter = PaperBrokerAdapter(cfg=cfg)
            _log_fn("[BROKER_EXEC] Using PaperBrokerAdapter for execution")
        else:
            # Try real broker via factory
            factory = BrokerFactory(
                cfg=cfg,
                index_map=cfg.get("INDEX_MAP", {}),
                manual_signals_only=False,
                broker_api_enabled=cfg.get("BROKER_API_ENABLED", False),
                paper_mode=False,
                execution_mode=cfg.get("EXECUTION_MODE", "AUTO"),
            )
            adapter = factory.create_broker(log_fn=_log_fn)
            _log_fn("[BROKER_EXEC] Using real broker adapter: %s", type(adapter).__name__)
    except (ImportError, ValueError, TypeError, OSError) as exc:
        _log_fn(f"[BROKER_EXEC] Failed to create broker adapter: {exc} - using no-op")
        adapter = None

    def execute_entry(symbol: str, direction: str, qty: int, price: float) -> bool:
        """Execute entry via broker adapter."""
        if adapter is None:
            _log_fn(f"[EXEC] No broker adapter - simulated entry {symbol} {direction} x{qty} @ {price:.2f}")
            return True
        try:
            if hasattr(adapter, "place_order"):
                result = adapter.place_order(symbol, direction, qty, 0)
                if result:
                    _log_fn(f"[EXEC] Entry order placed: {symbol} {direction} x{qty} (order_id={result})")
                    return True
                else:
                    _log_fn(f"[EXEC] Entry order failed: {symbol} {direction}")
                    return False
            # Fallback for BrokerAdapter wrapper
            _log_fn(f"[EXEC] Simulated entry {symbol} {direction} x{qty} @ {price:.2f}")
            return True
        except (ValueError, TypeError, OSError, ConnectionError) as exc:
            _log_fn(f"[EXEC] Entry error {symbol}: {exc}")
            return False

    def execute_exit(symbol: str, qty: int, price: float) -> None:
        """Execute exit via broker adapter."""
        if adapter is None:
            _log_fn(f"[EXEC] No broker adapter - simulated exit {symbol} x{qty} @ {price:.2f}")
            return
        try:
            if hasattr(adapter, "exit_order"):
                adapter.exit_order(symbol, "SELL", qty, 0)
                _log_fn(f"[EXEC] Exit order placed: {symbol} x{qty}")
            else:
                _log_fn(f"[EXEC] Simulated exit {symbol} x{qty} @ {price:.2f}")
        except (ValueError, TypeError, OSError, ConnectionError) as exc:
            _log_fn(f"[EXEC] Exit error {symbol}: {exc}")

    return execute_entry, execute_exit


__all__ = [
    "BrokerFactory",
    "build_broker_executor",
    "make_broker",
]
