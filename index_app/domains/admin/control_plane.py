...
def init_admin_control_plane(
    ...
    wal = WriteAheadJournal(db_path=cfg.get("wal_journal_db_path", "db/wal_journal.db"))
    certifier = IdempotencyCertifier(
        db_path=cfg.get("idempotency_db_path", "db/execution_state.db"),
    ...
    except (ImportError, ValueError, TypeError, KeyError, AttributeError, IndexError, OSError):
    ...
    try:
        ...
        model_registry = ModelRegistry(
            db_path=cfg.get("model_registry_db_path", "db/model_registry.db")
        )
    except (ImportError, ValueError, TypeError, KeyError, AttributeError, IndexError, OSError):
    ...
    # Resolve the already-created canonical RiskService from DI.
    # Never construct a second RiskService for the Admin Control Plane.
    from core.di_container import get_container
    from core.ports.risk.risk_port import RiskPort

    _container = get_container()
    _risk_service = _container.try_resolve(RiskPort)

    if _risk_service is None:
        raise RuntimeError(
            "Canonical RiskService unavailable; refusing to start "
            "Admin Control Plane with runtime risk mutation enabled."
        )
