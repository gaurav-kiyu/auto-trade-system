if not _TK_AVAILABLE or ttk is None:
    ...
    _gui_logger = LoggingService(
        ...
        json_log_file="",
        version="UNKNOWN",
        enable_correlation_ids=False,  # GUI doesn't need correlation IDs typically
...
mode_lbl=tk.Label(right_h,text=" PAPER ",font=(_FONT_UI,10,"bold"),bg=_GT.get("mode_paper","#9e6a03"),fg="#ffffff",padx=8,pady=2)
mode_lbl.pack(side=tk.RIGHT,padx=(8,0))
status_lbl=tk.Label(right_h,text=" STARTING ",font=(_FONT_UI,10,"bold"),bg=_GT.get("heading_bg","#21262d"),fg=_GT.get("warn","#f0883e"),padx=10,pady=2)
...
c0,v_time,v_time_sub=_card(kpi_row,"Last update",True);c0.grid(row=0,column=0,padx=(0,6),sticky=tk.EW)
c1,v_cap,v_cap_sub=_card(kpi_row,"Capital",True);c1.grid(row=0,column=1,padx=6,sticky=tk.EW)
c2,v_pnl,v_pnl_sub=_card(kpi_row,"Day P&L",True);c2.grid(row=0,column=2,padx=6,sticky=tk.EW)
c3,v_tr=_card(kpi_row,"Trades / Open slots");c3.grid(row=0,column=3,padx=(6,0),sticky=tk.EW)
...
lbl_manual_flow=tk.Label(desk_fr,text="",font=(_FONT_UI,9),bg=_GT.get("desk_bg","#0d1117"),fg=_GT.get("accent_soft","#79c0ff"),
                         anchor="w",justify="left",wraplength=int(_GU.get("default_wraplength_desk",1040)))
...
lbl_headline=tk.Label(info_row,text="Waiting for first scan…",font=(_FONT_UI,11,"bold"),bg=_GT.get("heading_bg","#21262d"),fg=_GT.get("fg_label","#c9d1d9"),
...
lbl_tg=tk.Label(sub_info,text="",font=(_FONT_UI,9),bg=bg_main,fg=_GT.get("accent_soft","#79c0ff"),anchor="w")
lbl_tg.pack(fill=tk.X)
lbl_api=tk.Label(sub_info,text="",font=(_FONT_UI,9),bg=bg_main,fg=_GT.get("warn","#d29922"),anchor="w")
lbl_api.pack(fill=tk.X)
lbl_last_tg=tk.Label(sub_info,text="",font=(_FONT_UI,8),bg=bg_main,fg=_GT.get("tg_muted","#bc8cff"),anchor="w",wraplength=1080,justify="left")
lbl_last_tg.pack(fill=tk.X,pady=(4,0))
lbl_gui_err=tk.Label(sub_info,text="",font=(_FONT_UI,8),bg=bg_main,fg=_GT.get("err","#f85149"),anchor="w",wraplength=1080,justify="left")
...
tv=ttk.Treeview(tbl_fr,columns=cols,show="headings",height=8,style="Gui.Treeview",selectmode="none",takefocus=0)
tv.heading("idx",text="Index");tv.heading("price",text="Price");tv.heading("pct",text="Day %")
tv.heading("dir",text="Dir");tv.heading("score",text="Score");tv.heading("thr",text="Thr");tv.heading("gap",text="Gap")
tv.heading("adx",text="ADX");tv.heading("iv",text="IV")
tv.heading("reg",text="Reg")
tv.heading("st",text="Status / gate")
tv.column("idx",width=82,stretch=False);tv.column("price",width=86,stretch=False);tv.column("pct",width=58,stretch=False)
tv.column("dir",width=34,stretch=False)
tv.column("score",width=46,stretch=False);tv.column("thr",width=40,stretch=False);tv.column("gap",width=40,stretch=False)
tv.column("adx",width=42,stretch=False);tv.column("iv",width=40,stretch=False)
tv.column("reg",width=56,stretch=False);tv.column("st",width=280,stretch=True)
...
tk.Label(wait_fr,text="Why entries are blocked (per index)",font=(_FONT_UI,9,"bold"),bg=bg_card,fg="#f0883e").pack(anchor="w",padx=8,pady=(6,2))
...
tk.Label(pre_fr,text="Closest watchlist setup (not an entry yet)",font=(_FONT_UI,9,"bold"),bg=bg_card,fg="#79c0ff").pack(anchor="w",padx=8,pady=(6,2))
pre_lbl=tk.Label(pre_fr,text="Waiting for first scan…",bg=bg_card,fg=_GT.get("fg_label","#c9d1d9"),font=(_FONT_UI,9),wraplength=int(_GU.get("wait_wrap_max",520)),justify="left")
...
tw=tk.Text(detail_fr,font=(_FONT_MONO,9),bg=_GT.get("tree_bg","#0d1117"),fg=_GT.get("tree_fg","#c9d1d9"),wrap=tk.WORD,state=tk.DISABLED,
           borderwidth=0,padx=10,pady=8,insertbackground=accent,selectbackground="#1f6feb",takefocus=1,
...
detail_fr.bind("<Button-4>",lambda e: tw.yview_scroll(-2,"units"))
detail_fr.bind("<Button-5>",lambda e: tw.yview_scroll(2,"units"))
tw.bind("<Button-4>",lambda e: tw.yview_scroll(-2,"units"))
tw.bind("<Button-5>",lambda e: tw.yview_scroll(2,"units"))
tv.bind("<Button-4>",lambda e: tv.yview_scroll(-2,"units"))
tv.bind("<Button-5>",lambda e: tv.yview_scroll(2,"units"))
tbl_fr.bind("<Button-4>",lambda e: tv.yview_scroll(-2,"units"))
tbl_fr.bind("<Button-5>",lambda e: tv.yview_scroll(2,"units"))
...
tw.bind("<Prior>",_tw_pgup)
tw.bind("<Next>",_tw_pgdn)
tw.bind("<Control-Home>",_tw_scroll_top)
tw.bind("<Control-End>",_tw_scroll_bottom)
...
def _open_find_details()->None:
    ...
    tk.Button(btnr,text="Close",command=_close_find,bg="#21262d",fg="#c9d1d9",activebackground="#30363d").pack(side=tk.LEFT)
...
def _init_pan_sash():
    try:
        ...
        _psmin=int(_GW.get("pane_sash_min",220));_psmax=int(_GW.get("pane_sash_max",1400))
        _pdsmin=int(_GW.get("pane_sash_default_min",380));_pdsmax=int(_GW.get("pane_sash_default_max",520))
...
_last_gui_tw_text=[None]

def _cancel_sched_after()->None:
    if sched_after[0] is not None:
        try: root.after_cancel(sched_after[0])
...
def _apply_colors(widget,text):
    ...
    for line in text.split("\n"):
        tag=None
        ll=line.strip()
        if ll.startswith("\u2550"): tag="separator"
        elif ll.startswith("\u2500"): tag="separator"
        elif "INDEX OPTION TRADER" in line: tag="header"
        elif "Capital:" in line and "P&L:" in line: tag="header"
        elif "ACTION ALERTS" in line: tag="signal_buy"
        elif "WATCHLIST" in line: tag="signal_watch"
        elif "OPEN POSITIONS" in line: tag="alert"
        ...
        elif "\u25b2" in line and "+" in line: tag="profit"
...
def _update():
    ...
    try:
        ...
        if struct_ok:
            ...
            try:
                ...
                if isinstance(b,dict) and b:
                    ...
                    top_map={
                        ...
                        "RR":"Top block: RR",
                        "ADX":"Top block: ADX",
                        "SCORE":"Top block: Score",
                        "COOLDOWN":"Top block: Cooldown",
                        "DATA":"Top block: Data",
                        "CIRCUIT":"Top block: Circuit",
                        "CAPS":"Top block: Caps",
            ...
            tg_bits=[]
            _tga=int(_GU.get("tg_display_action_sec",7200));_tgf=int(_GU.get("tg_display_filter_sec",1800))
            ...
            try:
                ...
                if isinstance(ps,dict) and ps.get("name"):
                    ...
                    price  =ps.get("price")
                    sl     =ps.get("stop_loss")
                    tp1    =ps.get("tp1"); tp2=ps.get("tp2"); tp3=ps.get("tp3")
                    lot    =ps.get("lot",0); step=ps.get("step",50)
                    ...
                    if isinstance(tp2,(int,float)) and tp2>0 and isinstance(price,(int,float)) and price>0:
                        ...
                        lines.append(f"TP 2        {R_SYM}{tp2:,.1f}  ({t2p:+.1f}%)  \u2190 primary")
