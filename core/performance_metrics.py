...
def load_trades(
    ...
    try:
        ...
        # Check if trades or execution_orders table exists
        table_check = conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='trades'").fetchone()
        if table_check:
            table_name = "trades"
            cols_info = [r[1] for r in conn.execute(f"PRAGMA table_info({table_name})").fetchall()]
            time_col = "ts" if "ts" in cols_info else ("timestamp" if "timestamp" in cols_info else "id")

            q_clauses, q_params = [], []
            if mode and "mode" in cols_info:
                q_clauses.append("mode = ?")
                q_params.append(mode.upper())
            if days and time_col in ["ts", "timestamp", "created_at"]:
                since = (now_ist() - timedelta(days=days)).isoformat()
                q_clauses.append(f"{time_col} >= ?")
                q_params.append(since)
            where_sql = ("WHERE " + " AND ".join(q_clauses)) if q_clauses else ""

            rows = conn.execute(
                f"SELECT * FROM {table_name} {where_sql} ORDER BY {time_col}", q_params,  # nosec B608
            ).fetchall()
        else:
            table_check_exec = conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='execution_orders'").fetchone()
            if table_check_exec:
                table_name = "execution_orders"
                new_clauses, new_params = [], []
                if clauses:
                    for c, p in zip(clauses, params):
                        if "ts" in c or "timestamp" in c:
                            new_clauses.append("created_at >= ?")
                            new_params.append(p)
                        else:
                            new_clauses.append(c)
                            new_params.append(p)
                where_sql = ("WHERE " + " AND ".join(new_clauses)) if new_clauses else ""
                rows = conn.execute(
                    f"SELECT * FROM {table_name} {where_sql} ORDER BY created_at", new_params,  # nosec B608
                ).fetchall()
