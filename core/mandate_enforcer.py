...
class ProductionMandateEnforcer:
    ...
    def update_market(self, vix: float, data_age_seconds: int) -> None:
        self._state.vix = vix
        self._state.data_stale_seconds = data_age_seconds

    def update_capital(self, capital: float, daily_pnl: float, weekly_pnl: float) -> None:
        self._state.capital = capital
        self._state.daily_pnl = daily_pnl
        self._state.weekly_pnl = weekly_pnl
        self._state.equity_peak = max(self._state.equity_peak, capital)
...
def get_mandate_enforcer(config: dict | None = None) -> ProductionMandateEnforcer:
...
def reset_mandate_enforcer() -> None:
    """For testing / restart"""
    global _production_enforcer
