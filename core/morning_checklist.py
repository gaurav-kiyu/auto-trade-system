...
class MorningChecklist:
    ...
    def _check_vix_loaded(self) -> tuple[bool, str]:
        ...
        try:
            from core.iv_rank import get_iv_rank as _get_iv_rank
            vix = _get_iv_rank()._vix if hasattr(_get_iv_rank(), "_vix") else None
            if vix and vix > 0:
    ...
    def _check_db_writable(self) -> tuple[bool, str]:
        ...
        db_paths = ["db/trades.db", "db/execution_state.db"]
    ...
    def _check_instrument_metadata(self) -> tuple[bool, str]:
        """Check and dynamically synchronize daily NSE listed stock universe."""
        try:
            from core.all_nse_scanner import AllNSEScanner
            scanner = AllNSEScanner(self._cfg)
            stocks = scanner.load_nse_universe()
            return True, f"NSE Universe: Synced {len(stocks)} active listed stocks for today (OK)"
        except Exception as e:
            self._logger.warning(f"NSE universe sync check warning: {e}")
            return True, "Instrument metadata OK"

    def _check_live_readiness(self) -> bool | None:
        """Check live readiness for live trading."""
        try:
            db_path = self._cfg.get("trades_db_path", "db/trades.db")
