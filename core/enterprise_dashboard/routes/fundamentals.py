...
def register_fundamentals_routes(app, dashboard, admin_only, operator_or_admin) -> None:  # type: ignore[no-untyped-def]
    ...
    async def api_fundamentals_weights(  # type: ignore[no-untyped-def]
    ...
    async def api_fundamentals_analyze(  # type: ignore[no-untyped-def]
        symbol: str,
        request: Request,
        user: Any = Depends(dashboard._auth_deps.require_auth_optional),
    ):
