...
def _quality_score(signal: Mapping[str, Any]) -> float:
    ...
    if (direction == "CALL" and 40 <= rsi <= 70) or (direction == "PUT" and 30 <= rsi <= 60):
        q += 0.10
...
class ExecutionPolicy:
    ...
    def apply(
        ...
        score = int(signal.get("score", 0))
        _strong_min, _moderate_min, _weak_min = tier_bounds_from_config(config)
        tier  = classify_tier(score, strong_min=_strong_min, moderate_min=_moderate_min, weak_min=_weak_min)
        ...
        if tier == "IGNORE":
            return _skip(f"score {score} below WEAK_MIN {_weak_min} - IGNORE tier")
        ...
        for rule in pol.get("custom_skip_rules", []):
            if _eval_skip_rule(signal, regime, rule, config):
        ...
        rules = get_tier_rules(score, strong_min=_strong_min, moderate_min=_moderate_min, weak_min=_weak_min)
        ...
        # config key PARTIAL_EXIT_ENABLED (default true) - a global on/off
        # override on top of TierRules' own per-tier partial_exit_enabled
        # (already live today for STRONG/MODERATE regardless of this key).
        # Defaults to true so today's already-shipped per-tier behavior is
        # unchanged; set to false to force FULL exits across every tier.
        _partial_exit_globally_enabled = bool(config.get("PARTIAL_EXIT_ENABLED", True))
        _tier_partial_exit_enabled = rules.partial_exit_enabled and _partial_exit_globally_enabled

        # Determine execution mode
        if tier == "STRONG" and not signal.get("soft_blocks"):
            mode = "FULL"
        elif tier == "MODERATE":
            mode = "PARTIAL" if _tier_partial_exit_enabled else "FULL"
        else:
            mode = "REDUCED"
        ...
        return ExecutionDecision(
            ...
            partial_exit_enabled=_tier_partial_exit_enabled,
...
def _eval_skip_rule(
    ...
    _s_min, _m_min, _w_min = tier_bounds_from_config(config)
    tier     = classify_tier(score, strong_min=_s_min, moderate_min=_m_min, weak_min=_w_min)
