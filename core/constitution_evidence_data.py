...
def collect_all_evidence() -> Evidence:
    ...
    add("ARCH-01", "CLAUDE.md mandates boundary rules: no direct broker SDK calls from core", "documentation", 0.3)
    if _exists("tests/test_environment.py"):
        add("ARCH-01", "Environment test validates deployment boundary enforcement (test_environment.py)", "test_pass", 0.4)
    if _exists("tests/test_config_bootstrap.py"):
        add("ARCH-01", "Config bootstrap test validates layer-merge architecture boundary rules", "test_pass", 0.4)
    if _exists("tests/test_broker_port.py"):
        add("ARCH-01", "Broker port test validates port-contract boundary between core and adapters", "test_pass", 0.4)
    if _exists("tests/test_di_container.py"):
        add("ARCH-01", "DI container test validates explicit dependency boundary wiring", "test_pass", 0.3)
    if _exists("core/environment.py"):
        add("ARCH-01", "Environment gate enforces deployment boundary: DEV/QA/PAPER/SHADOW/PRODUCTION isolation", "code_review", 0.3)
    if _exists("tests/test_api_gateway.py"):
        add("ARCH-01", "API gateway test validates API-level boundary enforcement for trading operations", "test_pass", 0.3)
