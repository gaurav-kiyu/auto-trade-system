...
class WriteAheadJournal:
    ...
    def __init__(self, db_path: str = WAL_DB_PATH) -> None:
        ...
        if self._async_writer is not None:
            ...
            while time.time() < deadline:
                if self._async_writer.stats["queue_size"] == 0:
                    return
                # Shutdown-aware: lazy import to avoid circular deps
                try:
                    from core.safety_state import _shutdown as _wal_shutdown_event
                    if _wal_shutdown_event.wait(0.05):
                        _log.warning("[WAL] Flush interrupted by shutdown signal")
                        return
