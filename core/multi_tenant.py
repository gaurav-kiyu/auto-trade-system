...
-----
    ...
    trades = tc.get_trades(db_path="db/trades.db")
...
class TenantContext:
    ...
    def __init__(self, tenant: Tenant, base_config: dict[str, Any] | None = None) -> None:
        self._tenant = tenant
    ...
    def get_trades(self, db_path: str = "db/trades.db", days: int = 30) -> list[dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if has_tenant_col:
                    ...
                    rows = conn.execute(
                        "SELECT * FROM trades WHERE timestamp >= datetime('now', ?) ORDER BY timestamp DESC",
                        (f"-{days} days",),
                    ).fetchall()
            finally:
                conn.close()
