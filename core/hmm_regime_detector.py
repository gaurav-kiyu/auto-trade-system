...
class HMMRegimeDetector:
    ...
    def __init__(self, cfg: dict[str, Any]) -> None:
        ...
        self.state_map : dict[str, Any] = {} # state_id -> label
    ...
    def _map_states(self) -> None:
        ...
        for i in range(self.n_components):
            ...
            if var > np.percentile([np.mean(c) for c in self.model.covars_], 75):
                self.state_map[i] = "PANIC"  # type: ignore[index]
            elif var < np.percentile([np.mean(c) for c in self.model.covars_], 25):
                self.state_map[i] = "CHOPPY"  # type: ignore[index]
            else:
                self.state_map[i] = "TRENDING"  # type: ignore[index]
