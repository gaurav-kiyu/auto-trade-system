...
def _ols(x_mat: list[list[float]], y: list[float]) -> tuple[list[float], float]:
    """Ordinary least squares via numpy.  Returns (coefficients, r_squared).
    coefficients = [intercept, coeff_1, coeff_2, ...]
    """
    try:
        import numpy as np
        A = np.column_stack([[1.0] * len(y)] + [[row[i] for row in x_mat] for i in range(len(x_mat[0]))])
        b = np.array(y)
...
def calibrate_model(
    db_path: str = "db/trade_journal.db",
