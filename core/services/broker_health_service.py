...
class BrokerHealthService(BrokerHealthPort):
    ...
    def check_broker_health(self, broker_name: str) -> BrokerHealthMetrics:
        ...
        with self._health_check_lock:
            ...
            try:
                ...
                elif metrics.status == BrokerStatus.UNKNOWN:
    ...
    def _check_latency(self, adapter: Any, broker_name: str) -> HealthCheckResult:
        ...
        except (ConnectionError, OSError, ValueError, BrokerTimeoutError, AttributeError) as e:
            # AttributeError included: PaperBrokerAdapter inherits BrokerAdapter.get_quote()
            # without overriding it, so hasattr(adapter, "get_quote") above is a false
            # positive for it - calling it always raised an uncaught AttributeError
            # ('PaperBrokerAdapter' object has no attribute '_port') instead of the
            # graceful failure result this except clause already returns for every
            # other adapter-capability gap.
            latency_ms = (time.time() - start_time) * 1000
            return HealthCheckResult(
                broker_name=broker_name,
                check_type=HealthCheckType.LATENCY,
    ...
    def _get_last_success_time(self, broker_name: str) -> datetime | None:
        ...
        for check in reversed(self._health_history[broker_name]):
            if check.success:
                return check.timestamp  # type: ignore[no-any-return]
        return None
