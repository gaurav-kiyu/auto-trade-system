...
class SignalOrchestrator:
    ...
    def process_market_data_multi_asset(
        self,
        symbol: str,
        asset_type: Any,
        ...
        """Process market data for any asset class using the unified SignalEvaluator.

        Dispatches to the appropriate asset-class-specific signal strategy
        via ``core.services.signal_evaluator.SignalEvaluator``.

        Args:
            symbol: Trading symbol.
            asset_type: AssetType enum value for the asset class.
            data_frames: Dict with df1m, df5m, df15m DataFrames.
            additional_info: Extra parameters (vix, pcr, capital, etc.).

        Returns:
            SignalResult if actionable, None otherwise.
        """
        try:
            from core.services.signal_evaluator import SignalEvaluator

            evaluator = SignalEvaluator(config=self.config)
            ...
            result = evaluator.evaluate(
                ...
                iv=float(info.get("iv", 0)),
                pcr=float(info.get("pcr", 1.0)),
                capital=float(info.get("capital", 100_000)),
                is_early_session=bool(info.get("is_early_session", False)),
            )

            if result is None:
                log.debug("[MULTI_ASSET] No signal for %s (%s)", symbol, asset_type)
                return None

            log.info("[MULTI_ASSET] Signal for %s [%s]: score=%d dir=%s strength=%s",
                     symbol, asset_type, result.score, result.direction, result.strength)
            return result

        except (ImportError, ValueError, TypeError, KeyError, AttributeError, OSError) as exc:
            log.warning("[MULTI_ASSET] Signal evaluation failed for %s: %s", symbol, exc)
            return None
...
def init_signal_orchestrator(config: dict[str, Any]) -> None:
