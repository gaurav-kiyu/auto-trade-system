...
class PersistenceServiceConfig:
    ...
    # Database paths
    trades_db_path: str = "db/trades.db"
    state_db_path: str = "json/trader_state.json"  # JSON file for state
    market_data_db_path: str = "db/market_data.db"
...
class PersistenceService:
    ...
    def __init__(self, config: PersistenceServiceConfig | None = None) -> None:
    ...
    def save_trade(self, trade_data: dict[str, Any]) -> str:
        ...
        return self._trades_adapter.create("trades", trade_data)  # type: ignore[no-any-return]
...
class _JSONStateAdapter:
    ...
    def __init__(self, file_path: str) -> None:
        ...
        _logger.debug(f"JSONStateAdapter initialized for {file_path}")
    ...
    def connect(self) -> bool:
        ...
        try:
            with self._lock:
                ...
                _logger.debug(f"Connected to JSON state file: {self.file_path}")
                return True

        except (OSError, PermissionError, ValueError) as e:
            _logger.error(f"Failed to connect to JSON state file {self.file_path}: {e}")
            self._is_connected = False
            return False
    ...
    def disconnect(self) -> None:
        ...
        with self._lock:
            self._is_connected = False
            _logger.debug(f"Disconnected from JSON state file: {self.file_path}")
    ...
    def save_state(self, state: dict[str, Any]) -> bool:
        ...
        try:
            with self._lock:
                ...
                _logger.debug(f"State saved to {self.file_path}")
                return True
        except (OSError, TypeError, ValueError) as e:
            _logger.error(f"Failed to save state to {self.file_path}: {e}")
            return False
    ...
    def load_state(self) -> dict[str, Any] | None:
        ...
        try:
            with self._lock:
                if not self.file_path.exists():
                    _logger.debug(f"State file {self.file_path} does not exist")
                    return None

                with open(self.file_path, encoding="utf-8") as f:
                    state = json.load(f)

                _logger.debug(f"State loaded from {self.file_path}")
                return state  # type: ignore[no-any-return]

        except json.JSONDecodeError as e:
            _logger.error(f"Invalid JSON in state file {self.file_path}: {e}")
            return None
        except Exception as e:
            _logger.error(f"Failed to load state from {self.file_path}: {e} (type: {type(e).__name__})")
            return None
        ...
        try:
            with self._lock:
                if self.file_path.exists():
                    ...
                    _logger.debug(f"State file {self.file_path} deleted")
                self._is_connected = False
                return True
        except (OSError, FileNotFoundError, PermissionError) as e:
            _logger.error(f"Failed to delete state file {self.file_path}: {e}")
            return False
