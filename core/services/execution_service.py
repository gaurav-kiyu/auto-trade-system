...
from core.ports.persistence.persistence_port import PersistenceError
...
class ExecutionServiceConfig:
    ...
    slippage_model_calibration_enabled: bool = False
    trade_journal_db_path: str = "db/trade_journal.db"
    # config key ORDER_FILL_TIMEOUT_SEC (default 10, matching wait_for_fill()'s
    # own hardcoded default; verify_order_fill()'s own prior default was 30,
    # changed here to the real shipped config default). Currently inert
    # either way: verify_order_fill() has no live caller in the real order-
    # placement path yet - only tests call it, always with an explicit
    # timeout_seconds override.
    order_fill_timeout_sec: int = 10
...
class ExecutionService(ExecutionPort):
    ...
    def __init__(
        ...
        reconciliation_db_path: str = "db/trades.db",
        wal_journal=None,  # Phase 5B: Write-ahead journal for crash-safe intent logging
        smart_router=None,  # Phase 6: Optional SmartRouter for multi-broker routing
    ):
        ...
        durable_db_path = reconciliation_db_path.replace("db/trades.db", "db/execution_state.db")
        ...
        def _persistence_callback(machine) -> None:
            try:
        ...
        _calibrated_slippage_model = None
        if self.config.slippage_model_calibration_enabled:
            try:
                from core.slippage_model import calibrate_model
                _calibrated_slippage_model = calibrate_model(
                    db_path=self.config.trade_journal_db_path,
                    cfg={"slippage_model_enabled": True},
                )
            except (ImportError, ValueError, TypeError, AttributeError, OSError) as _slip_err:
                self._logger.debug(f"Slippage model calibration skipped: {_slip_err}")

        self._paper_trader = PaperTrader(
            fill_delay_ms=self.config.paper_fill_delay_ms,
            slippage_pct=self.config.paper_fill_slippage_pct,
            shutdown_event=self._shutdown_event,
            calibrated_slippage_model=_calibrated_slippage_model,
        )
        ...
        # Wire smart router if provided — connects broker selection logic
        if self._smart_router is not None:
            self._logger.info("SmartRouter wired into ExecutionService with %d broker(s)",
                             len(getattr(self._smart_router, '_routers', {})))
    ...
    def run_stale_order_timeout(self, max_stale_seconds: float = 300.0) -> dict:
        ...
        Delegates to ``core.execution.order_lifecycle.run_stale_order_timeout``.
        ...
        """
        return run_stale_order_timeout(
    ...
    def run_ack_watchdog(self, max_ack_age_seconds: float = 30.0) -> dict:
        ...
        Delegates to ``core.execution.order_lifecycle.run_ack_watchdog``.
        ...
        """
        return run_ack_watchdog(
            broker_port=self._broker_port,
            logger=self._logger,
            max_ack_age_seconds=max_ack_age_seconds,
        )

    def _on_reconciliation_freeze(self, reason: TradingFreezeReason, details: str) -> None:
    ...
    def execute_order(
        ...
        try:
        ...
        except (PersistenceError, ValueError, OSError, AttributeError, ConnectionError) as e:
            # ── Phase 5B: WAL journal failure ───────────────────────────────
            if self._wal_journal is not None:
            ...
            audit_trail.order_result = OrderResult(
                order_id="error",
                status=OrderStatus.REJECTED,
                reject_reason=f"Execution service error: {e!s}",
                timestamp=now_ist(),
            )
        ...
        *,
        ...
        try:
            ...
            if hasattr(broker_result, "status"):
                ...
                return broker_result  # type: ignore[no-any-return]
    ...
    def get_order_status(self, order_id: str) -> OrderStatus:
        ...
        try:
            ...
            if hasattr(self.broker_port, "get_order_status"):
                return self.broker_port.get_order_status(order_id)  # type: ignore[no-any-return]
    ...
    def verify_order_fill(
        ...
        Args:
            ...
            timeout_seconds: Maximum time to wait for fill confirmation.
                Defaults to config key ORDER_FILL_TIMEOUT_SEC (via
                ExecutionServiceConfig.order_fill_timeout_sec) when omitted.
        ...
        try:
            if timeout_seconds is None:
                timeout_seconds = self.config.order_fill_timeout_sec
            start_time = time.time()
            ...
            if hasattr(self.broker_port, "get_fill_price"):
                average_price = self.broker_port.get_fill_price(order_id) or 0.0
    ...
    def record_execution_audit(
        ...
        except (PersistenceError, KeyError, ValueError, AttributeError, OSError) as e:
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                # Check smart router health if available
                router_status = {}
                if hasattr(self, '_smart_router') and self._smart_router is not None:
                    try:
                        router_status = self._smart_router.health_check()
                    except (ValueError, TypeError, AttributeError, OSError):
                        router_status = {"status": "unhealthy"}
                ...
                if router_status:
                    result["smart_router"] = router_status
                return result
    ...
    def _execute_with_retries(
        ...
        try:
            ...
            if result.status == OrderStatus.PARTIALLY_FILLED:
                ...
                state_machine.record_partial_fill(result.filled_quantity or 0, getattr(result, 'average_fill_price', 0) or 0)
                return result
            ...
            return OrderResult(
                ...
                status=OrderStatus.REJECTED,
                reject_reason=f"Execution failed: {e!s}",
                timestamp=now_ist(),
            )
    ...
    def _attempt_order_execution(
        ...
        try:
            ...
            if (execution_context.execution_mode == ExecutionMode.PAPER or
                self.broker_port.__class__.__name__ == "PaperBrokerAdapter"):
                return self._execute_paper_order(order_request, execution_context)

            # Multi-broker failover (opt-in - see ExecutionServiceConfig /
            # SECONDARY_BROKER_DRIVER wiring in index_app/domains/broker/factory.py).
            # When no smart_router is configured (the only case today - the
            # DI container never builds one unless a second broker driver is
            # set), this is skipped entirely and _active_broker stays
            # self.broker_port exactly as before.
            _active_broker = self.broker_port
            if self._smart_router is not None:
                _route_result = self._smart_router.route_to_best(
                    symbol=str(order_request.symbol),
                    direction="CALL" if str(order_request.direction).upper() == "BUY" else "PUT",
                    qty=int(order_request.lot_size),
                    strike=int(order_request.strike_price),
                )
                if not _route_result.success:
                    return OrderResult(
                        order_id="", status=OrderStatus.REJECTED,
                        reject_reason=f"Smart router: {_route_result.error or 'no broker available'}",
                        timestamp=now_ist(),
                    )
                place_order_result: Any = _route_result.order_id
                # Route the SAME order to whichever broker actually placed
                # it - querying self.broker_port for fill status here would
                # silently ask the wrong adapter whenever the router picked
                # the secondary broker (the same class of bug already fixed
                # once for get_fill_price() below - see its docstring).
                _routed_adapter = getattr(self._smart_router, "_routers", {}).get(_route_result.broker)
                if _routed_adapter is not None:
                    _active_broker = _routed_adapter

            # Execute via broker port
            elif hasattr(self.broker_port, "place_order"):
            ...
            if self._smart_router is not None or hasattr(self.broker_port, "place_order"):
                if isinstance(place_order_result, OrderResult):
                    ...
                    filled_quantity = 0
                    average_price = 0.0
                    if hasattr(_active_broker, "get_filled_quantity"):
                        filled_quantity = _active_broker.get_filled_quantity(order_id) or 0
                    if hasattr(_active_broker, "get_fill_price"):
                        average_price = _active_broker.get_fill_price(order_id) or 0.0
    ...
    def _persist_trade_from_order(
        ...
        except (PersistenceError, ValueError, TypeError, OSError, AttributeError) as e:
            # Best-effort persistence only — closed trades are canonically
            # recorded by core.trade_recorder on position exit, so a failure
            # here must never surface as a trading error (log at WARNING).
            self._logger.warning(
                "Trade persistence skipped (best-effort) for %s: %s",
