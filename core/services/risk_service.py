...
class RiskServiceConfig:
    ...
    # Position sizing
    default_risk_per_trade: float = 0.02  # 2% of capital per trade (config key RISK_PER_TRADE)
    max_risk_per_trade: float = 0.05      # 5% maximum per trade
    # config key RISK_MODE ("PERCENT" or "FIXED") - PERCENT (default) risks
    # default_risk_per_trade fraction of capital, matching today's real,
    # already-live sizing exactly. FIXED risks a flat rupee amount
    # (risk_fixed_amount) per trade instead, regardless of capital size.
    risk_mode: str = "PERCENT"
    risk_fixed_amount: float = 150.0      # config key RISK_FIXED_AMOUNT, only used when risk_mode == "FIXED"
    # config key MAX_LOT_CAPITAL_PCT - meant as a cap on the % of capital
    # deployable as option-premium notional in a single trade. NOT applied
    # in calculate_position_size() below: sizing_input.entry_price is the
    # underlying index price (no live option-premium feed exists - see
    # strike_selector_enabled), so a notional cap computed against it would
    # be checking the wrong number entirely (tens of thousands vs. an
    # actual premium of a few hundred rupees) and would wrongly crush every
    # real trade to 0 lots. Kept as a config field (so the value round-trips
    # through the admin UI without error) but deliberately not wired into
    # any calculation until a real premium feed exists.
    max_lot_capital_pct: float = 0.85
    ...
    # Capital scaling (CapitalManager) - config keys MAX_DRAWDOWN/DRAWDOWN_SIZE_SCALE
    max_drawdown_pct: float = 0.20        # Equity drawdown fraction that zeroes out sizing
    drawdown_size_scale_enabled: bool = True  # Gate for the drawdown-based size reduction above

    # VIX-based sizing (config key VIX_SIZE_SCALE)
    vix_size_scale_enabled: bool = True   # Gate for vix_threshold_low/high size multipliers

    # Portfolio/per-trade risk caps (config keys PORTFOLIO_MAX_SL_RISK_PCT / MAX_SINGLE_TRADE_LOSS_PCT)
    portfolio_max_sl_risk_pct: float = 0.75   # Max fraction of capital at risk across all open SLs combined
    max_single_trade_loss_pct: float = 0.08   # Max fraction of capital any single trade may risk at its SL

    # VIX halt (config keys VIX_HALT_THRESHOLD / VIX_BLOCK_THRESHOLD)
    vix_halt_threshold: float = 30.0      # Soft warning level, below the hard VIX_BLOCK_THRESHOLD
...
class RiskService(RiskPort):
    ...
    def __init__(
        ...
        # Real lot sizes come from INDEX_MAP/instruments config (single source
        # of truth, kept current when NSE revises a contract's lot size).
        # _get_lot_size() only falls back to its hardcoded dict when a symbol
        # isn't in this map - previously it never consulted config at all.
        ...
        # Daily trade-count tracking (MAX_TRADES_DAY / config.max_daily_trades).
        # Self-tracked here (not a caller-supplied callback) because nothing else
        # in the codebase counted trade *entries* per day for the index-options
        # path - only open-position count and daily P&L were injected.
        self._trade_entry_times: list[datetime] = []
    ...
    def update_risk_limit(
        ...
        """Update an explicitly supported runtime risk limit.

        The existing canonical RiskService instance is mutated in-place.
        Its RiskServiceConfig and RiskLimitsManager configuration remain
        synchronized. Unsupported limits are rejected.
        """
        field_map: dict[str, tuple[str, str, type, float | None, float | None]] = {
            "MAX_DAILY_LOSS": (
                "max_daily_loss",
                "max_daily_loss",
                float,
        ...
        if key not in field_map:
            raise ValueError(f"Unsupported runtime risk limit: {name}")

        service_attr, limits_attr, value_type, minimum, maximum = field_map[key]
        ...
        with self._lock:
            previous = getattr(self.config, service_attr)

            setattr(self.config, service_attr, converted)
            setattr(self.limits.config, limits_attr, converted)
    ...
    def evaluate_trade(
        ...
        with self._lock:
            try:
                ...
                return RiskEvaluation(
                    decision=RiskDecision.DENIED,
                    reason=f"Risk evaluation error: {e!s}",
                    risk_score=1.0,
                )
    ...
    def calculate_position_size(
        ...
        try:
            ...
            # config key RISK_MODE ("PERCENT", default) risks a fraction of
            # capital (sizing_input.risk_per_trade, from RISK_PER_TRADE);
            # ("FIXED") risks a flat rupee amount (risk_fixed_amount, from
            # RISK_FIXED_AMOUNT) instead, regardless of capital size.
            if str(self.config.risk_mode).upper() == "FIXED":
                risk_amount = self.config.risk_fixed_amount
            else:
    ...
    def reset_daily_metrics(self) -> None:
        ...
        with self._lock:
            ...
            if hours_since_reset >= self.config.loss_reset_hours:
                reset_consecutive_losses()
                self._last_loss_reset = now_ist()
            # Drop entries from before today - get_trades_today() already
            # filters by date, this just bounds long-uptime memory growth.
            today_start = now_ist().replace(hour=0, minute=0, second=0, microsecond=0)
            self._trade_entry_times = [t for t in self._trade_entry_times if t >= today_start]
    ...
    def health_check(self) -> dict[str, Any]:
        ...
        except Exception as e:  # health checks must never crash
    ...
    def record_trade_entry(self, symbol: str = "") -> None:
        """Record a new trade entry for daily trade-count tracking.

        Call exactly once per successful order fill (mirrors the exit-side
        record_trade_outcome() call). Feeds _check_daily_trade_count() below -
        without this, MAX_TRADES_DAY/config.max_daily_trades is threaded through
        RiskServiceConfig/LimitConfig but never actually enforced for entries.
        """
        with self._lock:
            self._trade_entry_times.append(now_ist())

    def get_trades_today(self) -> int:
        """Count trade entries recorded since local midnight (IST)."""
        with self._lock:
            today_start = now_ist().replace(hour=0, minute=0, second=0, microsecond=0)
            return sum(1 for t in self._trade_entry_times if t >= today_start)
    ...
    def _check_daily_trade_count(
        ...
        """Check the daily trade-count limit (MAX_TRADES_DAY / config.max_daily_trades)."""
        trades_today = self.get_trades_today()
        if trades_today >= self.config.max_daily_trades:
            return RiskEvaluation(
                decision=RiskDecision.DENIED,
                reason=f"Daily trade count limit reached: {trades_today} >= {self.config.max_daily_trades}",
                risk_score=0.8,
            )
        return RiskEvaluation(
            decision=RiskDecision.ALLOWED,
            reason="Daily trade count check passed",
            risk_score=0.0,
        )
    ...
    # Uses inline CapitalManager (importing from this module, not deprecated core.capital_manager).

    def _get_capital_manager(self) -> None:
        ...
        with self._lock:
            ...
            self._capital_manager = CapitalManager(
                ...
                drawdown_scale_enabled=self.config.drawdown_size_scale_enabled,
            )
            return self._capital_manager

    def scale_position(self, base_lots: int, max_lots: int = 1) -> ScaleResult:
        """Apply equity-aware capital scaling to base_lots."""
        try:
            cm = self._get_capital_manager()
            return cm.scale(base_lots=base_lots, max_lots=max_lots)  # type: ignore[no-any-return]
    ...
    def lock_profits(self, lock_pct: float = 0.50) -> float:
        ...
        try:
            cm = self._get_capital_manager()
            return cm.lock_profits(lock_pct=lock_pct)  # type: ignore[no-any-return]
        except (ValueError, TypeError, AttributeError) as e:
    ...
    def get_capital_state(self) -> dict[str, Any]:
        ...
        try:
            cm = self._get_capital_manager()
            return cm.get_state()  # type: ignore[no-any-return]
        except (ValueError, TypeError, AttributeError) as e:
            self._logger.warning(f"Capital state fetch failed: {e}")
            return {"current_capital": self._get_capital(), "daily_pnl": self._get_daily_pnl()}
    ...
    def get_min_score_for_regime(self, regime: str) -> int:
        ...
        if reg in ["TRENDING", "BULLISH"]:
            return 68
        if reg in ["SIDEWAYS", "NEUTRAL"]:
            return 73
        if reg in ["RANGE", "CHOPPY"]:
            return 78
        return 73
    ...
    def _lazy_vix_getter(self) -> float:
        ...
        try:
            ...
            vix = getattr(get_iv_rank(), '_vix', None)
            if vix and vix > 0:
                return vix
    ...
    def _get_lot_size(self, symbol: str) -> int:
        """Get lot size for a symbol - reads INDEX_MAP/instruments config first."""
        instrument = self._index_map.get(str(symbol).upper(), {})
        configured_lot = instrument.get("lot")
        if configured_lot:
            try:
                return int(configured_lot)
            except (TypeError, ValueError):
                pass
        # Fallback only for symbols absent from index_map (e.g. tests that
        # construct RiskService without one) - not the source of truth.
        lot_sizes = {
            "NIFTY": 50,
            "BANKNIFTY": 15,
            "FINNIFTY": 40,
    ...
    def _get_volatility_multiplier(self, volatility: float) -> float:
        ...
        if volatility >= self.config.vix_threshold_high:
    ...
    def get_vix_size_multiplier(self, volatility: float) -> float:
        """Public, config-gated (VIX_SIZE_SCALE) wrapper for the VIX-based
        size multiplier above. calculate_position_size() (this method's
        original caller) is advisory-only in the live path - the real order
        quantity is decided elsewhere (mandate_service) - so this is exposed
        separately for callers on the real entry path to apply directly.
        Returns 1.0 (no adjustment) when the gate is off.
        """
        if not self.config.vix_size_scale_enabled:
            return 1.0
        return self._get_volatility_multiplier(volatility)

    def get_single_trade_loss_cap_lots(
        self, capital_available: float, price_diff: float, lot_size: int,
    ) -> int:
        """Max lots such that this one trade's SL-loss doesn't exceed
        max_single_trade_loss_pct of capital (config key
        MAX_SINGLE_TRADE_LOSS_PCT). Previously validated but never enforced
        anywhere. Returns a very large number (no cap) if inputs are
        degenerate, so callers can safely min() it against their own qty.
        """
        try:
            if price_diff <= 0 or lot_size <= 0 or capital_available <= 0:
                return 10**9
            max_loss_amount = capital_available * self.config.max_single_trade_loss_pct
            return max(0, int(max_loss_amount / (price_diff * lot_size)))
        except (TypeError, ValueError, ZeroDivisionError):
            return 10**9

    def check_portfolio_sl_risk(
        self, open_positions_sl_risk: float, capital_available: float, new_trade_sl_risk: float = 0.0,
    ) -> bool:
        """True if adding new_trade_sl_risk to the existing open positions'
        combined SL-risk would stay within portfolio_max_sl_risk_pct of
        capital (config key PORTFOLIO_MAX_SL_RISK_PCT). Previously validated
        but never enforced anywhere - this is the first real consumer.
        Callers pass the sum of (entry-SL distance x lot_size) across all
        currently open positions as open_positions_sl_risk.
        """
        if capital_available <= 0:
            return True
        cap = capital_available * self.config.portfolio_max_sl_risk_pct
        return (open_positions_sl_risk + new_trade_sl_risk) <= cap

    def is_vix_soft_warning(self, vix: float) -> bool:
        """True once VIX crosses the softer VIX_HALT_THRESHOLD, ahead of the
        hard VIX_BLOCK_THRESHOLD that actually blocks entries. Previously
        validated (and checked to stay below VIX_BLOCK_THRESHOLD) but never
        read anywhere - callers can use this for an early warning
        notification without blocking any trade themselves.
        """
        return vix >= self.config.vix_halt_threshold
    ...
    def _calculate_max_lots_by_capital(
        ...
        try:
            ...
            # Recalculate lots based on max allowed risk: max_risk_amount already
            # bakes in the risk fraction (capital_available * max_risk_per_trade),
            # so lots = max_risk_amount / (per-lot loss at SL). Dividing by
            # risk_per_trade again here previously inflated the cap ~1/risk_per_trade
            # (e.g. 50x at risk_per_trade=0.02), making it a near no-op guard.
            price_diff = abs(sizing_input.entry_price - sizing_input.stop_loss_price)
            if price_diff <= 0 or sizing_input.lot_size <= 0:
                return 0

            max_lots = int(max_risk_amount / (price_diff * sizing_input.lot_size))
