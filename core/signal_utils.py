...
def validate_ohlcv(
    ...
    # Reject non-finite market-data values before relational validation.
    # OHLCV must never allow NaN or +/-inf into indicator/scoring logic.
    finite_mask = df[list(required)].apply(
        pd.to_numeric, errors="coerce"
    ).notna().all(axis=1)

    finite_mask &= df[list(required)].apply(
        lambda col: np.isfinite(
