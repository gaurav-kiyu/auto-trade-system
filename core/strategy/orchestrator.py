...
class StrategyOrchestrator(StrategyPort):
    ...
    def __init__(
        ...
        # Lazy-import StrategyApprovalWorkflow for governance checks
        self._strategy_governance = None
        if config:
            try:
                from core.strategy.approval_workflow import get_approval_workflow
                self._strategy_governance = get_approval_workflow()
            except (ImportError, ValueError, TypeError):
                self._strategy_governance = None
    ...
    def generate_signal(self, **kwargs: Any) -> dict[str, Any] | None:
        """Generate and normalize a signal from the canonical signal evaluator.

        The constructor keyword ``signal_orchestrator`` is retained for
        backward compatibility. The production DI container supplies the
        canonical ``core.services.signal_evaluator.SignalEvaluator`` here.

        Canonical ``SignalResult`` objects are converted through ``to_dict()``
        so the complete signal metadata is preserved for downstream routing.
        """
        if self._signal_orchestrator is None:
            return None

        try:
            signal_result = self._signal_orchestrator.evaluate(**kwargs)
            ...
            # Canonical SignalResult -> complete standard signal dictionary.
            to_dict = getattr(signal_result, "to_dict", None)
            if callable(to_dict):
                signal_data = dict(to_dict())

                # SignalResult.to_dict() intentionally preserves its existing
                # public key contract and does not expose confidence.
                # StrategyDecision/StrategyPort, however, require confidence.
                # Carry the canonical value across at the orchestration boundary.
                signal_data.setdefault(
                    "confidence",
                    float(getattr(signal_result, "confidence", 0.0)),
                )

                # StrategyPort consumers historically expect should_trade.
                # Derive it from the canonical score/strength without
                # discarding the richer SignalResult fields.
                signal_data.setdefault(
                    "should_trade",
                    float(signal_data.get("score", 0)) > 0
                    and signal_data.get("strength") != "IGNORE",
                )

                return signal_data

            # Backward-compatible SignalIntent/dataclass normalization.
            score = float(getattr(signal_result, "score", 0))
            return {
                "symbol": getattr(
                    signal_result, "symbol", kwargs.get("symbol", "")
                ),
                "should_trade": score > 0,
                "direction": getattr(signal_result, "direction", ""),
                "score": score,
                "confidence": float(
                    getattr(signal_result, "confidence", 0.0)
                ),
                "price": float(getattr(signal_result, "price", 0.0)),
                "reason": getattr(
                    signal_result,
                    "reason",
                    getattr(signal_result, "rationale", ""),
                ),
                "regime": getattr(
                    signal_result, "regime", "NEUTRAL"
                ),
            }

        except (ImportError, ValueError, TypeError, AttributeError) as e:
            _log.error("Signal evaluator evaluation failed: %s", e)
    ...
    def evaluate(self, **kwargs) -> StrategyDecision:
        """Full signal pipeline: governance check → generate → approve.
        ...
        Accepts kwargs for generation, plus:
          ...
          strategy_name - for governance check
        """
        # 0. Governance check (StrategyApprovalWorkflow)
        strategy_name = kwargs.get("strategy_name", "")
        if strategy_name and self._strategy_governance is not None:
            try:
                state = self._strategy_governance._get_current_state(strategy_name)
                if state == "DONT_RUN":
                    _log.warning("Strategy %s is BLOCKED (state=DONT_RUN) - no evaluation", strategy_name)
                    return StrategyDecision(
                        action="HOLD",
                        reason=f"Strategy '{strategy_name}' is in DONT_RUN state — skipped by governance",
                    )
            except Exception:
                pass  # Governance failure should not block trading
