...
def reconcile_broker_truth(broker_port: Any | None = None) -> dict:
    ...
    except (ValueError, TypeError, OSError, AttributeError, ConnectionError, RuntimeError) as exc:
        # RuntimeError included: broker SDKs and test doubles may raise it;
        # reconciliation must degrade to an ERROR report, never crash the caller.
...
def get_broker_truth_reconciler(broker_port, config: dict | None = None) -> BrokerTruthReconciler:
