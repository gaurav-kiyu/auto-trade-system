...
class ExecutionStateMachine:
    ...
    def record_fill(self, filled_qty: int, price: float) -> bool:
        ...
        with self._lock:
            ...
            if self.state == ExecutionState.PARTIAL_FILL:
...
class ExecutionStateMachineManager:
    ...
    def __init__(self, persistence_callback: Callable | None = None) -> None:
        ...
        with self._lock:
            ...
            for k in keys:
                ...
                if m.state in terminal_states:
                    # created_at uses time_provider.format_ts() which returns a
                    # naive IST string (%Y-%m-%d %H:%M:%S). Parse it as IST-aware
                    # so .timestamp() yields the real epoch on ANY host TZ -
                    # time.mktime() interprets the naive string in the machine's
                    # local timezone, which on UTC hosts shifted ages ~5.5h and
                    # stale terminal machines were never pruned.
                    try:
                        if m.created_at:
                            created_ts = datetime.strptime(
                                m.created_at, "%Y-%m-%d %H:%M:%S"
