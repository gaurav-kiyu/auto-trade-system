...
class TransientBrokerError(BrokerException):
    ...
    def __init__(self, message: str, broker_code: str | None = None,        original: Exception | None = None) -> None:
        super().__init__(
            message=message,
...
def _dispatch_error_code(
    ...
    return exc_cls(f"{msg} (code: {error_code})", broker_code=error_code, original=exception)  # type: ignore[no-any-return]
