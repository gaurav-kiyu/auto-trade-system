...
class BrokerGateway:
    ...
    def register_adapter(self, name: str, adapter_class: type[BrokerAdapter]) -> None:
    ...
    def place_order(self, request: OrderRequest) -> OrderResponse:
        ...
        if request.order_type == "TWAP":
            try:
                from core.execution.algo_router import get_algo_engine
                engine = get_algo_engine()

                # Execute callback that places standard market orders
                def _twap_execute(sym: str, q: int, act: str) -> bool:
                    try:
                        import copy
                        slice_req = copy.copy(request)
                        slice_req.qty = q
                        slice_req.order_type = "MARKET"
                        res = self._active_adapter.place_order(slice_req)
                        return res.status != OrderStatus.FAILED
                    except Exception:
                        return False

                # Slice into 5 parts, 30 seconds apart
                engine.execute_twap(
                    ...
                    slices=5,
                    interval_sec=30,
                    current_price=self.get_ltp(request.symbol),
                    execute_callback=_twap_execute
                )
                return OrderResponse(
                    order_id="TWAP_ALGO_ROUTED",
                    status=OrderStatus.PENDING
                )
            except ImportError:
                log.warning("TWAP engine not found. Defaulting to MARKET.")
