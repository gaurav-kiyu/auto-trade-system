...
class DurableExecutionStore:
    ...
    def __init__(self, db_path: str = "db/execution_state.db") -> None:
    ...
    def update_state(
        ...
        try:
            with self._lock, get_connection(self._db_path) as conn:
                ...
                conn.execute(
                    f"UPDATE execution_state SET {', '.join(updates)} WHERE intent_id = ?",  # nosec B608
                    params,
                )
                conn.commit()
            return True
    ...
    def cleanup_old_records(self, hours: int = 24) -> int:
        ...
        Computes the cutoff in Python (now_ist() - hours) and compares
        against updated_at's own isoformat() string, rather than SQLite's
        datetime('now', ...) - that produces a space-separated, UTC string
        while every updated_at value is written via .isoformat() (a 'T'
        separator, local/IST time per core/datetime_ist.now_ist()). Both the
        UTC-vs-IST offset and the 'T'-vs-space separator make the two string
        forms compare incorrectly, so records could silently never be
        cleaned up once the shifted dates landed on the same calendar day.
        """
        try:
            ...
            with self._lock, get_connection(self._db_path) as conn:
                cursor = conn.execute("""
                        ...
                        WHERE state IN (?, ?, ?, ?)
                        AND updated_at < ?
                    """, tuple(s.value for s in self._TERMINAL_STATES) + (cutoff,))
                conn.commit()
...
def get_durable_store(db_path: str = "db/execution_state.db") -> DurableExecutionStore:
