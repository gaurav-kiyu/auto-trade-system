...
def append_tier_engine_errors(
    ...
    # -- TG_ALERT_MIN_SCORE -- Intentional Notification Filter Gate -
    tg_min = int(cfg.get("TG_ALERT_MIN_SCORE", ai_thr))
    has_cat_thresholds = bool(cfg.get("CATEGORY_SCORE_THRESHOLDS") or cfg.get("INDEX_MIN_SCORE"))
    if tg_min > ai_thr and not has_cat_thresholds:
        # Signals are evaluated, pass all filters, then dropped before Telegram if no category filters set.
        warnings.append(
            f"TG_ALERT_MIN_SCORE={tg_min} > AI_THRESHOLD={ai_thr}: "
            f"notification filter active -- alerts dispatched only for scores >= {tg_min}.",
        )
    elif tg_min < ai_thr:
    ...
    # -- STRONG_THRESHOLD/MODERATE_THRESHOLD are a SEPARATE system --------
    # TIER_STRONG_MIN/TIER_MODERATE_MIN drive the real execution tier
    # (core.tier_engine.classify_tier, called with these config values from
    # core.adaptive_signal / core.execution_policy). STRONG_THRESHOLD/
    # MODERATE_THRESHOLD instead drive core.pure_index_signal's UI-only
    # "strength" display label (finalize_index_signal_with_threshold) and are
    # not read anywhere on the execution path. They are allowed to differ by
    # design -- this is informational, not a warning to "fix".
    ...
    if legacy_strong != t_strong:
        warnings.append(
            f"STRONG_THRESHOLD={legacy_strong} vs TIER_STRONG_MIN={t_strong}: "
            f"different systems by design (display label vs execution tier) -- no action needed.",
        )
    if legacy_moderate != t_moderate:
        warnings.append(
            f"MODERATE_THRESHOLD={legacy_moderate} vs TIER_MODERATE_MIN={t_moderate}: "
            f"different systems by design (display label vs execution tier) -- no action needed.",
        )
...
def validate_config(cfg: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    # -- 8. STRONG_THRESHOLD/MODERATE_THRESHOLD are a separate system -----
    # (see the matching comment in append_tier_engine_errors() above)
    legacy_strong = int(cfg.get("STRONG_THRESHOLD", t_strong))
    if legacy_strong != t_strong:
        warn(
            f"STRONG_THRESHOLD={legacy_strong} vs TIER_STRONG_MIN={t_strong}: "
...
def get_instrument_param(cfg: dict[str, Any], index_name: str, key: str, fallback=None) -> None:
