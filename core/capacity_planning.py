...
-----
    ...
    db_growth = planner.estimate_db_growth("db/trades.db")
...
class CapacityPlanner:
    ...
    Enhanced with throughput trend analysis (rolling average over 7 days),
    config history tracking for capacity changes, and resource scoring.
    """
    ...
    def __init__(self, cfg: dict[str, Any] | None = None) -> None:
        ...
        self._max_history = 168  # One week hourly
        # Config / capacity change tracking
    ...
    def get_throughput_trend(self) -> dict[str, Any]:
        ...
        Thread-safe: acquires self._lock for access to shared history.

        Returns:
            Dict with hourly averages, trend direction, and peak metrics.
        """
        with self._lock:
            if not self._throughput_history:
                return {"status": "insufficient_data", "hours": 0}

            # Compute hourly averages
            hourly: dict[str, list[float]] = defaultdict(list)
            for entry in self._throughput_history:
                hour_key = entry.get("hour", "unknown")
                hourly[hour_key].append(entry.get("rate", 0.0))

            hourly_avgs = {
                k: round(sum(v) / len(v), 2) for k, v in sorted(hourly.items())
            }

            # Trend direction (compare first half to second half)
            rates = [entry.get("rate", 0.0) for entry in self._throughput_history]
            mid = len(rates) // 2
            ...
            return {
                "status": "ok",
                "hours": len(self._throughput_history),
                "current_avg_rate": round(avg_rate, 2),
                "peak_rate": round(peak_rate, 2),
                "trend": trend,
                "hourly_averages": hourly_avgs,
                "peak_hour": max(hourly_avgs, key=hourly_avgs.get) if hourly_avgs else "",
            }

    def record_throughput_observation(self, rate: float) -> None:
        """Record a throughput observation for trend analysis.

        Thread-safe: acquires self._lock for append/pop.

        Args:
            rate: Observed throughput rate (trades/hour or events/sec).
        """
        with self._lock:
            ...
            if len(self._throughput_history) > self._max_history:
                self._throughput_history.pop(0)

    def get_change_log(self, limit: int = 50) -> list[dict[str, Any]]:
        """Get capacity change log.

        Tracked changes: config updates, resource limit changes, scaling events.

        Thread-safe: acquires self._lock for access to shared log.
    ...
    def log_capacity_change(self, change_type: str, description: str, details: dict | None = None) -> None:
        """Log a capacity-related change.

        Thread-safe: acquires self._lock for append.

        Args:
            change_type: Type of change (CONFIG, RESOURCE_LIMIT, SCALING).
    ...
    def _check_databases(self, report: CapacityReport, issues: list[str]) -> None:
        ...
        dbs = ["db/trades.db", "db/trade_journal.db", "db/ml_tracker.db",
               "db/oi_snapshots.db", "db/event_store.db", "db/execution_state.db"]
    ...
    def _check_trade_throughput(self, report: CapacityReport, issues: list[str]) -> None:
        ...
        try:
            ...
            try:
                ...
                row = conn.execute(
                    "SELECT COUNT(*) FROM trades WHERE timestamp >= datetime('now', '-1 day')",
                ).fetchone()
    ...
    def _check_memory_usage(self, report: CapacityReport, issues: list[str]) -> None:
        ...
        except (OSError, ValueError, AttributeError) as _mem_exc:
            _log.debug("[CAPACITY] Memory check failed: %s", _mem_exc)
            report.metrics.append(ResourceMetric(
                resource="process_memory", current_value=0, unit="MB",
