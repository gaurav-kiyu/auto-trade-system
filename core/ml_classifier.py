...
# ── Safe unpickling helper with integrity verification ────────────────
_MODEL_CHECKSUMS: dict[str, str] = {}  # path -> sha256; guarded by _model_lock
_CHECKSUM_LOCK = threading.RLock()


def _safe_load_model(path: Path) -> Any | None:
    """Load a pickled model with integrity verification.

    Computes SHA-256 of the file before unpickling, then stores
    the checksum for subsequent verification.  Refuses to load
    if the checksum has changed between consecutive loads.

    Thread-safe: uses _CHECKSUM_LOCK for checksum dict access.
    """
    try:
        data = path.read_bytes()
        checksum = hashlib.sha256(data).hexdigest()
        with _CHECKSUM_LOCK:
            prev = _MODEL_CHECKSUMS.get(str(path))
        if prev is not None and prev != checksum:
            _log.warning("[ML] Model file %s checksum changed! Refusing to load (possible tamper).", path)
            return None
        with _CHECKSUM_LOCK:
            _MODEL_CHECKSUMS[str(path)] = checksum
        # Restrict unpickling to known-safe classes (LightGBM Booster)
        from lightgbm import Booster, LGBMClassifier, LGBMRegressor
        safe_classes = (Booster, LGBMClassifier, LGBMRegressor, dict, list, tuple, set, str, int, float, bool, type(None))
        restored = pickle.loads(data)  # nosec B301
        if not isinstance(restored, safe_classes):
            _log.warning("[ML] Unpickled object type %s is not in the safe class allowlist; rejecting.", type(restored).__name__)
            return None
        return restored
    except (OSError, pickle.UnpicklingError, ValueError, TypeError, ImportError) as exc:
        _log.debug("[ML] Safe load failed: %s", exc)
        return None
...
def save_model(model: Any, path: str | Path) -> bool:
    ...
    Clears the integrity checksum cache for this path so that
    subsequent loads via _safe_load_model() accept the new file.
    """
    try:
        p = Path(path)
        ...
        with open(p, "wb") as f:
            f.write(model_bytes)
        # Clear stale checksum so next load doesn't reject the updated file
        with _CHECKSUM_LOCK:
            _MODEL_CHECKSUMS.pop(str(p), None)
...
def load_model(path: str | Path) -> Any | None:
    ...
    Uses _safe_load_model() which:
      - Verifies SHA-256 checksum consistency across loads
      - Restricts unpickling to known-safe classes (LightGBM only)
      - Logs warnings on tamper detection

    Returns None if file missing, checksum mismatch, or unsafe type.
    """
    try:
        p = Path(path)
        if not p.is_file():
            return None
        return _safe_load_model(p)
...
def get_classifier(
    ...
    tracker_db_path            = c.get("ml_tracker_db_path", "db/ml_tracker.db")
