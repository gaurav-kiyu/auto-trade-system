...
# Alias for gap audit compliance (WS8 Admin CP)
...
class ControlPlaneServer:
    ...
    def control_risk_limit(
        self,
        name: str,
        value: str,
        identity: str,
    ) -> dict[str, Any]:
        """Validate, audit, and apply a runtime risk-limit change."""
        try:
        ...
        if self._risk_service is None:
            rec = self._audit(
                ...
                identity,
                False,
                "Canonical RiskService not wired",
            )
            return {
                "success": False,
                "reason": "Canonical RiskService not wired",
                "action_id": rec.action_id,
            }

        try:
            previous, applied = self._risk_service.update_risk_limit(
                name,
                numeric_value,
            )
        ...
        with self._lock:
            self._risk_limits[normalized_name] = float(applied)
        ...
        rec = self._audit(
            ...
            identity,
            True,
            previous_state={"value": previous},
            new_state={"value": applied},
        )

        if self._state_callback:
            self._state_callback(
                f"risk_limit/{normalized_name}",
                {"value": applied},
            )

        _log.warning(
            "[CTRL] Risk limit %s set to %s by %s",
            normalized_name,
            applied,
            identity,
        )
...
def create_control_plane_app(
    ...
    def _resolve_identity(authorization: str | None) -> str:
        ...
        return token_obj.identity  # type: ignore[no-any-return]
...
def maybe_start_control_plane(
    ...
    # Canonical RiskService supplied by the application DI container.
    # Never construct a second RiskService inside the control plane.
    risk_service: Any | None = None,
    # New-style callbacks
    ...
    # Create new-style ControlPlaneServer with the canonical RiskService.
    # Never construct a second RiskService inside the control plane.
    ...
    # Publish the exact server instance used by the running control plane.
    # Consumers such as the Enterprise Dashboard must never construct
    # a second ControlPlaneServer.
    global _ACTIVE_CONTROL_PLANE_SERVER
    with _ACTIVE_CONTROL_PLANE_SERVER_LOCK:
        _ACTIVE_CONTROL_PLANE_SERVER = _server
    if halt_callback:
...
__all__ = [
    ...
    "get_active_control_plane_server",
]
