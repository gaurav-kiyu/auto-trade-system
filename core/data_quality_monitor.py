...
class DataQualityMonitor:
    ...
    def __init__(self, config: DataQualityConfig | None = None) -> None:
        ...
        # Attached scorer for auto-recording findings
    ...
    def set_scorer(self, scorer: Any) -> None:
        """Attach a DataQualityScorer to auto-feed findings.

        When set, all subsequent check methods will automatically
        record their findings to the scorer for aggregated quality scoring.

        Args:
            scorer: DataQualityScorer instance.
        """
        self._scorer = scorer

    def _auto_record(self, findings: list[DataQualityFinding], source: str = "data_quality_monitor") -> None:
        """Automatically record findings to attached scorer."""
        if hasattr(self, '_scorer') and self._scorer is not None:
            try:
                self._scorer.record_findings(findings)
            except Exception:
                pass  # Non-critical; scorer must not break monitoring
    ...
    def check_price_anomaly(
        ...
        self._auto_record(findings, source="price_anomaly")
        return findings
    ...
    def check_data_freshness(
        ...
        self._total_findings += len(findings)
        self._auto_record(findings, source="freshness")
        return findings
    ...
    def check_completeness(
        ...
        self._total_findings += len(findings)
        self._auto_record(findings, source="completeness")
        return findings
    ...
    def check_schema(
        ...
        self._total_findings += len(findings)
        self._auto_record(findings, source="schema")
        return findings
