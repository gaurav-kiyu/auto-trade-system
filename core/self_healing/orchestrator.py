...
class SelfHealingOrchestrator:
    ...
    def _is_cooled_off(self, action_key: str) -> bool:
        ...
        return (now - last) >= cooldown  # type: ignore[no-any-return]
    ...
    def _query_kb_for_guidance(self, pattern_name: str) -> str:
        """Query Knowledge Base for past incidents matching a failure pattern.

        Returns a string with recommendations from similar past incidents,
        ...
        if not self._kb:
            return ""
        try:
            results = self._kb.find_similar(pattern_name, max_results=3)
            if not results:
                return ""
            parts = []
            for r in results:
                parts.append(f"[KB {r.pattern_type}] ({r.confidence:.0%}) {r.pattern[:100]}")
                if r.solution:
                    parts.append(f"  -> {r.solution[:100]}")
            return " | ".join(parts)
        except Exception as exc:
            _log.debug("[SELF-HEALING] KB query failed: %s", exc)
            return ""
    ...
    def _execute_recovery(self, pattern: FailurePattern) -> HealingAction:
        ...
        # Query Knowledge Base for guidance from past incidents
        kb_guidance = self._query_kb_for_guidance(pattern.name)
        if kb_guidance:
            _log.info("[SELF-HEALING] KB found past patterns for %s: %s", pattern.name, kb_guidance[:150])
        ...
        # All actions failed — append KB guidance if available
        msg = f"All recovery actions exhausted for {pattern.name}"
        if kb_guidance:
            msg += f" | From past incidents: {kb_guidance[:200]}"
        return HealingAction(
            action=pattern.recovery_actions[0],
            component=pattern.name,
            status="FAILED",
    ...
    def _execute_single_action(self, action: RecoveryAction,
        ...
        try:
            ...
            if action == RecoveryAction.RECONNECT_BROKER:
                return self._reconnect_broker()
            if action == RecoveryAction.RESTART_STALE_FEED:
                return self._restart_stale_feed(component)
            if action == RecoveryAction.RECONNECT_DATABASE:
                return self._reconnect_database()
            if action == RecoveryAction.RELOAD_CONFIG:
                return self._reload_config()
            if action == RecoveryAction.CLEAR_HARD_HALT:
                return self._clear_hard_halt()
            if action == RecoveryAction.RECYCLE_SESSION:
                return self._recycle_session()
            if action == RecoveryAction.RESTART_WATCHDOG:
                return self._restart_watchdog()
            if action == RecoveryAction.NOTIFY_OPERATOR:
                return self._notify_operator(f"Action required: {component}")
            if action == RecoveryAction.DISK_CLEANUP:
                return self._disk_cleanup()
            if action == RecoveryAction.FORCE_WAL_CHECKPOINT:
                return self._force_wal_checkpoint()
            if action == RecoveryAction.CLEAR_STALE_LOCKS:
                return self._clear_stale_locks()
            if action == RecoveryAction.RUN_RUNBOOK:
    ...
    def _reconnect_broker(self) -> dict[str, Any]:
        ...
        if self._broker_adapter is None:
            return {"status": "SUCCESS", "message": "Paper mode active / no live broker to reconnect"}
        ...
        try:
            ...
            if hasattr(self._broker_adapter, "reconnect"):
                ...
                return {"status": "SUCCESS", "message": "Broker adapter is passive / paper"}
    ...
    def _reconnect_database(self) -> dict[str, Any]:
        ...
        try:
            ...
            dbs = ["db/trades.db", "db/trade_journal.db", "db/ml_tracker.db", "db/oi_snapshots.db"]
            reconnected = 0
    ...
    def _clear_hard_halt(self) -> dict[str, Any]:
        ...
        try:
            ...
            clear_hard_halt(source="self_healing", reason="Automatic recovery — conditions cleared")
