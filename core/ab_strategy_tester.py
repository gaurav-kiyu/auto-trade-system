...
-----------------------------------------
    ...
    ab_state_path                   : str   default "json/ab_state.json"
"""
...
class ABStrategyTester:
    ...
    def save_state(self, path: str | None = None) -> None:
        ...
        if not self._enabled:
            return
        fpath = path or self._cfg.get("ab_state_path", "json/ab_state.json")
        with self._lock:
...
def _cli() -> None:
    ...
    ap.add_argument("--state", default="json/ab_state.json", help="Path to ab_state.json")
