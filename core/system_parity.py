...
Scope note (2026-08-21): core.tier_engine.classify_tier() now accepts config-sourced
strong_min/moderate_min/weak_min overrides, and the LIVE path (core.adaptive_signal,
core.execution_policy) passes TIER_STRONG_MIN/TIER_MODERATE_MIN/TIER_WEAK_MIN from
config through to it -- so changing those keys now genuinely changes live tier
classification. core.simulation_engine (backtest) was deliberately left calling
classify_tier()/get_tier_rules() with no overrides, i.e. it always uses this
module's hardcoded constants regardless of config. This means a config change to
those keys makes live diverge from backtest, and assert_backtest_live_parity()
below does NOT detect that divergence (it only compares hardcoded constants to
each other). If backtest/live parity under a non-default TIER_*_MIN config
matters, simulation_engine's SimConfig needs the same config threading -- not
done here to keep this fix scoped to making live config actually take effect.
"""
