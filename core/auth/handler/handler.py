...
class AuthHandler(MfaHandlerMixin, SessionManagerMixin):
    ...
    def __init__(self, db_path: str = "db/auth.db", token_ttl: int = TOKEN_TTL_SECONDS) -> None:
    ...
    def _init_db(self) -> None:
        ...
        The clean source release intentionally excludes runtime databases.
        Ensure the parent directory exists so a fresh local checkout can
        bootstrap authentication without requiring a pre-existing ``db/``
        directory. Production deployments may override this path to a
        mounted persistent directory.
        """
        db_parent = Path(self._db_path).expanduser().parent
    ...
    def _create_default_admin(self, conn: sqlite3.Connection) -> None:
        ...
        If OPBUYING_DEFAULT_ADMIN_PASSWORD isn't set, a random password is
        generated - previously that random value was never surfaced
        anywhere, so a first-time user had no way to ever log in. Print it
        once, here, at creation time (never again afterward - the hash is
        all that's stored) so first run is actually usable.
        """
        auto_generated = "OPBUYING_DEFAULT_ADMIN_PASSWORD" not in os.environ
        ...
        if auto_generated:
            print("=" * 70)
            print(" FIRST-RUN ADMIN LOGIN (shown once - not stored anywhere in plain text)")
    ...
    def create_user(
        ...
        if role not in ("super_admin", "admin", "operator", "viewer", "observer", "developer"):
        ...
        try:
            ...
            return {"success": True, "user_id": user_id, "username": username, "role": role, "email": email, "telegram_chat_id": telegram_chat_id}
    ...
    def list_users(self) -> list[dict[str, Any]]:
        ...
        try:
            cursor = conn.execute(
                "SELECT user_id, username, role, display_name, must_change_password, disabled, created_ts, last_login_ts, metadata "
    ...
    def update_user_metadata(self, username: str, metadata_updates: dict[str, Any], display_name: str | None = None) -> bool:
        """Update user metadata and display name."""
        username = username.strip().lower()
        conn = self._get_conn()
        try:
            cursor = conn.execute("SELECT metadata, display_name FROM users WHERE username = ?", (username,))
            row = cursor.fetchone()
            if not row:
                return False
            meta = {}
            ...
            conn.execute(
                "UPDATE users SET metadata = ?, display_name = ? WHERE username = ?",
                (json.dumps(meta), new_display, username),
            )
            conn.commit()
            return True
        except Exception:
    ...
    def update_user_role(self, username: str, new_role: str, admin_username: str) -> dict[str, Any]:
        ...
        username = username.strip().lower()
        if new_role not in ("super_admin", "admin", "operator", "viewer", "observer", "developer"):
    ...
    def _audit_log(self, event_type: str, username: str, ip_address: str = "", details: dict | None = None, success: bool | None = None) -> None:
        try:
            if success is None:
    ...
    def verify_password_reset_token(self, token_str: str, mark_used: bool = True) -> str | None:
        ...
        try:
            ...
            if row is None or row["used"] or time.time() >= row["expires_ts"]:
                if row is not None and time.time() >= row["expires_ts"]:
            ...
            if mark_used:
                ...
                conn.commit()
            return row["username"]  # type: ignore[no-any-return]
    ...
    def reset_password_with_token(self, token_str: str, new_password: str) -> dict[str, Any]:
        """Reset password using a DB-backed reset token."""
        username = self.verify_password_reset_token(token_str)
        if not username:
            return {"success": False, "error": "Invalid or expired reset token"}
        res = self.admin_reset_password(username, new_password, admin_username="self-service-token")
        if res.get("success"):
            self.revoke_all_user_sessions(username)
            return {"success": True, "username": username}
        return res

    def emergency_master_reset_password(self, username: str, recovery_key: str, new_password: str) -> dict[str, Any]:
        """Emergency self-service password reset using a master recovery authorization key.

        The key is NOT hardcoded. It must be set via the OPBUYING_EMERGENCY_MASTER_RECOVERY_KEY
        env var, following the project's standard OPBUYING_* secret-bridge convention. If unset,
        this feature is disabled entirely (fail-closed). Previously this accepted any one of
        three hardcoded literal strings, one of which was displayed in plaintext on the public,
        unauthenticated /forgot-password page - effectively a published backdoor to reset any
        user's (including admin's) password.
        """
        configured_key = os.environ.get("OPBUYING_EMERGENCY_MASTER_RECOVERY_KEY", "")
        if not configured_key or not hmac.compare_digest(recovery_key, configured_key):
            return {"success": False, "error": "Invalid recovery authorization key"}
        res = self.admin_reset_password(username, new_password, admin_username="emergency-master-key")
        if res.get("success"):
            self.revoke_all_user_sessions(username)
            return {"success": True, "username": username}
        return res
