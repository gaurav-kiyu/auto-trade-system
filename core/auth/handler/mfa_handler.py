...
# Methods (_get_conn, _audit_log) provided by host class at runtime
...
if TYPE_CHECKING:
    ...
    class MfaHandlerHostProtocol:
        def _get_conn(self) -> sqlite3.Connection: ...
        def _audit_log(self, event_type: str, username: str, ip_address: str, details: object = None) -> None: ...
    ...
    def get_mfa_recovery_codes(self, username: str) -> list[str]:
        ...
        try:
            ...
            if row is None or not row["mfa_recovery_codes"]:
                return []
            try:
                return json.loads(row["mfa_recovery_codes"])  # type: ignore[no-any-return]
