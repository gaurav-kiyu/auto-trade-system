...
# Attributes (_lock, _tokens, _token_ttl, _get_conn) provided by host class at runtime
...
if TYPE_CHECKING:
    ...
    class SessionManagerHostProtocol:
...
class SessionManagerMixin:
    ...
    def purge_expired_sessions(self) -> int:
        ...
        try:
            ...
            # A dashboard cleanup worker can outlive a temporary/test database
            # or encounter a database created before the sessions migration.
            # Do not let maintenance crash the background thread.
            if "no such table: sessions" in str(exc).lower():
                _log.warning("[AUTH] Session cleanup skipped: sessions table is unavailable")
            else:
                raise
        finally:
            conn.close()
