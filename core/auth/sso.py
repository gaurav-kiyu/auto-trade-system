...
class SSOAuthenticator:
    ...
    def is_available(self) -> bool:
        ...
        try:
            return find_spec("authlib") is not None
        except ImportError:
            return False
...
def get_public_base_url(config=None):
    """Return the canonical externally visible application base URL.

    Resolution is intentionally centralized so SSO/action URLs do not embed
    deployment-specific localhost or host values.
    """
    config = config or {}
    return (config.get("public_base_url")
            or config.get("base_url")
            or config.get("deployment_url")
            or "").rstrip("/")


def build_action_url(path, config=None):
    """Build an externally visible action URL from the canonical base URL."""
