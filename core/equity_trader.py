...
def _build_asset_symbols(cfg: dict[str, Any], map_name: str) -> list[str]:
    ...
    if not cfg.get(info["enabled_key"], default_enabled):  # type: ignore[index]
        return []

    asset_map = cfg.get(info["map_key"], {})  # type: ignore[index]
    # Priority list if available
    priority = cfg.get(info["priority_key"]) if info["priority_key"] else None  # type: ignore[index]
...
class EquityTrader:
    ...
    def __init__(
        ...
        for asset_class, info in _ASSET_MAP_NAMES.items():
            ...
            if qty_key:
                self._default_qtys[asset_class] = int(self._cfg.get(qty_key, info["default_qty"]))  # type: ignore[index]
            else:
                self._default_qtys[asset_class] = info["default_qty"]  # type: ignore[index]
        ...
        _log.info("[EQUITY] Loaded %d total symbols: %s", len(self._all_symbols), active_counts)
    ...
    def start(self) -> None:
        ...
        self._thread.start()
        _log.info("[EQUITY] Trader started with %d symbols across %d asset classes",
    ...
    def stop(self) -> None:
        ...
        _log.info("[EQUITY] Trader stopped")
    ...
    def enter_position(self, symbol: str, direction: str, score: int,
        ...
        """
        if not self._is_market_open():
            _log.info("[EQUITY] %s: market closed - cannot enter", symbol)
            return False

        with self._lock:
            if self._daily_trades >= self._max_daily_trades:
                _log.info("[EQUITY] %s: max daily trades reached", symbol)
                return False
            if symbol in self._positions:
                _log.info("[EQUITY] %s: already have position", symbol)
                return False
        ...
        if _rt is not None:
            ...
            if not reentry_dec.allowed:
                _log.info("[EQUITY] %s: reentry blocked - %s", symbol, reentry_dec.reason)
                return False

        price = self._get_price_fn(symbol)
        if price is None or price <= 0:
            _log.warning("[EQUITY] %s: cannot get price", symbol)
            return False
        ...
        if self._execute_entry_fn is not None:
            try:
                ...
                _log.error("[EQUITY] %s: entry failed - %s", symbol, e)
                return False
        ...
        _log.info("[EQUITY] Entered %s [%s] %s x%d @ %.2f (score=%d)",
    ...
    def exit_position(self, symbol: str, reason: str) -> bool:
        ...
        if self._execute_exit_fn is not None:
            try:
                ...
                _log.error("[EQUITY] %s: exit execution failed - %s", symbol, e)

        _log.info("[EQUITY] Exited %s: %s @ %.2f (P&L=%.0f)", symbol, reason, current_price, pnl)
        ...
        for symbol in list(self._positions.keys()):
            ...
            with self._lock:
                if symbol in self._positions:
                    ...
                    if current_price > self._positions[symbol].get("peak_price", entry_price):
    ...
    def _run_loop(self) -> None:
        ...
        while self._running and not self._stop_event.is_set():
            try:
                ...
                _log.warning("[EQUITY] Loop error: %s", e)

            self._stop_event.wait(scan_interval)

        _log.info("[EQUITY] Trading loop stopped")
    ...
    def evaluate_equity_signal(self, symbol: str, df1m: Any) -> dict[str, Any] | None:
        ...
        try:
            ...
            # VWAP position check
            if (price > vwap and direction == "BUY") or (price < vwap and direction == "SELL"):
            ...
            _log.debug("[EQUITY] Signal eval failed for %s: %s", symbol, e)
            return None
