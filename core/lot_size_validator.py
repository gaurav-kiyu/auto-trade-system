...
class LotSizeValidator:
    ...
    def get_lot_size(self, index_name: str) -> int:
        ...
        val: int = int(self._cfg.get(config_key, DEFAULT_INDEX_LOT_SIZES.get(index_name, 50)))
        return val

    def validate(self, broker_port: Any = None, strict: bool = True) -> bool:
    ...
    def validate_all(self, broker_port: Any = None) -> list[LotSizeResult]:
    ...
    def validate_one(self, index_name: str, broker_port: Any = None) -> LotSizeResult:
    ...
    def validate_order_size(self, index_name: str, qty: int, broker_port: Any = None) -> tuple[bool, str]:
    ...
    def _get_cached_lot_size(self, index_name: str, broker_port: Any = None) -> int | None:
    ...
    def _get_live_lot_size(self, index_name: str, broker_port: Any = None) -> int | None:
        ...
        if broker_port is not None:
            try:
                ...
                if hasattr(broker_port, "get_lot_size"):
                    return broker_port.get_lot_size(index_name)  # type: ignore[no-any-return]
    ...
    def _fetch_from_nse_api(self, index_name: str) -> int | None:
        ...
        if not _NSE_LOT_API_URL.lower().startswith(("http://", "https://")):
            log.warning("[LOT] Skipping non-http(s) NSE contracts API URL")
            return None
...
def validate_lot_sizes(cfg: dict[str, Any], broker_port: Any = None, strict: bool = True) -> bool:
