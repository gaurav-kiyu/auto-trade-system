...
Cache       : json/iv_history_cache.json, refreshed every 24 h
...
def _ttl_seconds(config: dict[str, Any]) -> float:
    _val = config.get("iv_rank_cache_hours", _DEF_TTL_HRS)
    return float(_val) * 3600.0


def _cache_is_stale(cache: dict[str, Any], ttl: float) -> bool:
    _fetched = cache.get("fetched_at", 0.0)
    return (time.time() - float(_fetched)) > ttl


def _load_file_cache() -> dict[str, Any]:
    try:
        if _CACHE_PATH.exists():
            _data: Any = json.loads(_CACHE_PATH.read_text(encoding="utf-8"))
...
def _fetch_vix_history() -> list[float]:
    ...
    try:
        ...
        min_d = df.index.min().date() if len(df) else "?"
        max_d = df.index.max().date() if len(df) else "?"
        _log.info(f"[IV_RANK] Fetched {len(closes)} VIX sessions ({min_d} → {max_d})")
        return closes
