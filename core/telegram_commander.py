...
class TelegramCommander:
    ...
    def __init__(
        ...
        self._positions_fn    = positions_fn or (list)
        self._pnl_fn          = pnl_fn or (dict)
    ...
    def _get_updates(self) -> list[dict]:
        ...
        try:
            r = requests.get(
                f"https://api.telegram.org/bot{self._token}/getUpdates",  # nosec B113
    ...
    def _dispatch(self, cmd: str, args: list[str], user_id: str, username: str) -> None:
        ...
        # ── Signal history "order placed" marker ───────────────────────────
        elif cmd == "/placed":
            self._cmd_mark_order_placed(args, username, placed=True)
        elif cmd == "/unplaced":
            self._cmd_mark_order_placed(args, username, placed=False)
    ...
    def _cmd_balance(self) -> None:
        """Show current capital breakdown: total, reserved, available."""
        try:
            state = self._state_fn()
            capital = state.get("capital", state.get("BASE_CAPITAL", 0))
            reserved = state.get("reserved_capital", state.get("locked_capital", 0))
            daily_pnl = state.get("daily_pnl", 0)
            open_pos = state.get("open_positions", 0)
            max_open = state.get("max_open", 1)
            available = capital - reserved
            pnl_e = "✅" if daily_pnl >= 0 else "❌"
            lines = [
                f"{'─'*26}",
                "💰 *Balance*",
                f"{'─'*26}",
                ...
                f"{'─'*26}",
                f"Day P&L:   {pnl_e} ₹{daily_pnl:+,.0f}",
                f"Positions: {open_pos}/{max_open}",
                f"{'─'*26}",
            ]
            self._reply("\n".join(lines), critical=False)
        except Exception as exc:
            _log.error("[TG_CMD] Balance unavailable: %s", exc)
            self._reply(f"⚠️ Balance unavailable: {exc}", critical=False)
    ...
    def _cmd_pnl(self) -> None:
        try:
            ...
            lines.append(f"{'─'*26}")
            # Show cumulative metrics when /perf is used
            if self._pnl_fn:
                try:
                    perf = self._pnl_fn()
                    ...
                    if profit_factor > 0:
                        lines.append(f"PF: {profit_factor:.2f}x | Sharpe: {sharpe:.2f}")
                        lines.append(f"{perf_mode} {perf.get('cumulative_pnl', 0):+,.0f} total")
                except (ValueError, TypeError, KeyError, AttributeError) as _perf_err:
                    _log.debug("[TG_CMD] Cumulative perf enrichment skipped: %s", _perf_err)
            self._reply("\n".join(lines), critical=False)
        except Exception as exc:
    ...
    def _cmd_mark_order_placed(self, args: list[str], username: str, placed: bool) -> None:
        """Marks/unmarks a SignalTracker-recorded signal as "I actually placed
        an order off this" - the same historical record the admin dashboard's
        checkbox writes to (core.signals.signal_tracker.mark_order_placed),
        so a reply from Telegram and a click in the dashboard both land in
        the same place rather than two disconnected data paths."""
        cmd_name = "/placed" if placed else "/unplaced"
        if not args:
            self._reply(f"Usage: {cmd_name} {{signal_id}}", critical=False)
            return
        signal_id = args[0]
        try:
            from core.signals.signal_tracker import SignalTracker
            tracker = SignalTracker.get_instance()
            ok = tracker.mark_order_placed(signal_id, placed, f"telegram:{username}")
        except (ValueError, TypeError, KeyError, OSError) as exc:
            self._reply(f"⚠️ Could not update `{signal_id}`: {exc}", critical=False)
            return
        if ok:
            verb = "Marked as ordered" if placed else "Unmarked"
            self._reply(f"✅ {verb}: `{signal_id}`\nBy: {username}", critical=False)
        else:
            self._reply(f"❌ Signal `{signal_id}` not found.", critical=False)
    ...
    def _cmd_help(self) -> None:
        lines = [
            ...
            f"{'─'*28}",
            "🗂️ *Signal History*",
            "  /placed {signal_id}   - mark: I placed this order",
