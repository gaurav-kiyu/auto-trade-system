...
def compute_index_score(
    ...
    *,
    ...
    if (t5 == "UP"   and _rsi_hl_c <= rsi <= _rsi_hh_c) or (t5 == "DOWN" and _rsi_hl_p <= rsi <= _rsi_hh_p):
        s += _rsi_bonus
    ...
    # Keep the historical public behaviour by default, but allow the adaptive
    # pipeline to retain the uncapped component score for proper normalization.
    return max(0, int(s)) if return_raw else max(0, min(100, int(s)))
...
def evaluate_index_signal_partial(
    *,
    ...
    # Frame-alignment tolerances. Resolution order: explicit top-level keys
    # first (the simulation engine disables the check with
    # ``FRAME_ALIGN_1M_5M = 99999`` — that override must win), then the nested
    # ``indicator`` block (canonical home in index_config.defaults.json), then
    # one-full-bar-period defaults. Higher-timeframe bars are only completed
    # every 5m/15m, so a tolerance tighter than one bar period makes the gate
    # impossible to pass with live intraday data.
    ind = sc.get("indicator") or {}
    tol_5m = float(
        sc.get("FRAME_ALIGN_1M_5M")
        or ind.get("frame_align_1m_5m_secs")
        or 360
    )
    tol_15m = float(
        sc.get("FRAME_ALIGN_1M_15M")
        or ind.get("frame_align_1m_15m_secs")
        or 960
    )
    ...
    try:
        if _orb_bonus > 0 and hasattr(df1.index, "time"):
            ...
            if len(_orb_df) >= 5:
                ...
                if (_is_call and price > _orb_high * 1.001) or (_is_put and price < _orb_low * 0.999):
                    _orb_pts = _orb_bonus
    ...
    # Preserve the uncapped evidence score for diagnostics and historical
    # saturation analysis. The public score remains 0..100 for compatibility.
    _raw_score_evidence = int(sum(_score_components.values()) + max(0, int(learning_score_bonus)))
    _score_saturated = _raw_score_evidence > 100
    score = min(100, max(0, _raw_score_evidence))

    # Fallbacks below match json/index_config.defaults.json's flat-key defaults
    # (ATR_SL_MULTIPLIER=1.5, FIB_TP1/2/3_RATIO=0.618/1.0/1.618) -- these are
    # the values actually used whenever the key is present, which it always
    # is today; the fallback only matters if a config layer ever omits it.
    atr_sl_mult = float(sc.get("ATR_SL_MULTIPLIER", 1.5))
    ...
    tps = calc_fibonacci_targets(
        ...
        fib_r2=float(sc.get("FIB_TP2_RATIO", 1.0)),
        fib_r3=float(sc.get("FIB_TP3_RATIO", 1.618)),
        vix=vix,
    )
