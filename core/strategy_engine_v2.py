...
class BaseStrategy:
    ...
    def evaluate(self, features: dict[str, Any], direction: str) -> dict[str, Any]:
        """
        Returns a dict: {"score": int, "status": bool, "reason": str}
        """
...
class MeanReversionStrategy(BaseStrategy):
    def evaluate(self, features: dict[str, Any], direction: str) -> dict[str, Any]:
        ...
        # Only activate in CHOPPY regimes
        if regime != "CHOPPY":
            return {"score": 0, "status": False, "reason": "Not a choppy regime"}

        # Rubber band logic
        dist_pct = abs(price - vwap) / vwap if vwap > 0 else 0
        is_stretched = dist_pct > 0.005  # 0.5% from VWAP — sensible for index-sized handles
        ...
        elif direction == "PUT" and price > vwap and rsi > 55 and is_stretched:
...
class VWAPStrategy(BaseStrategy):
    def evaluate(self, features: dict[str, Any], direction: str) -> dict[str, Any]:
        ...
        # In choppy markets, VWAP acts as a magnet, so trend-following VWAP logic is disabled
        if regime == "CHOPPY":
        ...
        elif direction == "PUT" and vwap_pos == "below":
            return {"score": 15, "status": True, "reason": "Price < VWAP"}
...
class MomentumStrategy(BaseStrategy):
    def evaluate(self, features: dict[str, Any], direction: str) -> dict[str, Any]:
        ...
        elif direction == "PUT" and hist < 0 and m_line < s_line:
...
class SmartMoneyStrategy(BaseStrategy):
    def evaluate(self, features: dict[str, Any], direction: str) -> dict[str, Any]:
        ...
        if (direction == "CALL" and pcr > pcr_bull) or (direction == "PUT" and pcr < pcr_bear):
            score += 5
            reasons.append(f"PCR Supports ({pcr:.2f})")
            status = True
        else:
