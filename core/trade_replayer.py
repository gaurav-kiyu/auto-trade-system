...
def load_trade(trade_id: int, db_path: str = _DEFAULT_DB) -> dict | None:
    ...
    except (ValueError, TypeError, KeyError, AttributeError, IndexError, OSError, ConnectionError, TimeoutError) as exc:
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            if worst:
                # 'where' holds static literals only; all user data is parameterized
                sql = (
                    f"SELECT * FROM trades WHERE {' AND '.join(where)} "  # nosec B608
