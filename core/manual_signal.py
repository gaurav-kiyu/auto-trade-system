...
-----------
    ...
    manual_signal_db_path          : str    default "db/manual_signals.db"
...
class ManualSignalQueue:
    ...
    def __init__(self, cfg: dict[str, Any]) -> None:
        self._cfg = cfg
        self._db_path = str(cfg.get("manual_signal_db_path", "db/manual_signals.db"))
    ...
    def _update(self, signal_id: str, fields: dict[str, Any]) -> None:
        ...
        self._conn.execute(
            f"UPDATE manual_signals SET {set_clause} WHERE signal_id=?", vals,  # nosec B608
        )
        self._conn.commit()
