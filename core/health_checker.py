...
-----------------------------------------
    ...
    health_check_db_warn_mb       : dict   default {"db/trades.db":50,"db/ml_tracker.db":200,...}
...
_DB_WARN_MB_DEFAULTS: dict[str, float] = {
    "db/trades.db":       50.0,
    "db/ml_tracker.db":   200.0,
    "db/trade_journal.db": 100.0,
    "db/oi_snapshots.db": 500.0,
}
...
def check_ml_health(cfg: dict[str, Any]) -> list[HealthCheckResult]:
    ...
    try:
        ...
        db_path = cfg.get("ml_tracker_db_path", "db/ml_tracker.db")
...
def check_broker_health(cfg: dict[str, Any]) -> list[HealthCheckResult]:
    ...
    results: list[HealthCheckResult] = []
    try:
        # If running in Paper or Signal-Only mode, consider healthy by default
        is_paper = cfg.get("paper_mode", True) or cfg.get("execution_mode") in ("SIGNAL_ONLY", "PAPER")
        import importlib
        ...
        if broker is None:
            if is_paper:
                results.append(HealthCheckResult(
                    "BROKER", "Broker availability", "OK", True,
                    "Paper / Simulation mode active.",
                ))
            else:
                results.append(HealthCheckResult(
        ...
        results.append(HealthCheckResult(
            "BROKER", "Broker connection", "OK" if healthy else ("OK" if is_paper else "FAIL"),
            healthy, "Broker is connected and authenticated." if healthy else "Paper simulation active.",
        ))
