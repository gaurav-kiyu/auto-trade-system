...
class PositionSizingManager:
    ...
    def get_volatility_multiplier(self, volatility: float) -> float:
        if volatility <= self.config.vix_threshold_low:
            return self.config.vix_size_multiplier_low  # type: ignore[no-any-return]
        if volatility >= self.config.vix_threshold_high:
            return self.config.vix_size_multiplier_high  # type: ignore[no-any-return]
        ratio = (volatility - self.config.vix_threshold_low) / (self.config.vix_threshold_high - self.config.vix_threshold_low)
        ratio = max(0, min(1, ratio))
        return self.config.vix_size_multiplier_low + (ratio * (self.config.vix_size_multiplier_high - self.config.vix_size_multiplier_low))  # type: ignore[no-any-return]
