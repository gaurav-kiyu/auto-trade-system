    ...
    while not _shutdown.is_set():
        try:
            if is_kill_file_present():
                ...
                # ALSO signal graceful shutdown so the main trading loop can exit
                # cleanly (state flush, EOD report, WS disconnect). Without this the
                # kill file only blocks entries and the process keeps scanning.
                # Note: this runs in the watcher thread; callbacks must not block on
                # main-loop locks (execute_shutdown is idempotent and lock-guarded).
                request_shutdown("STOP_TRADING file found in project root")
                break
...
def trip_hard_halt(reason: str, *, source: str = "", cfg: dict | None = None) -> None:
    ...
    Args:
        ...
        cfg: Optional config dict. Only used to gate the STRUCTURED_EVENTS_ENABLED
            structured-log wire-in below; no caller is required to pass it and
    ...
    # First STRUCTURED_EVENTS_ENABLED integration point (default False = no-op).
    # core/log_helpers.py::log_structured_event() had zero call sites before this;
    # hard-halt is rare and high-signal, not exhaustive coverage of every event.
    if cfg is not None and cfg.get("STRUCTURED_EVENTS_ENABLED", False):
        try:
            from core.log_helpers import log_structured_event
            log_structured_event(_log, "hard_halt_tripped", {"reason": reason, "source": source})
        except (ValueError, TypeError, KeyError, AttributeError, IndexError, OSError):
            _log.debug("[STRUCTURED_EVENTS] hard_halt_tripped logging failed", exc_info=True)
