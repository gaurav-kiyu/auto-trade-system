...
def register_schema(version: int, description: str) -> Callable:
    ...
    Registration is **idempotent**: if a migration with the same version is already
    registered, it is replaced in place. This keeps ``_SCHEMA_REGISTRY`` free of
    duplicate versions when modules are imported/reloaded more than once (e.g. by
    tests calling ``importlib.reload``), which would otherwise trip
    ``_verify_migration_order()`` with a "vN followed by vN" error.
    """
    def decorator(func: Callable[[sqlite3.Connection], None]) -> Callable:
        for i, existing in enumerate(_SCHEMA_REGISTRY):
            if existing.version == version:
                log.warning(
                    "Replacing duplicate schema migration v%d ('%s' -> '%s')",
                    version, existing.description, description,
                )
                _SCHEMA_REGISTRY[i] = Migration(
                    version=version, description=description, apply=func,
                )
                return func
