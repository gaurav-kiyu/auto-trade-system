...
def _check_otel_available() -> bool:
    ...
    importlib.util.find_spec() for a dotted name (e.g. "opentelemetry.sdk.trace")
    has to import each parent package to look inside it - if the base
    "opentelemetry" package (opentelemetry-api) is installed but "opentelemetry.sdk"
    (a separate package, opentelemetry-sdk) is not, find_spec raises
    ModuleNotFoundError instead of returning None. Without the try/except
    here, that turned "otel not installed" (a graceful-degradation case this
    module is explicitly designed to handle) into an unhandled crash.
    """
...
def shutdown_tracing() -> None:
    ...
    global _tracer_provider, _tracing_enabled, _otel_available
...
class trace_event:
    ...
    def __init__(self, span_name: str, attributes: dict[str, Any] | None = None) -> None:
