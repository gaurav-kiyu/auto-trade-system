"""Observability metrics — Prometheus metric definitions and ObservabilityManager.

Moved from core/observability.py into the core/observability/ package.
"""
import logging
import time
...
class ObservabilityManager:
    ...
    def record_order_latency(self, start_time: float) -> None:
        """Records the time taken to execute an order."""
        latency = time.time() - start_time
        ORDER_LATENCY.observe(latency)

    def record_slippage(self, symbol: str, direction: str, expected: float, actual: float) -> None:
    ...
    def update_broker_health(self, broker_name: str, is_healthy: bool) -> None:
        BROKER_HEALTH.labels(broker_name=broker_name).set(1 if is_healthy else 0)

    def increment_ml_fallback(self) -> None:
        ML_FALLBACK_COUNT.inc()

    def update_risk_metrics(self, current_pnl: float, max_loss: float) -> None:
