...
class RiskService:
    ...
    def _reset_daily_tracking(self) -> None:
        """Reset daily P&L and loss streak at the start of a new IST trading day."""
        today = now_ist().date()
        if today != self._last_reset_date:
            _log.info("[RISK] New trading day %s: resetting daily tracking metrics", today)
            self._daily_pnl = 0.0
            self._consecutive_losses = 0
            self._daily_start_equity = self._get_current_equity()
    ...
    def _get_historical_stats(self, symbol: str, direction: str, portfolio_state: dict[str, Any]) -> dict[str, float]:
        """Get historical statistics for Kelly calculation from portfolio state history."""
        trade_history = portfolio_state.get("trade_history") or portfolio_state.get("trades") or []
        relevant_trades = [
            t for t in trade_history
            if (t.get("symbol") == symbol or not symbol)
            and (t.get("direction") == direction or not direction)
            and "pnl" in t
        ]
        if not relevant_trades:
            # Fall back to whole-portfolio closed trades if symbol-specific history is absent.
            relevant_trades = [t for t in trade_history if "pnl" in t]

        if not relevant_trades:
            # Without historical data, Kelly sizing cannot estimate win rate/odds.
            return {"win_rate": 0.0, "avg_win": 0.0, "avg_loss": 0.0, "sample_size": 0.0}

        pnls = [float(t["pnl"]) for t in relevant_trades if t.get("pnl") is not None]
        if not pnls:
            return {"win_rate": 0.0, "avg_win": 0.0, "avg_loss": 0.0, "sample_size": 0.0}
    ...
    def _calculate_new_exposure(
        ...
        prices = portfolio_state.get("prices", {}) or portfolio_state.get("market_prices", {}) or {}
        price = prices.get(symbol)
        if price is None:
            for position in portfolio_state.get("positions", []):
                if getattr(position, "symbol", None) == symbol:
                    price = getattr(position, "average_price", None)
                    break
        if price is None:
            # Missing market price is a data-quality failure; conservatively block by
            # reporting exposure at the configured limit rather than inventing a price.
            return max(current_exposure, self.risk_limits.max_portfolio_exposure)
        total_equity = self._get_current_equity()
        estimated_position_value = abs(float(price)) * abs(int(suggested_size))
        new_exposure = current_exposure + (estimated_position_value / total_equity if total_equity > 0 else 1.0)
        return min(max(new_exposure, 0.0), 1.0)
    ...
    def _calculate_max_correlation(
        ...
        correlations = portfolio_state.get("correlations", {}) or {}
        observed: list[float] = []
        for position in positions:
            pos_sym = getattr(position, "symbol", "")
            if pos_sym == symbol:
                observed.append(1.0)
                continue
            key_candidates = [f"{symbol}:{pos_sym}", f"{pos_sym}:{symbol}"]
            value = next((correlations[k] for k in key_candidates if k in correlations), None)
            if value is not None:
                try:
                    ...
                    pass
        if observed:
            return min(max(max(observed), 0.0), 1.0)
        # Without pairwise return history, same-direction exposure remains a
        # conservative concentration signal rather than a fabricated correlation.
        return 0.6 if any(getattr(p, "direction", "") == direction for p in positions) else 0.0
    ...
    def _estimate_portfolio_volatility(self, portfolio_state: dict[str, Any]) -> float:
        """Estimate portfolio volatility from empirical return series or position weights."""
        position_vols = portfolio_state.get("position_volatilities", {}) or {}
        if position_vols:
            weights = []
            total = 0.0
            for position in portfolio_state.get("positions", []):
                pos_sym = getattr(position, "symbol", "")
                vol = position_vols.get(pos_sym)
                if vol is None:
                    continue
                value = abs(float(getattr(position, "market_value", 0.0)))
                weights.append((value, max(float(vol), 0.0)))
                total += value
            if total > 0 and weights:
                return sum((value / total) * vol for value, vol in weights)

        returns = portfolio_state.get("portfolio_returns", []) or []
        if len(returns) >= 2:
            try:
                return statistics.stdev(float(r) for r in returns) * (252 ** 0.5)
    ...
    def _calculate_trade_risk_metrics(
        ...
        volatility = max(float(market_conditions.volatility), 0.0)
        estimated_risk = notional * volatility / (252 ** 0.5) if notional > 0 else 0.0
        equity = max(self._get_current_equity(), 1.0)
        return {
            "symbol": symbol,
            "position_size": size,
            "direction": direction,
            ...
            "risk_to_equity": estimated_risk / equity,
            "portfolio_impact": notional / equity,
    ...
    def update_portfolio_risk(self, positions: list[Position]) -> PortfolioRiskMetrics:
        ...
        with self._risk_lock:
            ...
            # Estimate volatility only from supplied empirical position/portfolio returns.
            ...
            # One-day 95% parametric VaR. Zero when no empirical volatility is available.
            daily_volatility = portfolio_volatility / (252 ** 0.5)
