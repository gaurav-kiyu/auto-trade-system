...
class SignalService:
    ...
    def generate_signal(
        ...
        try:
            ...
            # Step 7: Add metadata required by quality scoring.
            volumes = [c.volume for c in market_data.candles[-20:]]
            volume_trend = (statistics.mean(volumes[-5:]) / statistics.mean(volumes[:-5])
                            if len(volumes) >= 10 and statistics.mean(volumes[:-5]) > 0 else 1.0)
            ...
            # Step 8: Final signal validation and quality scoring
            ...
            return self._create_weak_signal(symbol, f"Signal generation error: {e!s}")
    ...
    def _calculate_all_indicators(self, market_data: MarketData) -> TechnicalIndicators:
        ...
        if cache_key in self._indicator_cache:
            return self._indicator_cache[cache_key]  # type: ignore[no-any-return]
        ...
        indicators.current_price = market_data.candles[-1].close if market_data.candles else None
    ...
    def _generate_sentiment_signal(self, market_data: MarketData) -> float:
        ...
        return float(max(-1.0, min(1.0, price_change * 10)))  # Amplify small moves
    ...
    def _validate_signal_quality(self, signal: TradingSignal) -> float:
        ...
        # Factor 3: Time consistency from recent directional evidence (0-0.2).
        history = signal.metadata.get("recent_directions", [])
        if history:
            same_direction = sum(1 for item in history if item == signal.direction)
            consistency_score = 0.2 * same_direction / len(history)
        else:
            consistency_score = 0.0
        quality_score += consistency_score
        max_score += 0.2

        # Factor 4: Volume confirmation from measured recent-volume ratio (0-0.2).
        volume_trend = max(float(signal.metadata.get("volume_trend", 1.0)), 1e-9)
        volume_score = 0.2 * max(0.0, min(1.0, 1.0 - abs(math.log(volume_trend))))
        quality_score += volume_score
        max_score += 0.2

        # Factor 5: Market-structure alignment (0-0.1).
        regime = str(signal.metadata.get("regime", "UNKNOWN"))
        aligned = ((signal.direction == "BUY" and regime in {"STRONG_UPTREND", "WEAK_UPTREND", "ACCUMULATION"})
                   or (signal.direction == "SELL" and regime in {"STRONG_DOWNTREND", "WEAK_DOWNTREND", "DISTRIBUTION"}))
        structure_score = 0.1 if aligned else 0.0
    ...
    def _calculate_sma(self, data: list[Any], period: int) -> float | None:
        ...
        return sum(closes) / len(closes)  # type: ignore[no-any-return]
    ...
    def _calculate_ema(self, data: list[Any], period: int) -> float | None:
        ...
        return ema  # type: ignore[no-any-return]

    @staticmethod
    def _ema_numeric(values: list[float], period: int) -> float:
    ...
    def _ema_values(self, values: list[float], period: int) -> list[float]:
        if not values:
            return []
        multiplier = 2.0 / (period + 1.0)
        ema = values[0]
    ...
    def _calculate_macd(self, data: list[Any]) -> tuple[float | None, float | None, float | None]:
        ...
        if ema_12 is None or ema_26 is None:
            return None, None, None
        macd = ema_12 - ema_26
        # Build the MACD series, then calculate its 9-period EMA signal line.
        macd_series = []
        for i in range(25, len(closes)):
            fast = self._ema_values(closes[: i + 1], 12)[-1]
            slow = self._ema_values(closes[: i + 1], 26)[-1]
            macd_series.append(fast - slow)
        if len(macd_series) < 9:
            return macd, None, None
        macd_signal = self._ema_numeric(macd_series, 9)
    ...
    def _calculate_atr(self, data: list[Any], period: int) -> float | None:
        ...
        if len(tr_values) < period:
            return None
        return sum(tr_values[-period:]) / period  # type: ignore[no-any-return]

    def _calculate_adx(self, data: list[Any], period: int) -> float | None:
        """Calculate Wilder-style ADX from OHLC data."""
        ...
        for i in range(1, len(data)):
            ...
            up = high - data[i - 1].high
            down = data[i - 1].low - low
    ...
    def _calculate_stochastic(self, data: list[Any], k_period: int, d_period: int) -> tuple[float | None, float | None]:
        """Calculate %K and %D stochastic oscillator."""
        if len(data) < k_period + d_period - 1:
            return None, None
        ks = []
        for end in range(k_period, len(data) + 1):
            window = data[end-k_period:end]
            high = max(c.high for c in window)
            low = min(c.low for c in window)
            close = data[end-1].close
            ks.append(100.0 * (close - low) / (high - low) if high > low else 50.0)
        return ks[-1], sum(ks[-d_period:]) / d_period
