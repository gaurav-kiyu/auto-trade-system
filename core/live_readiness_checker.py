...
def _load_paper_trades(db_path: str, days: int) -> list[dict]:
    ...
    try:
        ...
        try:
            ...
            sql = f"SELECT * FROM trades WHERE {' AND '.join(where)} ORDER BY ts"  # nosec B608
...
def check_live_readiness(
    ...
    try:
        ...
        ml_acc = 0.5
        if n_total > 0:
            # compute_calibration()'s bin dicts carry predicted_mean/actual_rate,
            # not a "calibrated" flag (that key never existed) — a bin counts as
            # well-calibrated when its actual win rate tracks its predicted
            # probability within tolerance.
            tolerance = float(c.get("ml_calibration_tolerance", 0.15))
            correct = sum(
                b.get("count", 0)
                for b in cal_bins
                if abs(b.get("actual_rate", 0.0) - b.get("predicted_mean", 0.0)) <= tolerance
            )
