...
class TradeMandateEnforcer:
    ...
    def update_market_state(self, vix: float, last_tick_time: datetime) -> None:
        self._current_vix = vix
        self._last_data_time = last_tick_time

    def update_capital_state(self, capital: float, equity_peak: float, daily_pnl: float, weekly_pnl: float) -> None:
    ...
    def get_operating_mode(self) -> str:
        ...
        if vix > 28 or (vix > 25 and self._loss_streak >= 2):
    ...
    def _get_min_score_by_regime(self, regime: str) -> int:
        ...
        if r == "SIDEWAYS" or r == "NEUTRAL":
            return self.cfg.score_sideways_min
        if r == "RANGE" or r == "CHOPPY":
