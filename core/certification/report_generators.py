...
def generate_architecture_certification(
    ...
    schema_exists, schema_evidence = _check_file_exists("json/index_config.defaults.json")
    criteria.append(CertCriteria(
        id="ARCH-06",
...
def generate_production_certification(
    ...
    - Independent Audit
    """
    # When called directly without an explicit configuration, use the
    # live production configuration, matching generate_all_reports().
    if config is None:
        config = _load_config()
...
def _load_config() -> dict[str, Any]:
    ...
    try:
        ...
        import json
        from pathlib import Path
        defaults_path = Path("json/index_config.defaults.json")
        cfg: dict[str, Any] = {}
