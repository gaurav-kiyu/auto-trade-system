...
-----
    ...
    cert = PaperCertifier()
    report = cert.certify(db_path="db/trades.db")
...
class PaperCertifier:
    ...
    def __init__(self) -> None:
        self._execution_db = "db/trade_journal.db"
    ...
    def certify(self, db_path: str = _DEFAULT_DB) -> PaperCertificationReport:
        ...
        try:
            ...
            if exec_db is not None:
                try:
                    # Check if execution_quality table exists before querying
                    eq_table_check = exec_db.fetchall(
                        "SELECT name FROM sqlite_master WHERE type='table' AND name='execution_quality'",
                    )
                    if not eq_table_check:
...
def certify_paper_trading(db_path: str = _DEFAULT_DB) -> PaperCertificationReport:
    ...
    Usage:
        report = certify_paper_trading("db/trades.db")
        print(report.summary())
    """
