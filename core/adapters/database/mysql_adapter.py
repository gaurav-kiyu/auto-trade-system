...
Usage:
    ...
    # Context manager:
    with MySQLDatabaseAdapter(host="localhost", user="root", password=os.getenv("DB_PASSWORD", ""), database="trading") as db:
...
def _parse_mysql_dsn(dsn: str) -> dict[str, Any]:
    ...
    if len(parts) >= 3:
        result: dict[str, Any] = {
            "host": parts[0],  # type: ignore[index]
            "port": int(parts[1]) if parts[1].isdigit() else 3306,  # type: ignore[index]
            "database": parts[2],  # type: ignore[index]
        }
        if len(parts) > 3:
            result["user"] = parts[3]  # type: ignore[index]
        if len(parts) > 4:
            result["password"] = parts[4]  # type: ignore[index]
        return result  # type: ignore[no-any-return]


    # Just a hostname
    return {"host": dsn, "port": 3306}  # type: ignore[no-any-return]
...
class MySQLDatabaseAdapter(DatabasePort):
    ...
    def is_connected(self) -> bool:
        ...
        try:
            return self._conn.open  # type: ignore[no-any-return]

        except Exception as _conn_exc:
            _log.debug("[MYSQL_DB] Connection check error: %s", _conn_exc)
            return False  # type: ignore[no-any-return]
    ...
    def execute_many(
        ...
        with self._lock:
            try:
                ...
                return len(params_list)  # type: ignore[no-any-return]
    ...
    def table_exists(self, table_name: str) -> bool:
        ...
        return row is not None  # type: ignore[no-any-return]
    ...
    def stats(self) -> DatabaseStats:
        ...
        return DatabaseStats(  # type: ignore[no-any-return]
    ...
    def _require_conn(self) -> Any:
        ...
        return self._conn  # type: ignore[no-any-return]
