...
def _parse_redis_dsn(dsn: str) -> dict[str, Any]:
    ...
    if len(parts) >= 2:
        result: dict[str, Any] = {
            ...
            "port": int(parts[1]) if parts[1].isdigit() else 6379,  # type: ignore[index]
        }
        if len(parts) > 2 and parts[2].isdigit():  # type: ignore[index]
            result["db"] = int(parts[2])  # type: ignore[index]
        if len(parts) > 3:
            result["password"] = parts[3]  # type: ignore[index]
        return result  # type: ignore[no-any-return]


    return {"host": dsn, "port": 6379}  # type: ignore[no-any-return]
...
class RedisDatabaseAdapter(DatabasePort):
    ...
    def is_connected(self) -> bool:
        ...
        try:
            ...
            _log.debug("[REDIS_DB] Connection check failed: %s", _conn_exc)
            return False  # type: ignore[no-any-return]
    ...
    def execute_many(
        ...
        with self._lock:
            try:
                ...
                return len(results)  # type: ignore[no-any-return]
    ...
    def fetchone(self, sql: str, params: tuple[Any, ...] | dict[str, Any] = ()) -> Any | None:
        ...
        """
        try:
            return self.execute(sql, params)  # type: ignore[no-any-return]
    ...
    def table_exists(self, table_name: str) -> bool:
        ...
        try:
            ...
            _log.debug("[REDIS_DB] table_exists error: %s", _te_exc)
            return False  # type: ignore[no-any-return]
    ...
    def _parse_redis_version(self, info: Any) -> str | None:
        ...
        if isinstance(info, dict):
            return info.get("redis_version")  # type: ignore[no-any-return]
