...
-----
    ...
    report = reporter.generate_trade_register(db_path="db/trades.db")
...
class ComplianceReport:
    ...
    generated_at: str = field(default_factory=lambda: now_ist().isoformat())
    report_period: str = ""
    trader_id: str = ""
    broker_name: str = ""
    ...
    def generate_risk_limits_report(self) -> ComplianceReport:
        ...
        try:
            ...
            try:
                ...
                ts_file = _Path("json/trader_state.json")
                if ts_file.is_file():
            ...
            report.entries = entries
            over_limit = [e for e in entries if float(e.get("utilization_pct", 0)) >= 100]
            if over_limit:
