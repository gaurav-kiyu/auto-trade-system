...
def compute_max_pain(  # OptionChainStrike/float false pos
    spot_price: float,
    ...
    min_pain_strike = min(pain_curve, key=lambda k: pain_curve[k])
    ...
    for offset in range(-2, 3):
        ...
        if 0 <= i < len(sorted_strikes):
            s = sorted_strikes[i]
            diff_pct = abs(pain_curve[s] / min_pain_value - 1) * 100 if min_pain_value > 0 else 0  # type: ignore[index]
            nearest_strikes.append({
                "strike": s,
                "pain": pain_curve[s],  # type: ignore[index]
