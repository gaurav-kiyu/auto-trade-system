...
class RiskBudgetEngine:
    ...
    def __init__(self, total_capital: float = 0.0, cfg: dict[str, Any] | None = None) -> None:
    ...
    def compute_risk_parity_allocation(
        ...
        try:
            ...
            returns = dict.fromkeys(volatilities, 0.0)
            cov = {k: {k2: v * v2 * 0.3 for k2, v2 in volatilities.items()}
                   for k, v in volatilities.items()}

            _rp_result = opt.risk_parity(returns, cov)
            if isinstance(_rp_result, dict) and _rp_result.get("weights"):
                budget_map = {
                    k: v * 100.0 for k, v in _rp_result["weights"].items()
                }
    ...
    def compute_equal_risk_contribution(
        ...
        try:
            ...
            _erc_result = opt.equal_risk_contribution(returns, cov)
            if isinstance(_erc_result, dict) and _erc_result.get("weights"):
                budget_map = {
                    k: v * 100.0 for k, v in _erc_result["weights"].items()
                }
                return self.allocate_risk_budget(budget_map, capital)
        except ImportError:
        ...
        equal_pct = 100.0 / n
        budget_map = dict.fromkeys(volatilities, equal_pct)
        result = self.allocate_risk_budget(budget_map, capital)
