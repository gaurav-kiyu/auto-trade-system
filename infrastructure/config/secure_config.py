...
class SecureConfig:
    ...
    def _load_defaults(self, path: str | Path) -> None:
        ...
        try:
            # encoding="utf-8-sig" — the defaults file may contain non-ASCII
            # (comments/emoji).  The Windows default codec (cp1252) would raise
            # UnicodeDecodeError on valid UTF-8 bytes; utf-8-sig also tolerates
            # a UTF-8 BOM and matches the rest of the codebase.
            ...
            raise SecureConfigError(f"Invalid JSON in defaults file {path}: {e}") from e
