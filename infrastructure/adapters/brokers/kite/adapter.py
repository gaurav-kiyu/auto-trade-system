...
class KiteBrokerAdapter(LegacyBrokerPort):
    ...
    def __init__(
        ...
        # Instrument cache - keyed per-exchange (a single flat cache built
        # from one exchange's instruments() dump would silently return
        # "not found" for another exchange's symbols within the TTL window,
        # e.g. NFO option lookups after an earlier NSE equity lookup).
        self._instruments_cache: dict[str, dict[str, int]] = {}
        self._instruments_cache_time: dict[str, float] = {}
    ...
    def _get_instrument_token(
        ...
        cache_time = self._instruments_cache_time.get(exchange, 0.0)
        if cached is not None and now - cache_time < self._cache_ttl:
            ...
            self._instruments_cache_time[exchange] = now
    ...
    def place_order(self, order: Order) -> str:
        ...
        if not self._connected or self._kite is None:
            raise RuntimeError("Kite adapter is not connected - call connect() first") from None

        symbol = getattr(order, "symbol", "")
        instrument_token = self._get_instrument_token(symbol)
        if instrument_token is None:
            raise RuntimeError(f"Cannot resolve instrument token for {symbol}") from None
    ...
    def get_fill_price(self, order_id: str) -> float | None:
        """Return the average fill price for a Kite order, or None if not found/filled.

        Added because ``core.adapters.broker_adapters.BrokerAdapter`` (the
        legacy wrapper this class is always constructed inside of) only
        delegates to the wrapped port when the port itself implements this
        method — without it, every live Kite exit silently recorded zero
        P&L regardless of actual price movement.
        """
        if not self._connected or self._kite is None:
            return None
        try:
    ...
    def get_filled_quantity(self, order_id: str) -> int | None:
        """Return the filled quantity for a Kite order, or None if not found."""
        if not self._connected or self._kite is None:
            return None
        try:
            orders = self._make_request(self._kite.orders)
            for o in orders:
                if o.get("order_id") == order_id:
                    try:
                        return int(o.get("filled_quantity") or 0)
                    except (TypeError, ValueError):
                        return None
            return None
        except (OSError, ConnectionError, TimeoutError, RuntimeError, ValueError, TypeError, KeyError):
    ...
    def get_quote(self, symbol: str, exchange: str = "NSE") -> Quote:
        ...
        Args:
            symbol: Trading symbol (e.g. "NIFTY", or an NFO option
                tradingsymbol like "NIFTY24DEC23500CE").
            exchange: Exchange the symbol is listed on. Pass "NFO" for
                index/stock options - the instrument-token cache is kept
                per-exchange so mixing NSE and NFO lookups in the same
        ...
        instrument_token = self._get_instrument_token(symbol, exchange=exchange)
        if instrument_token is None:
            raise RuntimeError(f"Cannot resolve instrument token for {symbol}") from None
