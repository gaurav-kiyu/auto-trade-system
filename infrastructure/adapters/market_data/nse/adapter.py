...
class NSEAdapter(MarketDataPort):
    ...
    def get_quote(self, symbol: str) -> Quote:
        ...
        try:
            ...
            # If we got here, we didn't get valid data from NSE direct endpoint
            self._get_logger()
            self._logger.debug(f"NSE direct API returned empty for {symbol}, trying yfinance fallback...")

        except (OSError, ConnectionError, TimeoutError, ValueError, TypeError, KeyError, json.JSONDecodeError) as e:
            self._get_logger()
            self._logger.debug(f"NSE direct API error for {symbol}: {e}, switching to yfinance fallback")

        # Seamless Fallback to yfinance if NSE direct scraping failed or is blocked by Cloudflare
        try:
            import yfinance as yf
            sym_clean = symbol.upper().strip()
            if sym_clean == "NIFTY":
                yf_sym = "^NSEI"
            elif sym_clean == "BANKNIFTY":
                yf_sym = "^NSEBANK"
            elif sym_clean == "FINNIFTY":
                yf_sym = "NIFTY_FIN_SERVICE.NS"
            elif sym_clean == "MIDCPNIFTY":
                yf_sym = "NIFTY_MID_SELECT.NS"
            elif sym_clean == "SENSEX":
                yf_sym = "^BSESN"
            elif not sym_clean.endswith(".NS") and not sym_clean.endswith(".BO"):
            ...
            last_price = 0.0
            if fast_info:
                last_price = float(getattr(fast_info, "last_price", 0.0) or getattr(fast_info, "previous_close", 0.0) or 0.0)
            if last_price <= 0:
            ...
            self._get_logger()
            self._logger.debug(f"[NSE] yfinance quote fallback failed for {symbol}: {ex}")
