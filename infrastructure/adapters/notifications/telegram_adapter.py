    ...
    def _resolve_channel(self, signal: dict) -> list:
        ...
        if signal.get("direction") in ("CALL", "PUT") and signal.get("signal") != "HOLD":
            ...
            if asset_type == "index":
                ...
                if idx_id and idx_id not in raw_targets:
                    raw_targets.append(idx_id)

        if strength == "STRONG":
            strong_id = self.channel_map.get("STRONG")
            if strong_id and strong_id not in raw_targets:
                raw_targets.append(strong_id)

        if not raw_targets:
            raw_targets.append(self.default_chat_id)

        # Expand any comma-separated chat IDs into individual target IDs
        final_targets = []
        for item in raw_targets:
            if isinstance(item, str):
                for cid in item.split(","):
                    clean_cid = cid.strip()
                    if clean_cid and clean_cid not in final_targets:
                        final_targets.append(clean_cid)
            elif item and item not in final_targets:
...
class TelegramNotificationAdapter(NotificationPort):
    ...
    def send_raw(self, text: str, chat_id: str | None = None, critical: bool = False) -> bool:
        """Send arbitrary text directly, bypassing the signal-alert format.

        Public passthrough to the embedded _TelegramClient, for callers (e.g.
        core.ics_telegram_bridge) that need free-form text delivery - an
        incident alert isn't a trading signal, so routing it through
        send_notification()'s Notification->signal_dict conversion would be
        the wrong shape entirely. Exists so those callers don't need to
        reach into the private _TelegramClient themselves.
        """
        return self._client.send_raw(text, chat_id=chat_id, critical=critical)
